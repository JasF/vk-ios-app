...
class FriendsService:
    def getFriends(self, offset):
        ids = self.getFriendsIds(offset)
        usersData = users.getShortUsersByIds(set(ids))
        return {'response':usersData}

    def getFriendsIds(self, offset):
        api = vk.api()
        try:
            db = FriendsDatabase()
            friendsArray = db.getFriendsIds(offset, g_count)
            if not friendsArray:
                friendsArray = []
            ...
            response = api.friends.get(offset=offset, count=g_count, order='hints')
            l = response['items']
