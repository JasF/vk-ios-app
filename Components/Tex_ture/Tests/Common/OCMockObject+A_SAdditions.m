...
@interface A_STestCase (OCMockObjectRegistering)

- (void)registerMockObject:(id)mockObject;

@end

@implementation OCMockObject (A_SAdditions)
...
{
  [self swizzled_init];
  [A_STestCase.currentTestCase registerMockObject:self];
  return self;
}
