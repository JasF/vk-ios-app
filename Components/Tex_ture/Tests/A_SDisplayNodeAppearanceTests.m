//
//  A_SDisplayNodeAppearanceTests.m
//  Tex_ture
//
...
@interface A_SDisplayNode (PrivateStuffSoWeDontPullInCPPInternalH)
...
@interface A_SDisplayNodeAppearanceTests : XCTestCase
@end
...
@implementation A_SDisplayNodeAppearanceTests
{
  _A_SDisplayView *_view;
...
- (void)setUp
{
  ...
  dispatch_block_t cleanupBlock = modifyMethodByAddingPrologueBlockAndReturnCleanupBlock([A_SDisplayNode class], @selector(willEnterHierarchy), ^(id blockSelf){
    [_willEnterHierarchyCounts addObject:blockSelf];
  });
  [_swizzleCleanupBlocks addObject:cleanupBlock];
  cleanupBlock = modifyMethodByAddingPrologueBlockAndReturnCleanupBlock([A_SDisplayNode class], @selector(didExitHierarchy), ^(id blockSelf){
...
{
  // A_SDisplayNode visibility does not change if modifying a hierarchy that is not in a window.  So create one and add the superview to it.
...
- (void)checkManualAppearanceViewLoaded:(BOOL)isViewLoaded layerBacked:(BOOL)isLayerBacked
{
  ...
  for (A_SDisplayNode *n in @[parent, a, b, aa, ab]) {
    n.layerBacked = isLayerBacked;
    if (isViewLoaded)
      [n layer];
  }
...
- (void)testSynchronousIntermediaryView
{
  ...
  A_SDisplayNode *parentSynchronousNode = [[A_SDisplayNode alloc] initWithViewClass:[UIScrollView class]];
...
- (void)checkMoveAcrossHierarchyLayerBacked:(BOOL)isLayerBacked useManualCalls:(BOOL)useManualDisable useNodeAPI:(BOOL)useNodeAPI
{
  ...
  for (A_SDisplayNode *n in @[parentA, parentB, child, childSubnode]) {
    n.layerBacked = isLayerBacked;
  }
  ...
  if (useManualDisable) {
    A_SDisplayNodeDisableHierarchyNotifications(child);
  }
  if (!useNodeAPI) {
  ...
  if (useManualDisable) {
    ...
    A_SDisplayNodeEnableHierarchyNotifications(child);
  }
