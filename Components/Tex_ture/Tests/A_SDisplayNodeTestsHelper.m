...
BOOL A_SDisplayNodeRunRunLoopUntilBlockIsTrue(as_condition_block_t block)
{
...
void A_SDisplayNodeSizeToFitSize(A_SDisplayNode *node, CGSize size)
{
  CGSize sizeThatFits = [node layoutThatFits:A_SSizeRangeMake(size)].size;
  node.bounds = (CGRect){.origin = CGPointZero, .size = sizeThatFits};
}

void A_SDisplayNodeSizeToFitSizeRange(A_SDisplayNode *node, A_SSizeRange sizeRange)
{
