...
@interface A_STextNodeWordKernerTests : XCTestCase

@property (nonatomic, readwrite, strong) A_STextNodeWordKerner *layoutManagerDelegate;
@property (nonatomic, readwrite, strong) A_STextKitComponents *components;
@property (nonatomic, readwrite, copy) NSAttributedString *attributedString;

@end

@implementation A_STextNodeWordKernerTests

- (void)setUp
{
  [super setUp];
  _layoutManagerDelegate = [[A_STextNodeWordKerner alloc] init];
...
- (void)setupTextKitComponentsWithoutWordKerning
{
  ...
  _components = [A_STextKitComponents componentsWithAttributedSeedString:attributedString textContainerSize:size];
}

- (void)setupTextKitComponentsWithWordKerning
{
  CGSize size = CGSizeMake(200, 200);
  NSDictionary *attributes = @{A_STextNodeWordKerningAttributeName: @".5"};
  NSString *seedString = @"Hello world";
  NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:seedString attributes:attributes];
  _components = [A_STextKitComponents componentsWithAttributedSeedString:attributedString textContainerSize:size];
}
...
- (void)testSpaceBoundingBoxForWordKerning
{
  ...
  NSDictionary *attributes = @{A_STextNodeWordKerningAttributeName: @(kernValue),
