...
@interface A_SEditableTextNodeTests : XCTestCase
@property (nonatomic, readwrite, strong) A_SEditableTextNode *editableTextNode;
@property (nonatomic, readwrite, copy) NSAttributedString *attributedText;
@end

@implementation A_SEditableTextNodeTests

- (void)setUp
{
  [super setUp];
  
  _editableTextNode = [[A_SEditableTextNode alloc] init];
...
- (void)testAllocA_SEditableTextNode
{
  A_SEditableTextNode *node = [[A_SEditableTextNode alloc] init];
  XCTAssertTrue([[node class] isSubclassOfClass:[A_SEditableTextNode class]], @"A_SEditableTextNode alloc should return an instance of A_SEditableTextNode, instead returned %@", [node class]);
}

#pragma mark - A_SEditableTextNode Tests

- (void)testUITextInputTraitDefaults
{
  A_SEditableTextNode *editableTextNode = [[A_SEditableTextNode alloc] init];
  
  XCTAssertTrue(editableTextNode.autocapitalizationType == UITextAutocapitalizationTypeSentences, @"_A_STextInputTraitsPendingState's autocapitalizationType default should be UITextAutocapitalizationTypeSentences.");
  XCTAssertTrue(editableTextNode.autocorrectionType == UITextAutocorrectionTypeDefault,           @"_A_STextInputTraitsPendingState's autocorrectionType default should be UITextAutocorrectionTypeDefault.");
  XCTAssertTrue(editableTextNode.spellCheckingType == UITextSpellCheckingTypeDefault,             @"_A_STextInputTraitsPendingState's spellCheckingType default should be UITextSpellCheckingTypeDefault.");
  XCTAssertTrue(editableTextNode.keyboardType == UIKeyboardTypeDefault,                           @"_A_STextInputTraitsPendingState's keyboardType default should be UIKeyboardTypeDefault.");
  XCTAssertTrue(editableTextNode.keyboardAppearance == UIKeyboardAppearanceDefault,               @"_A_STextInputTraitsPendingState's keyboardAppearance default should be UIKeyboardAppearanceDefault.");
  XCTAssertTrue(editableTextNode.returnKeyType == UIReturnKeyDefault,                             @"_A_STextInputTraitsPendingState's returnKeyType default should be UIReturnKeyDefault.");
  XCTAssertTrue(editableTextNode.enablesReturnKeyAutomatically == NO,                             @"_A_STextInputTraitsPendingState's enablesReturnKeyAutomatically default should be NO.");
  XCTAssertTrue(editableTextNode.isSecureTextEntry == NO,                                         @"_A_STextInputTraitsPendingState's isSecureTextEntry default should be NO.");
...
- (void)testUITextInputTraitsSetTraitsBeforeViewLoaded
{
  ...
  A_SEditableTextNode *secureEditableTextNode = [[A_SEditableTextNode alloc] init];
...
- (void)testCalculatedSizeIsGreaterThanOrEqualToConstrainedSize
{
  for (NSInteger i = 10; i < 500; i += 50) {
    ...
    CGSize calculatedSize = [_editableTextNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, constrainedSize)].size;
...
- (void)testRecalculationOfSizeIsSameAsOriginallyCalculatedSize
{
  for (NSInteger i = 10; i < 500; i += 50) {
    CGSize constrainedSize = CGSizeMake(i, i);
    CGSize calculatedSize = [_editableTextNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, constrainedSize)].size;
    CGSize recalculatedSize = [_editableTextNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, constrainedSize)].size;
