...
@interface A_STextNodePerformanceTests : XCTestCase

@end

@implementation A_STextNodePerformanceTests

#pragma mark Performance Tests

static NSString *const kTestCaseUIKit = @"UIKit";
static NSString *const kTestCaseA_SDK = @"A_SDK";
...
- (void)testPerformance_RealisticData
{
  ...
  A_SPerformanceTestContext *ctx = [[A_SPerformanceTestContext alloc] init];
  ...
  uiKitSize.width = A_SCeilPixelValue(uiKitSize.width);
  uiKitSize.height = A_SCeilPixelValue(uiKitSize.height);
  ctx.results[kTestCaseUIKit].userInfo[@"size"] = NSStringFromCGSize(uiKitSize);

  [ctx addCaseWithName:kTestCaseA_SDK block:^(NSUInteger i, dispatch_block_t  _Nonnull startMeasuring, dispatch_block_t  _Nonnull stopMeasuring) {
    A_STextNode *node = [[A_STextNode alloc] init];
    NSAttributedString *text = data[i % data.count];
    startMeasuring();
    node.attributedText = text;
    asdkSize = [node layoutThatFits:A_SSizeRangeMake(CGSizeZero, maxSize)].size;
    stopMeasuring();
  }];
  ctx.results[kTestCaseA_SDK].userInfo[@"size"] = NSStringFromCGSize(asdkSize);

  A_SXCTAssertEqualSizes(uiKitSize, asdkSize);
  A_SXCTAssertRelativePerformanceInRange(ctx, kTestCaseA_SDK, 0.2, 0.5);
}

- (void)testPerformance_TwoParagraphLatinNoTruncation
{
  NSAttributedString *text = [A_STextNodePerformanceTests twoParagraphLatinText];
  ...
  A_SXCTAssertEqualSizes(uiKitSize, asdkSize);
  A_SXCTAssertRelativePerformanceInRange(ctx, kTestCaseA_SDK, 0.5, 0.9);
}

- (void)testPerformance_OneParagraphLatinWithTruncation
{
  NSAttributedString *text = [A_STextNodePerformanceTests oneParagraphLatinText];
  
  CGSize maxSize = CGSizeMake(355, 150);
  CGSize __block uiKitSize, __block asdkSize;
  
  A_SPerformanceTestContext *testCtx = [[A_SPerformanceTestContext alloc] init];
  ...
  [testCtx addCaseWithName:kTestCaseA_SDK block:^(NSUInteger i, dispatch_block_t  _Nonnull startMeasuring, dispatch_block_t  _Nonnull stopMeasuring) {
    A_STextNode *node = [[A_STextNode alloc] init];
    startMeasuring();
    node.attributedText = text;
    asdkSize = [node layoutThatFits:A_SSizeRangeMake(CGSizeZero, maxSize)].size;
  ...
  testCtx.results[kTestCaseA_SDK].userInfo[@"size"] = NSStringFromCGSize(asdkSize);
  
  XCTAssert(CGSizeEqualToSizeWithIn(uiKitSize, asdkSize, 5));
  A_SXCTAssertRelativePerformanceInRange(testCtx, kTestCaseA_SDK, 0.1, 0.3);
}

- (void)testThatNotUsingA_StringDrawingContextHasSimilarPerformanceToHavingOne
{
  A_SPerformanceTestContext *ctx = [[A_SPerformanceTestContext alloc] init];
  ...
  A_SXCTAssertRelativePerformanceInRange(ctx, kTestCaseUIKitWithReusedContext, 0.8, 1.2);
  A_SXCTAssertRelativePerformanceInRange(ctx, kTestCaseUIKitWithFreshContext, 0.8, 1.2);
}
...
- (void)testThatUIKitPrivateLayoutCachingIsAwesome
{
  ...
  XCTAssertTrue([ctx areAllUserInfosEqual]);
  A_SXCTAssertRelativePerformanceInRange(ctx, kTestCaseUIKitPrivateCaching, 1.2, FLT_MAX);
}
