...
@interface A_STextNodeTestDelegate : NSObject <A_STextNodeDelegate>
...
@implementation A_STextNodeTestDelegate
...
@interface A_STextNodeTests : XCTestCase

@property (nonatomic, readwrite, strong) A_STextNode *textNode;
@property (nonatomic, readwrite, copy) NSAttributedString *attributedText;

@end

@implementation A_STextNodeTests
...
- (void)testAllocA_STextNode
{
  A_STextNode *node = [[A_STextNode alloc] init];
  XCTAssertTrue([[node class] isSubclassOfClass:[A_STextNode class]], @"A_STextNode alloc should return an instance of A_STextNode, instead returned %@", [node class]);
}
...
- (void)testCalculatedSizeIsGreaterThanOrEqualToConstrainedSize
{
  for (NSInteger i = 10; i < 500; i += 50) {
    ...
    CGSize calculatedSize = [_textNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, constrainedSize)].size;
...
- (void)testRecalculationOfSizeIsSameAsOriginallyCalculatedSize
{
  for (NSInteger i = 10; i < 500; i += 50) {
    CGSize constrainedSize = CGSizeMake(i, i);
    CGSize calculatedSize = [_textNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, constrainedSize)].size;
    CGSize recalculatedSize = [_textNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, constrainedSize)].size;
...
- (void)testMeasureWithZeroSizeAndPlaceholder
{
  ...
  XCTAssertNoThrow([_textNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, CGSizeZero)], @"Measure with zero size and placeholder enabled should not throw an exception");
  XCTAssertNoThrow([_textNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, CGSizeMake(0, 100))], @"Measure with zero width and placeholder enabled should not throw an exception");
  XCTAssertNoThrow([_textNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, CGSizeMake(100, 0))], @"Measure with zero height and placeholder enabled should not throw an exception");
}
...
- (void)testLinkAttribute
{
  ...
  A_STextNodeTestDelegate *delegate = [A_STextNodeTestDelegate new];
  _textNode.delegate = delegate;

  A_SLayout *layout = [_textNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, CGSizeMake(100, 100))];
...
- (void)testTapNotOnALinkAttribute
{
  ...
  CGSize calculatedSize = [_textNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, CGSizeMake(100, 100))].size;
...
- (void)testAddingExclusionPathsShouldInvalidateAndIncreaseTheSize
{
  ...
  CGSize sizeWithoutExclusionPaths = [_textNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, constrainedSize)].size;
  _textNode.exclusionPaths = @[[UIBezierPath bezierPathWithRect:CGRectMake(50, 20, 30, 40)]];
  CGSize sizeWithExclusionPaths = [_textNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, constrainedSize)].size;
