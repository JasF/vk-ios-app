...
@interface A_SVideoNodeTests : XCTestCase <A_SVideoNodeDelegate>
{
...
@property (nonatomic, readwrite) A_SInterfaceState interfaceState;
@property (nonatomic, readonly) A_SDisplayNode *spinner;
@property (nonatomic, readwrite) A_SDisplayNode *playerNode;
...
@implementation A_SVideoNodeTests
...
{
  _videoNode.interfaceState = A_SInterfaceStatePreload;
  [_videoNode play];
}
...
{
  [_videoNode setInterfaceState:A_SInterfaceStateNone];
  [_videoNode didLoad];
...
{
  [_videoNode pause];
  [_videoNode setInterfaceState:A_SInterfaceStateVisible | A_SInterfaceStateDisplay];
  [_videoNode didLoad];
...
- (void)doVideoStartsPlayingOnDidDidBecomeVisibleWhenShouldAutoplay
{
  ...
  _videoNode.playerNode = [[A_SDisplayNode alloc] initWithLayerBlock:^CALayer *{
...
- (void)doVideoShouldPauseWhenItLeavesVisibleButShouldKnowPlayingShouldRestartLater
{
  ...
  [_videoNode interfaceStateDidChange:A_SInterfaceStateNone fromState:A_SInterfaceStateVisible];
...
- (void)doVideoThatIsPlayingWhenItLeavesVisibleRangeStartsAgainWhenItComesBack
{
  ...
  [_videoNode interfaceStateDidChange:A_SInterfaceStateVisible fromState:A_SInterfaceStateNone];
  [_videoNode interfaceStateDidChange:A_SInterfaceStateNone fromState:A_SInterfaceStateVisible];
...
- (void)testVideoThatDoesNotAutorepeatsShouldPauseOnPlaybackEnd
{
  ...
  [_videoNode didLoad];
  [_videoNode setInterfaceState:A_SInterfaceStateVisible | A_SInterfaceStateDisplay | A_SInterfaceStatePreload];
...
- (void)testDelegateProperlySetForClassHierarchy
{
  ...
  XCTAssertTrue([_videoNode.delegate conformsToProtocol:@protocol(A_SVideoNodeDelegate)]);
  XCTAssertTrue([_videoNode.delegate conformsToProtocol:@protocol(A_SNetworkImageNodeDelegate)]);
  XCTAssertTrue([((A_SNetworkImageNode*)_videoNode).delegate conformsToProtocol:@protocol(A_SNetworkImageNodeDelegate)]);
  
  XCTAssertEqual(_videoNode.delegate, self);
  XCTAssertEqual(((A_SNetworkImageNode*)_videoNode).delegate, self);
}
