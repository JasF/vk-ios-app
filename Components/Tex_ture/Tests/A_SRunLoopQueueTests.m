...
@interface A_SRunLoopQueueTests : XCTestCase

@end

@implementation A_SRunLoopQueueTests

#pragma mark enqueue tests

- (void)testEnqueueNilObjectsToQueue
{
  A_SRunLoopQueue *queue = [[A_SRunLoopQueue alloc] initWithRunLoop:CFRunLoopGetMain() retainObjects:YES handler:nil];
  id object = nil;
...
- (void)testEnqueueSameObjectTwiceToDefaultQueue
{
  ...
  A_SRunLoopQueue *queue = [[A_SRunLoopQueue alloc] initWithRunLoop:CFRunLoopGetMain() retainObjects:YES handler:^(id  _Nonnull dequeuedItem, BOOL isQueueDrained) {
...
- (void)testWeakQueueDoesNotRetainsObjects
{
  ...
  __unsafe_unretained id weakObject = object;
  __block BOOL didProcessObject = NO;
  A_SRunLoopQueue *queue = [[A_SRunLoopQueue alloc] initWithRunLoop:CFRunLoopGetMain() retainObjects:NO handler:^(id  _Nonnull dequeuedItem, BOOL isQueueDrained) {
    if (dequeuedItem == weakObject) {
      didProcessObject = YES;
    }
  }];
...
{
  A_SRunLoopQueue *queue = [[A_SRunLoopQueue alloc] initWithRunLoop:CFRunLoopGetMain() retainObjects:NO handler:nil];
  id object = [[NSObject alloc] init];
  [queue enqueue:object];
  object = nil;
  XCTAssertFalse(queue.isEmpty);
