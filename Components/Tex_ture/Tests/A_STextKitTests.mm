...
@interface A_STextKitTests : XCTestCase

@end

static UITextView *UITextViewWithAttributes(const A_STextKitAttributes &attributes,
...
static UIImage *UITextViewImageWithAttributes(const A_STextKitAttributes &attributes,
                                              const CGSize constrainedSize,
                                              NSDictionary *linkTextAttributes)
{
...
static UIImage *A_STextKitImageWithAttributes(const A_STextKitAttributes &attributes, const CGSize constrainedSize)
{
  A_STextKitRenderer *renderer = [[A_STextKitRenderer alloc] initWithTextKitAttributes:attributes
...
static BOOL checkAttributes(const A_STextKitAttributes &attributes, const CGSize constrainedSize, NSDictionary *linkTextAttributes)
{
  ...
  UIImage *textKitImage = A_STextKitImageWithAttributes(attributes, constrainedSize);
  return [controller compareReferenceImage:labelImage toImage:textKitImage tolerance:0.0 error:nil];
}

@implementation A_STextKitTests

- (void)testSimpleStrings
{
  A_STextKitAttributes attributes {
...
- (void)testChangingAPropertyChangesHash
{
  ...
  A_STextKitAttributes attrib1 {
    .attributedString = as,
    .lineBreakMode =  NSLineBreakByClipping,
  };
  A_STextKitAttributes attrib2 {
    .attributedString = as,
  };
...
- (void)testLinkInTextUsesForegroundColor
{
  ...
  NSURL *linkURL = [NSURL URLWithString:@"https://github.com/facebook/Async_DisplayKit/issues/967"];
  ...
  A_STextKitAttributes textKitattributes {
    .attributedString = attrStr
  };
...
- (void)testRectsForRangeBeyondTruncationSizeReturnsNonZeroNumberOfRects
{
  ...
  [[NSAttributedString alloc]
   initWithString:@"90's cray photo booth tote bag bespoke Carles. Plaid wayfarers Odd Future master cleanse tattooed four dollar toast small batch kale chips leggings meh photo booth occupy irony.  " attributes:@{A_STextKitEntityAttributeName : [[A_STextKitEntityAttribute alloc] initWithEntity:@"entity"]}];
  A_STextKitRenderer *renderer =
  [[A_STextKitRenderer alloc]
  ...
  XCTAssert([renderer rectsForTextRange:NSMakeRange(0, attributedString.length) measureOption:A_STextKitRendererMeasureOptionBlock].count > 0);
}
...
- (void)testTextKitComponentsCanCalculateSizeInBackground
{
  ...
  [[NSAttributedString alloc]
   initWithString:@"90's cray photo booth tote bag bespoke Carles. Plaid wayfarers Odd Future master cleanse tattooed four dollar toast small batch kale chips leggings meh photo booth occupy irony.  " attributes:@{A_STextKitEntityAttributeName : [[A_STextKitEntityAttribute alloc] initWithEntity:@"entity"]}];
  A_STextKitComponents *components = [A_STextKitComponents componentsWithAttributedSeedString:attributedString textContainerSize:CGSizeZero];
  components.textView = [[A_STextKitComponentsTextView alloc] initWithFrame:CGRectZero textContainer:components.textContainer];
