...
@interface A_STestDataController : A_SDataController
@property (nonatomic) int numberOfAllNodesRelayouts;
@end

@implementation A_STestDataController
...
@interface A_STestTableView : A_STableView
@property (nonatomic, copy) void (^willDeallocBlock)(A_STableView *tableView);
@end

@implementation A_STestTableView

- (instancetype)__initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
  
  return [super _initWithFrame:frame style:style dataControllerClass:[A_STestDataController class] owningNode:nil eventLog:nil];
}

- (A_STestDataController *)testDataController
{
  return (A_STestDataController *)self.dataController;
}
...
@interface A_STableViewTestDelegate : NSObject <A_STableDataSource, A_STableDelegate>
@property (nonatomic, copy) void (^willDeallocBlock)(A_STableViewTestDelegate *delegate);
@property (nonatomic) CGFloat headerHeight;
@property (nonatomic) CGFloat footerHeight;
@end

@implementation A_STableViewTestDelegate
...
@interface A_STestTextCellNode : A_STextCellNode
...
@implementation A_STestTextCellNode
...
@interface A_STableViewFilledDataSource : NSObject <A_STableDataSource, A_STableDelegate>
@property (nonatomic) BOOL usesSectionIndex;
@property (nonatomic) NSInteger rowsPerSection;
@property (nonatomic, nullable, copy) A_SCellNodeBlock(^nodeBlockForItem)(NSIndexPath *);
@end

@implementation A_STableViewFilledDataSource
...
- (A_SCellNode *)tableView:(A_STableView *)tableView nodeForRowAtIndexPath:(NSIndexPath *)indexPath
{
  A_STestTextCellNode *textCellNode = [A_STestTextCellNode new];
...
@interface A_STableViewFilledDelegate : NSObject <A_STableDelegate>
@end

@implementation A_STableViewFilledDelegate

- (A_SSizeRange)tableView:(A_STableView *)tableView constrainedSizeForRowAtIndexPath:(NSIndexPath *)indexPath
{
  return A_SSizeRangeMake(CGSizeMake(10, 42));
}

@end

@interface A_STableViewTests : XCTestCase
@property (nonatomic, retain) A_STableView *testTableView;
@end

@implementation A_STableViewTests

- (void)testDataSourceImplementsNecessaryMethods
{
  A_STestTableView *tableView = [[A_STestTableView alloc] __initWithFrame:CGRectMake(0, 0, 100, 400)
                                                                  style:UITableViewStylePlain];
  
  
  
  A_STableViewFilledDataSource *dataSource = (A_STableViewFilledDataSource *)[NSObject new];
  XCTAssertThrows((tableView.asyncDataSource = dataSource));
  
  dataSource = [A_STableViewFilledDataSource new];
...
- (void)testConstrainedSizeForRowAtIndexPath
{
  ...
  A_STableViewFilledDelegate *delegate = [A_STableViewFilledDelegate new];
  A_STableViewFilledDataSource *dataSource = [A_STableViewFilledDataSource new];
  ...
  CGFloat separatorHeight = 1.0 / A_SScreenScale();
...
{
  A_STestTableView *tableView = [[A_STestTableView alloc] __initWithFrame:CGRectZero style:UITableViewStylePlain];
  
  __block BOOL tableViewDidDealloc = NO;
  tableView.willDeallocBlock = ^(A_STableView *v){
    tableViewDidDealloc = YES;
  };
  
  A_STableViewTestDelegate *delegate = [[A_STableViewTestDelegate alloc] init];
  
  __block BOOL delegateDidDealloc = NO;
  delegate.willDeallocBlock = ^(A_STableViewTestDelegate *d){
...
- (void)DISABLED_testReloadData
{
  ...
  A_STableView *tableView = [[A_STestTableView alloc] __initWithFrame:CGRectMake(0, 0, 100, 500)
                                                              style:UITableViewStylePlain];
  ...
  for (int i = 0; i < NumberOfReloadIterations; ++i) {
    ...
    //NSLog(@"Iteration %03d: %@|%@|%@|%@|%g", i, (rowAnimation == UITableViewRowAnimationNone) ? @"NONE  " : @"MIDDLE", animatedScroll ? @"A_SCR" : @"    ", reloadRowsInsteadOfSections ? @"ROWS" : @"SECS", useBeginEndUpdates ? @"BEGEND" : @"      ", runLoopDelay);
...
- (void)testRelayoutAllNodesWithNonZeroSizeInitially
{
  ...
  // Width and height are swapped so that a later size change will simulate a rotation
  A_STestTableView *tableView = [[A_STestTableView alloc] __initWithFrame:CGRectMake(0, 0, tableViewFinalSize.height, tableViewFinalSize.width)
                                                                  style:UITableViewStylePlain];
...
- (void)testRelayoutVisibleRowsWhenEditingModeIsChanged
{
  ...
  A_STestTableView *tableView = [[A_STestTableView alloc] __initWithFrame:CGRectMake(0, 0, tableViewSize.width, tableViewSize.height)
                                                                style:UITableViewStylePlain];
  A_STableViewFilledDataSource *dataSource = [A_STableViewFilledDataSource new];
  ...
  dataSource.nodeBlockForItem = ^(NSIndexPath *indexPath) {
    return (A_SCellNodeBlock)^{
      A_STestTextCellNode *textCellNode = [[A_STestTextCellNode alloc] init];
      textCellNode.text = indexPath.description;
      return textCellNode;
    };
  };
  ...
  [tableView endUpdatesAnimated:YES completion:^(BOOL completed) {
    for (int section = 0; section < NumberOfSections; section++) {
      for (int row = 0; row < dataSource.rowsPerSection; row++) {
        NSIndexPath *indexPath = [NSIndexPath indexPathForRow:row inSection:section];
        A_STestTextCellNode *node = (A_STestTextCellNode *)[tableView nodeForRowAtIndexPath:indexPath];
...
- (void)DISABLED_testRelayoutRowsAfterEditingModeIsChangedAndTheyBecomeVisible
{
  ...
  [tableView endUpdatesAnimated:YES completion:^(BOOL completed) {
    A_STestTextCellNode *node = (A_STestTextCellNode *)[tableView nodeForRowAtIndexPath:lastRowIndexPath];
    XCTAssertEqual(node.numberOfLayoutsOnMainThread, 1);
    XCTAssertLessThan(node.constrainedSizeForCalculatedLayout.max.width, tableViewSize.width);
...
{
  CGSize tableViewSize = CGSizeMake(100, 500);
  A_STestTableView *tableView = [[A_STestTableView alloc] initWithFrame:CGRectMake(0, 0, tableViewSize.width, tableViewSize.height)
                                                                style:UITableViewStylePlain];
  A_STableViewFilledDataSource *dataSource = [A_STableViewFilledDataSource new];
  ...
  [tableView reloadDataWithCompletion:^{
    for (NSUInteger i = 0; i < NumberOfSections; i++) {
      for (NSUInteger j = 0; j < dataSource.rowsPerSection; j++) {
        ...
        A_SCellNode *cellNode = [tableView nodeForRowAtIndexPath:indexPath];
...
- (void)triggerFirstLayoutMeasurementForTableView:(A_STableView *)tableView{
  XCTestExpectation *reloadDataExpectation = [self expectationWithDescription:@"reloadData"];
  [tableView reloadDataWithCompletion:^{
    for (int section = 0; section < NumberOfSections; section++) {
      for (int row = 0; row < [tableView numberOfRowsInSection:section]; row++) {
...
- (void)triggerSizeChangeAndAssertRelayoutAllNodesForTableView:(A_STestTableView *)tableView newSize:(CGSize)newSize
{
...
{
  A_STableNode *node = [[A_STableNode alloc] initWithStyle:UITableViewStylePlain];
...
- (void)testThatInitialDataLoadHappensInOneShot
{
  ...
  // The first reloadData call helps prevent UITableView from calling it multiple times while A_SDataController is working.
...
- (void)testThatNodeConstrainedSizesAreCorrectIfReloadIsPreempted
{
  ...
  A_SSizeRange expectedSizeRange = A_SSizeRangeMake(CGSizeMake(cellWidth, 0));
  ...
  for (NSInteger i = 0; i < node.numberOfSections; i++) {
    for (NSInteger j = 0; j < [node numberOfRowsInSection:i]; j++) {
      ...
      A_STestTextCellNode *cellNode = (id)[node nodeForRowAtIndexPath:indexPath];
      A_SXCTAssertEqualSizeRanges(cellNode.constrainedSizeForCalculatedLayout, expectedSizeRange);
...
// https://github.com/facebook/Async_DisplayKit/issues/2252#issuecomment-263689979
- (void)testIssue2252
{
  ...
  A_STableNode *node = [[A_STableNode alloc] initWithStyle:UITableViewStyleGrouped];
  node.frame = window.bounds;
  A_STableViewTestDelegate *del = [[A_STableViewTestDelegate alloc] init];
  del.headerHeight = 32;
  del.footerHeight = 0.01;
  node.delegate = del;
  A_STableViewFilledDataSource *ds = [[A_STableViewFilledDataSource alloc] init];
  ds.rowsPerSection = 1;
  node.dataSource = ds;
  ...
  A_SXCTAssertEqualRects(CGRectMake(0, 32, 375, 44), [node rectForRowAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:0]], @"This text requires very specific geometry. The rect for the first row should match up.");
...
- (void)testThatInvalidUpdateExceptionReasonContainsDataSourceClassName
{
  ...
  @try {
    ...
    XCTAssertEqual(e.name, A_SCollectionInvalidUpdateException);
...
- (void)testAutomaticallyAdjustingContentOffset
{
  ...
  [node performBatchAnimated:NO updates:^{
    ...
    // This is silly but it's a consequence of how A_STableViewFilledDataSource is built.
    ds.rowsPerSection -= 1;
