...
@interface A_STextCellNodeWithSetSelectedCounter : A_STextCellNode
...
@implementation A_STextCellNodeWithSetSelectedCounter
...
@interface A_STestSectionContext : NSObject <A_SSectionContext>
...
@implementation A_STestSectionContext

@synthesize sectionName = _sectionName, collectionView = _collectionView;

@end

@interface A_SCollectionViewTestDelegate : NSObject <A_SCollectionDataSource, A_SCollectionDelegate, UICollectionViewDelegateFlowLayout>

@property (nonatomic, assign) NSInteger sectionGeneration;
@property (nonatomic, copy) void(^willBeginBatchFetch)(A_SBatchContext *);

@end

@implementation A_SCollectionViewTestDelegate {
  @package
...
- (A_SCellNode *)collectionView:(A_SCollectionView *)collectionView nodeForItemAtIndexPath:(NSIndexPath *)indexPath {
  A_STextCellNodeWithSetSelectedCounter *textCellNode = [A_STextCellNodeWithSetSelectedCounter new];
...
- (id<A_SSectionContext>)collectionNode:(A_SCollectionNode *)collectionNode contextForSection:(NSInteger)section
{
  A_STestSectionContext *context = [[A_STestSectionContext alloc] init];
...
- (A_SCellNode *)collectionView:(A_SCollectionView *)collectionView nodeForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath
{
  return [[A_STextCellNodeWithSetSelectedCounter alloc] init];
}
...
@interface A_SCollectionViewTestController: UIViewController

@property (nonatomic, strong) A_SCollectionViewTestDelegate *asyncDelegate;
@property (nonatomic, strong) A_SCollectionView *collectionView;
@property (nonatomic, strong) A_SCollectionNode *collectionNode;

@end

@implementation A_SCollectionViewTestController

- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
  self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil];
  if (self) {
    // Populate these immediately so that they're not unexpectedly nil during tests.
    self.asyncDelegate = [[A_SCollectionViewTestDelegate alloc] initWithNumberOfSections:10 numberOfItemsInSection:10];
    id realLayout = [UICollectionViewFlowLayout new];
    id mockLayout = [OCMockObject partialMockForObject:realLayout];
    self.collectionNode = [[A_SCollectionNode alloc] initWithFrame:self.view.bounds collectionViewLayout:mockLayout];
...
@interface A_SCollectionView (InternalTesting)

- (NSArray<NSString *> *)dataController:(A_SDataController *)dataController supplementaryNodeKindsInSections:(NSIndexSet *)sections;

@end

@interface A_SCollectionViewTests : XCTestCase

@end

@implementation A_SCollectionViewTests
...
- (void)testDataSourceImplementsNecessaryMethods
{
  ...
  dataSource = [OCMockObject niceMockForProtocol:@protocol(A_SCollectionDataSource)];
...
- (void)testThatItSetsALayoutInspectorForFlowLayouts
{
  ...
  XCTAssert([collectionView.layoutInspector isKindOfClass:[A_SCollectionViewFlowLayoutInspector class]], @"should have a flow layout inspector by default");
}
...
- (void)testThatADefaultLayoutInspectorIsProvidedForCustomLayouts
{
  ...
  A_SCollectionView *collectionView = [[A_SCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
  XCTAssert(collectionView.layoutInspector != nil, @"should automatically set a layout delegate for flow layouts");
  XCTAssert([collectionView.layoutInspector isKindOfClass:[A_SCollectionViewLayoutInspector class]], @"should have a default layout inspector by default");
}

- (void)testThatRegisteringA_SupplementaryNodeStoresItForIntrospection
{
  UICollectionViewFlowLayout *layout = [[UICollectionViewFlowLayout alloc] init];
  A_SCollectionView *collectionView = [[A_SCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
...
{
  __block A_SCollectionViewTestController *testController = [[A_SCollectionViewTestController alloc] initWithNibName:nil bundle:nil];
  __block A_SCollectionViewTestDelegate *del = testController.asyncDelegate;
  __block A_SCollectionNode *cn = testController.collectionNode;

  void (^reset)() = ^void() {
    testController = [[A_SCollectionViewTestController alloc] initWithNibName:nil bundle:nil];
  ...
  A_STextCellNodeWithSetSelectedCounter *node = (A_STextCellNodeWithSetSelectedCounter*)[cn nodeForItemAtIndexPath:indexPath];
...
- (void)testSelection
{
  A_SCollectionViewTestController *testController = [[A_SCollectionViewTestController alloc] initWithNibName:nil bundle:nil];
  ...
  NSIndexPath *indexPath = [NSIndexPath indexPathForItem:0 inSection:0];
  A_SCellNode *node = [testController.collectionView nodeForItemAtIndexPath:indexPath];
  ...
  A_SCellNode *node2 = [testController.collectionView nodeForItemAtIndexPath:indexPath2];
  ...
  XCTAssertTrue([(A_STextCellNodeWithSetSelectedCounter *)node setSelectedCounter] == (setSelectedCount + 1), @"setSelected: should not be called on node multiple times.");
}
...
- (void)testTuningParametersWithExplicitRangeMode
{
  ...
  A_SRangeTuningParameters minimumRenderParams = { .leadingBufferScreenfuls = 0.1, .trailingBufferScreenfuls = 0.1 };
  A_SRangeTuningParameters minimumPreloadParams = { .leadingBufferScreenfuls = 0.1, .trailingBufferScreenfuls = 0.1 };
  A_SRangeTuningParameters fullRenderParams = { .leadingBufferScreenfuls = 0.5, .trailingBufferScreenfuls = 0.5 };
  A_SRangeTuningParameters fullPreloadParams = { .leadingBufferScreenfuls = 1, .trailingBufferScreenfuls = 0.5 };
  
  [collectionNode setTuningParameters:minimumRenderParams forRangeMode:A_SLayoutRangeModeMinimum rangeType:A_SLayoutRangeTypeDisplay];
  [collectionNode setTuningParameters:minimumPreloadParams forRangeMode:A_SLayoutRangeModeMinimum rangeType:A_SLayoutRangeTypePreload];
  [collectionNode setTuningParameters:fullRenderParams forRangeMode:A_SLayoutRangeModeFull rangeType:A_SLayoutRangeTypeDisplay];
  [collectionNode setTuningParameters:fullPreloadParams forRangeMode:A_SLayoutRangeModeFull rangeType:A_SLayoutRangeTypePreload];
  
  XCTAssertTrue(A_SRangeTuningParametersEqualToRangeTuningParameters(minimumRenderParams,
                                                                    [collectionNode tuningParametersForRangeMode:A_SLayoutRangeModeMinimum rangeType:A_SLayoutRangeTypeDisplay]));
  XCTAssertTrue(A_SRangeTuningParametersEqualToRangeTuningParameters(minimumPreloadParams,
                                                                    [collectionNode tuningParametersForRangeMode:A_SLayoutRangeModeMinimum rangeType:A_SLayoutRangeTypePreload]));
  XCTAssertTrue(A_SRangeTuningParametersEqualToRangeTuningParameters(fullRenderParams,
                                                                    [collectionNode tuningParametersForRangeMode:A_SLayoutRangeModeFull rangeType:A_SLayoutRangeTypeDisplay]));
  XCTAssertTrue(A_SRangeTuningParametersEqualToRangeTuningParameters(fullPreloadParams,
                                                                    [collectionNode tuningParametersForRangeMode:A_SLayoutRangeModeFull rangeType:A_SLayoutRangeTypePreload]));
}
...
- (void)testTuningParameters
{
  ...
  A_SRangeTuningParameters renderParams = { .leadingBufferScreenfuls = 1.2, .trailingBufferScreenfuls = 3.2 };
  A_SRangeTuningParameters preloadParams = { .leadingBufferScreenfuls = 4.3, .trailingBufferScreenfuls = 2.3 };
  
  [collectionView setTuningParameters:renderParams forRangeType:A_SLayoutRangeTypeDisplay];
  [collectionView setTuningParameters:preloadParams forRangeType:A_SLayoutRangeTypePreload];
  
  XCTAssertTrue(A_SRangeTuningParametersEqualToRangeTuningParameters(renderParams, [collectionView tuningParametersForRangeType:A_SLayoutRangeTypeDisplay]));
  XCTAssertTrue(A_SRangeTuningParametersEqualToRangeTuningParameters(preloadParams, [collectionView tuningParametersForRangeType:A_SLayoutRangeTypePreload]));
}
...
{
  A_SCollectionNode *node = [[A_SCollectionNode alloc] initWithFrame:CGRectZero collectionViewLayout:[[UICollectionViewFlowLayout alloc] init]];
  XCTAssert([node conformsToProtocol:@protocol(A_SRangeControllerUpdateRangeProtocol)]);
}
...
#define updateValidationTestPrologue \
  A_SCollectionViewTestController *testController = [[A_SCollectionViewTestController alloc] initWithNibName:nil bundle:nil];\
  __unused A_SCollectionViewTestDelegate *del = testController.asyncDelegate;\
  __unused A_SCollectionView *cv = testController.collectionView;\
  A_SCollectionNode *cn = testController.collectionNode;\
  UIWindow *window = [[UIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];\
  [window makeKeyAndVisible]; \
...
- (void)testThatDeletingAndReloadingA_SectionThrowsAnException
{
  updateValidationTestPrologue
  NSInteger sectionCount = del->_itemCounts.size();
...
- (void)testCellNodeLayoutAttributes
{
  ...
  for (A_STextCellNodeWithSetSelectedCounter *node in nodeBatch1) {
  ...
  for (UICollectionViewLayoutAttributes *layoutAttributes in visibleLayoutAttributesBatch1) {
    ...
    A_STextCellNodeWithSetSelectedCounter *node = (A_STextCellNodeWithSetSelectedCounter *)[cv supplementaryNodeForElementKind:layoutAttributes.representedElementKind atIndexPath:layoutAttributes.indexPath];
...
- (void)testCellNodeIndexPathConsistency
{
  ...
  A_SCellNode *cell = [cn nodeForItemAtIndexPath:indexPath];
...
/**
 * https://github.com/facebook/Async_DisplayKit/issues/2011
 *
...
- (void)testThatDisappearingSupplementariesWithLayerBackedNodesDontFailAssert
{
  ...
  A_SCollectionNode *cn = [[A_SCollectionNode alloc] initWithFrame:window.bounds collectionViewLayout:layout];
  A_SCollectionView *cv = cn.view;


  __unused NSMutableSet *keepaliveNodes = [NSMutableSet set];
  id dataSource = [OCMockObject niceMockForProtocol:@protocol(A_SCollectionDataSource)];
  static int nodeIdx = 0;
  [[[dataSource stub] andDo:^(NSInvocation *invocation) {
    __autoreleasing A_SCellNode *suppNode = [[A_SCellNode alloc] init];
    int thisNodeIdx = nodeIdx++;
    suppNode.debugName = [NSString stringWithFormat:@"Cell #%d", thisNodeIdx];
    [keepaliveNodes addObject:suppNode];

    A_SDisplayNode *layerBacked = [[A_SDisplayNode alloc] init];
...
/**
 * See corresponding test in A_SUICollectionViewTests
 *
...
- (void)testThatNestedBatchCompletionsAreCalledInOrder
{
  ...
  A_SCollectionView *cv = testController.collectionView;
...
#pragma mark - A_SSectionContext tests
...
- (void)testThatSectionContextsAreCorrectAfterTheInitialLayout
{
  ...
  for (NSInteger section = 0; section < sectionCount; section++) {
    A_STestSectionContext *context = (A_STestSectionContext *)[cn contextForSection:section];
...
- (void)testThatSectionContextsAreCorrectAfterSectionMove
{
  ...
  A_STestSectionContext *movedSectionContext = (A_STestSectionContext *)[cn contextForSection:toSection];
  XCTAssertNotNil(movedSectionContext);
  // A_SCollectionView currently splits a move operation to a pair of delete and insert ones.
...
- (void)testThatSectionContextsAreCorrectAfterReloadA_Section
{
  updateValidationTestPrologue
...
/// See the same test in A_SUICollectionViewTests for the reference behavior.
- (void)testThatIssuingAnUpdateBeforeInitialReloadIsAcceptable
{
  A_SCollectionViewTestDelegate *del = [[A_SCollectionViewTestDelegate alloc] initWithNumberOfSections:0 numberOfItemsInSection:0];
  A_SCollectionView *cv = [[A_SCollectionView alloc] initWithCollectionViewLayout:[UICollectionViewFlowLayout new]];
  ...
  // A_SUICollectionViewTests.testThatIssuingAnUpdateBeforeInitialReloadIsUnacceptable
...
- (void)testThatNodeAtIndexPathIsCorrectImmediatelyAfterSubmittingUpdate
{
  ...
  A_SCellNode *oldNode = [cn nodeForItemAtIndexPath:indexPath];
  XCTAssertNotNil(oldNode);
  del->_itemCounts[0] += 1;
  [cv insertItemsAtIndexPaths:@[ indexPath ]];
  A_SCellNode *newNode = [cn nodeForItemAtIndexPath:indexPath];
...
- (void)DISABLED_testThatSupplementaryNodeAtIndexPathIsCorrectImmediatelyAfterSubmittingUpdate
{
  updateValidationTestPrologue
  NSIndexPath *indexPath = [NSIndexPath indexPathForItem:0 inSection:0];
  A_SCellNode *oldHeader = [cv supplementaryNodeForElementKind:UICollectionElementKindSectionHeader atIndexPath:indexPath];
  ...
  A_SCellNode *newHeader = [cv supplementaryNodeForElementKind:UICollectionElementKindSectionHeader atIndexPath:indexPath];
...
- (void)testThatDeletedItemsAreMarkedInvisible
{
  ...
  A_SCellNode *node = [cn nodeForItemAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:0]];
...
- (void)disabled_testThatMultipleBatchFetchesDontHappenUnnecessarily
{
  ...
  __weak A_SCollectionViewTestController *weakController = testController;
  testController.asyncDelegate.willBeginBatchFetch = ^(A_SBatchContext *context) {
...
{
  A_SCollectionViewTestController *ctrl = [[A_SCollectionViewTestController alloc] initWithNibName:nil bundle:nil];
...
- (void)_primitiveBatchFetchingFillTestAnimated:(BOOL)animated visible:(BOOL)visible controller:(nullable A_SCollectionViewTestController *)testController
{
  ...
  // Wait for A_SDK reload to finish
  [cn waitUntilAllUpdatesAreProcessed];
...
- (void)testInitialRangeBounds
{
  ...
  A_SCollectionViewTestController *testController = [[A_SCollectionViewTestController alloc] initWithNibName:nil bundle:nil];
  A_SCollectionNode *cn = testController.collectionNode;
  [cn setTuningParameters:{ .leadingBufferScreenfuls = 2, .trailingBufferScreenfuls = 0 } forRangeMode:A_SLayoutRangeModeMinimum rangeType:A_SLayoutRangeTypePreload];
  window.rootViewController = testController;
  ...
  // Test the APIs that monitor A_SCollectionNode update handling
  XCTAssertTrue(cn.isProcessingUpdates, @"A_SCollectionNode should still be processing updates after initial layoutIfNeeded call (reloadData)");
  [cn onDidFinishProcessingUpdates:^{
    XCTAssertTrue(!cn.isProcessingUpdates, @"A_SCollectionNode should no longer be processing updates inside -onDidFinishProcessingUpdates: block");
  }];

  // Wait for A_SDK reload to finish
  [cn waitUntilAllUpdatesAreProcessed];

  XCTAssertTrue(!cn.isProcessingUpdates, @"A_SCollectionNode should no longer be processing updates after -wait call");
  ...
  CGRect preloadBounds = ({
    ...
    for (NSInteger s = 0; s < cn.numberOfSections; s++) {
      ...
      for (NSInteger i = 0; i < c; i++) {
        ...
        A_SCellNode *node = [cn nodeForItemAtIndexPath:ip];
  ...
  XCTAssertEqual([[cn valueForKeyPath:@"rangeController.currentRangeMode"] integerValue], A_SLayoutRangeModeMinimum, @"Expected range mode to be minimum before scrolling begins.");
}
...
- (void)testTraitCollectionChangesMidUpdate
{
  ...
  A_SPrimitiveTraitCollection traitCollection;
  ...
  for (NSInteger s = 0; s < cn.numberOfSections; s++) {
    ...
    for (NSInteger i = 0; i < c; i++) {
      NSIndexPath *ip = [NSIndexPath indexPathForItem:i inSection:s];
      A_SCellNode *node = [cn.view nodeForItemAtIndexPath:ip];
      XCTAssertTrue(A_SPrimitiveTraitCollectionIsEqualToA_SPrimitiveTraitCollection(traitCollection, node.primitiveTraitCollection));
    }
  }
}
...
- (void)DISABLED_testThatAutomaticallyManagedSubnodesGetPreloadCallBeforeDisplay
{
  ...
  for (NSInteger i = 0; i < itemCount; i++) {
    A_STextCellNodeWithSetSelectedCounter *node = [cn nodeForItemAtIndexPath:[NSIndexPath indexPathForItem:i inSection:0]];
    ...
    A_SDisplayNode *subnode = node.textNode;
    XCTAssertEqualObjects(NSStringFromA_SInterfaceState(subnode.interfaceState), NSStringFromA_SInterfaceState(node.interfaceState), @"Subtree interface state should match cell node interface state for A_SM nodes.");
