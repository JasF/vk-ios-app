...
@interface A_SMultiplexImageNodeTests : XCTestCase
{
@private
  id mockCache;
  id mockDownloader;
  id mockDataSource;
  id mockDelegate;
  A_SMultiplexImageNode *imageNode;
}

@end

@implementation A_SMultiplexImageNodeTests
...
- (void)setUp
{
  ...
  mockCache = OCMStrictProtocolMock(@protocol(A_SImageCacheProtocol));
  [mockCache setExpectationOrderMatters:YES];
  mockDownloader = OCMStrictProtocolMock(@protocol(A_SImageDownloaderProtocol));
  [mockDownloader setExpectationOrderMatters:YES];
  imageNode = [[A_SMultiplexImageNode alloc] initWithCache:mockCache downloader:mockDownloader];

  mockDataSource = OCMStrictProtocolMock(@protocol(A_SMultiplexImageNodeDataSource));
  [mockDataSource setExpectationOrderMatters:YES];
  imageNode.dataSource = mockDataSource;

  mockDelegate = OCMProtocolMock(@protocol(A_SMultiplexImageNodeDelegate));
...
- (void)testDataSourceURLMethod
{
  ...
  .andDo(^(NSInvocation *inv) {
    A_SImageCacherCompletion completionBlock = [inv as_argumentAtIndexAsObject:4];
...
- (void)testUncachedDownload
{
  ...
  OCMExpect([mockCache cachedImageWithURL:[self _testImageURL] callbackQueue:OCMOCK_ANY completion:[OCMArg isNotNil]])
  .andDo(^(NSInvocation *inv){
    A_SImageCacherCompletion completion = [inv as_argumentAtIndexAsObject:4];
    completion(nil);
  });
  ...
  .andDo(^(NSInvocation *inv){
    // Simulate progress.
    A_SImageDownloaderProgress progressBlock = [inv as_argumentAtIndexAsObject:4];
    progressBlock(mockedProgress);

    // Simulate completion.
    A_SImageDownloaderCompletion completionBlock = [inv as_argumentAtIndexAsObject:5];
