...
NSErrorDomain const PI_NURLErrorDomain = @"PI_NURLErrorDomain";

@interface PI_NURLSessionManager () <NSURLSessionDelegate, NSURLSessionDataDelegate>
...
@property (nonatomic, strong) NSMutableDictionary <NSNumber *, PI_NURLSessionDataTaskCompletion> *completions;

@end

@implementation PI_NURLSessionManager

- (instancetype)initWithSessionConfiguration:(NSURLSessionConfiguration *)configuration
{
    if (self = [super init]) {
        self.sessionManagerLock = [[NSLock alloc] init];
        self.sessionManagerLock.name = @"PI_NURLSessionManager";
        self.operationQueue = [[NSOperationQueue alloc] init];
        self.operationQueue.name = @"PI_NURLSessionManager Operation Queue";
...
- (nonnull NSURLSessionDataTask *)dataTaskWithRequest:(nonnull NSURLRequest *)request completionHandler:(nonnull PI_NURLSessionDataTaskCompletion)completionHandler
{
    [self lock];
        ...
        NSString *queueName = [NSString stringWithFormat:@"PI_NURLSessionManager delegate queue - %ld", (unsigned long)dataTask.taskIdentifier];
...
- (void)URLSession:(NSURLSession *)session dataTask:(NSURLSessionDataTask *)task didReceiveResponse:(NSURLResponse *)response completionHandler:(void (^)(NSURLSessionResponseDisposition))completionHandler
{
    ...
    NSAssert(delegateQueue != nil, @"There seems to be an issue in iOS 9 where this can be nil. If you can reliably reproduce hitting this, *please* open an issue: https://github.com/pinterest/PI_NRemoteImage/issues");
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error
{
    ...
    if (!error && [task.response isKindOfClass:[NSHTTPURLResponse class]]) {
        ...
        if (statusCode >= 400 && recoverable == NO) {
            error = [NSError errorWithDomain:PI_NURLErrorDomain
                                        code:statusCode
    ...
    dispatch_async(delegateQueue, ^{
        ...
        [strongSelf lock];
            PI_NURLSessionDataTaskCompletion completionHandler = strongSelf.completions[@(task.taskIdentifier)];
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didFinishCollectingMetrics:(NSURLSessionTaskMetrics *)metrics
{
    if (@available(iOS 10.0, macOS 10.12, *)) {
        [[PI_NSpeedRecorder sharedRecorder] processMetrics:metrics forTask:task];
    }
}
