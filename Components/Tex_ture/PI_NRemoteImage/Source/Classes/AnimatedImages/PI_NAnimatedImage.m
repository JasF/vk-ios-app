...
NSErrorDomain const kPI_NAnimatedImageErrorDomain = @"kPI_NAnimatedImageErrorDomain";

//http://nullsleep.tumblr.com/post/16524517190/animated-gif-minimum-frame-delay-browser
const Float32 kPI_NAnimatedImageDefaultDuration = 0.1;

@interface PI_NAnimatedImage ()
{
    CFTimeInterval _totalDuration;
}
@end

@implementation PI_NAnimatedImage
...
{
    return MAX(self.minimumFrameInterval * [PI_NAnimatedImage maximumFramesPerSecond], 1);
}
...
- (NSTimeInterval)minimumFrameInterval
{
    ...
    dispatch_once(&onceToken, ^{
        kGreatestCommonDivisorPrecision = 2.0 / (1.0 / [PI_NAnimatedImage maximumFramesPerSecond]);
    });
