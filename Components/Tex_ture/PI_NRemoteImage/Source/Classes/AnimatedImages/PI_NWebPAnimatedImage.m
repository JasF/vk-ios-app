...
@interface PI_NWebPAnimatedImage ()
{
...
@implementation PI_NWebPAnimatedImage
...
- (instancetype)initWithAnimatedImageData:(NSData *)animatedImageData
{
    if (self = [super init]) {
        ...
        if (_demux != NULL) {
            _width = WebPDemuxGetI(_demux, WEBP_FF_CANVA_S_WIDTH);
            _height = WebPDemuxGetI(_demux, WEBP_FF_CANVA_S_HEIGHT);
            ...
            if (WebPDemuxGetFrame(_demux, 1, &iter)) {
                do {
                    ...
                    dispatch_once(&onceToken, ^{
                        maximumDuration = 1.0 / [PI_NAnimatedImage maximumFramesPerSecond];
                    });
                    if (duration < maximumDuration) {
                        duration = kPI_NAnimatedImageDefaultDuration;
                    }
...
{
    PI_NLog(@"Drawing webp image at index: %lu", (unsigned long)index);
