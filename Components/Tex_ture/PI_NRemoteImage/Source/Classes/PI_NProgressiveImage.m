...
@interface PI_NProgressiveImage ()
...
@implementation PI_NProgressiveImage
...
{
    if (self = [super init]) {
        self.lock = [[PI_NRemoteLock alloc] initWithName:@"PI_NProgressiveImage"];
...
- (CFTimeInterval)l_estimatedRemainingTime
{
    ...
    float bytesPerSecond = [[PI_NSpeedRecorder sharedRecorder] weightedAdjustedBytesPerSecondForHost:[_dataTask.currentRequest.URL host]];
    if (bytesPerSecond == -1) {
        return MAXFLOAT;
    }
...
- (PI_NImage *)currentImageBlurred:(BOOL)blurred maxProgressiveRenderSize:(CGSize)maxProgressiveRenderSize renderedImageQuality:(out CGFloat *)renderedImageQuality
{
    [self.lock lock];
    ...
    #if DEBUG
        if (self.scanTime > 0) {
            PI_NLog(@"scan time: %f", self.scanTime);
            self.scanTime = 0;
        }
    #endif
        
        PI_NImage *currentImage = nil;
        ...
        if (self.isProgressiveJPEG && self.size.width > 0 && self.size.height > 0 && progress > [_progressThresholds[self.currentThreshold] floatValue]) {
            ...
            PI_NLog(@"Generating preview image");
            CGImageRef image = CGImageSourceCreateImageAtIndex(self.imageSource, 0, NULL);
            if (image) {
                if (blurred) {
                    currentImage = [self l_postProcessImage:[PI_NImage imageWithCGImage:image] withProgress:progress];
                } else {
                    currentImage = [PI_NImage imageWithCGImage:image];
                }
...
- (PI_NImage *)l_postProcessImage:(PI_NImage *)inputImage withProgress:(float)progress
{
    PI_NImage *outputImage = nil;
