...
@protocol PI_NRemoteImageManagerAlternateRepresentationProvider;
@protocol PI_NRemoteImageCaching;

@class PI_NRemoteImageManagerResult;

extern NSErrorDomain _Nonnull const PI_NRemoteImageManagerErrorDomain;

/**
 Error codes returned by PI_NRemoteImage
 */
typedef NS_ERROR_ENUM(PI_NRemoteImageManagerErrorDomain, PI_NRemoteImageManagerError) {
    /** The image failed to decode */
    PI_NRemoteImageManagerErrorFailedToDecodeImage = 1,
    /** The image could not be downloaded and therefore could not be processed */
    PI_NRemoteImageManagerErrorFailedToFetchImageForProcessing = 2,
    /** The image returned by the processor block was nil */
    PI_NRemoteImageManagerErrorFailedToProcessImage = 3,
    /** The image in the cache was invalid */
    PI_NRemoteImageManagerErrorInvalidItemInCache = 4,
    /** The image at the URL was empty */
    PI_NRemoteImageManagerErrorImageEmpty = 5,
};

/**
 Options with which to download and process images
 */
typedef NS_OPTIONS(NSUInteger, PI_NRemoteImageManagerDownloadOptions) {
    /** Download and process with default options (no other options set) */
    PI_NRemoteImageManagerDownloadOptionsNone = 0,
    /** Set to disallow any alternate representations such as FLAnimatedImage */
    PI_NRemoteImageManagerDisallowAlternateRepresentations = 1,
    ...
    PI_NRemoteImageManagerDownloadOptionsSkipDecode = 1 << 1,
    /** Skip the early check of the memory cache */
    PI_NRemoteImageManagerDownloadOptionsSkipEarlyCheck = 1 << 2,
    /** Save processed images as JPEGs in the cache. The default is PNG to support transparency */
    PI_NRemoteImageManagerSaveProcessedImageAsJPEG = 1 << 3,
    /** Ignore cache and force download */
    PI_NRemoteImageManagerDownloadOptionsIgnoreCache = 1 << 4,
    /** Skip download retry */
    PI_NRemoteImageManagerDownloadOptionsSkipRetry = 1 << 5,
};

/**
 Priority to download and process image at.
 */
typedef NS_ENUM(NSUInteger, PI_NRemoteImageManagerPriority) {
    /** Low priority */
    PI_NRemoteImageManagerPriorityLow = 0,
    /** Default priority */
    PI_NRemoteImageManagerPriorityDefault,
    /** High priority */
    PI_NRemoteImageManagerPriorityHigh,
    PI_NRemoteImageManagerPriorityVeryHigh DEPRECATED_ATTRIBUTE = PI_NRemoteImageManagerPriorityHigh,
    PI_NRemoteImageManagerPriorityVeryLow DEPRECATED_ATTRIBUTE = PI_NRemoteImageManagerPriorityLow,
    PI_NRemoteImageManagerPriorityMedium DEPRECATED_ATTRIBUTE = PI_NRemoteImageManagerPriorityDefault,
};

float dataTaskPriorityWithImageManagerPriority(PI_NRemoteImageManagerPriority priority);

/**
 Completion called for many PI_NRemoteImage tasks as well as progress updates. Passed in a PI_NRemoteImageManagerResult.
 
 @param result PI_NRemoteImageManagerResult which contains the downloaded image.
 */
typedef void (^PI_NRemoteImageManagerImageCompletion)(PI_NRemoteImageManagerResult * __nonnull result);

/**
 Processor block to post-process a downloaded image. Passed in a PI_NRemoteImageManagerResult and a pointer to an NSUInteger which can be updated to indicate the cost of processing the image.
 ...
 */
typedef PI_NImage  * _Nullable(^PI_NRemoteImageManagerImageProcessor)(PI_NRemoteImageManagerResult * __nonnull result, NSUInteger * __nonnull cost);

/**
 PI_NRemoteImageManager is the main workhorse of PI_NRemoteImage. It is unnecessary to access directly if you simply
 ...
 */
typedef void(^PI_NRemoteImageManagerAuthenticationChallengeCompletionHandler)(NSURLSessionAuthChallengeDisposition disposition, NSURLCredential * __nullable credential);
...
/**
 ...
 @param aHandler A PI_NRemoteImageManagerAuthenticationChallengeCompletionHandler, see example for further details.
 */
typedef void(^PI_NRemoteImageManagerAuthenticationChallenge)(NSURLSessionTask * __nullable task, NSURLAuthenticationChallenge * __nonnull challenge, PI_NRemoteImageManagerAuthenticationChallengeCompletionHandler __nullable aHandler);
 ...
 */
typedef NSURLRequest * _Nonnull(^PI_NRemoteImageManagerRequestConfigurationHandler)(NSURLRequest * __nonnull request);


/**
 Handler called for many PI_NRemoteImage tasks providing the progress of the download.
 ...
 */
typedef void(^PI_NRemoteImageManagerProgressDownload)(int64_t completedBytes, int64_t totalBytes);

/** An image downloading, processing and caching manager. It uses the concept of download and processing tasks to ensure that even if multiple calls to download or process an image are made, it only occurs one time (unless an item is no longer in the cache). PI_NRemoteImageManager is backed by GCD and safe to access from multiple threads simultaneously. It ensures that images are decoded off the main thread so that animation performance isn't affected. None of its exposed methods allow for synchronous access. However, it is optimized to call completions on the calling thread if an item is in its memory cache. **/
@interface PI_NRemoteImageManager : NSObject

@property (nonatomic, readonly, nonnull) id<PI_NRemoteImageCaching> cache;

/**
 Create and return a PI_NRemoteImageManager created with the specified configuration. If configuration is nil, [NSURLSessionConfiguration defaultConfiguration] is used. Specify a custom configuration if you need to configure timeout values, cookie policies, additional HTTP headers, etc.
 @param configuration The configuration used to create the PI_NRemoteImageManager.
 @return A PI_NRemoteImageManager with the specified configuration.
 */
- (nonnull instancetype)initWithSessionConfiguration:(nullable NSURLSessionConfiguration *)configuration;

/**
 Create and return a PI_NRemoteImageManager with the specified configuration and alternative representation delegate. If configuration is nil, [NSURLSessionConfiguration defaultConfiguration] is used. Specify a custom configuration if you need to configure timeout values, cookie policies, additional HTTP headers, etc. If alternativeRepresentationProvider is nil, the default is used (and supports FLAnimatedImage).
 @param configuration The configuration used to create the PI_NRemoteImageManager.
 @param alternativeRepresentationProvider a delegate which conforms to the PI_NRemoteImageManagerAlternateRepresentationProvider protocol. Provide a delegate if you want to have non image results. @see PI_NRemoteImageManagerAlternateRepresentationProvider for an example.
 @return A PI_NRemoteImageManager with the specified configuration.
 */
- (nonnull instancetype)initWithSessionConfiguration:(nullable NSURLSessionConfiguration *)configuration
                   alternativeRepresentationProvider:(nullable id <PI_NRemoteImageManagerAlternateRepresentationProvider>)alternativeRepresentationProvider;

/**
 Create and return a PI_NRemoteImageManager with the specified configuration and alternative representation delegate. If configuration is nil, [NSURLSessionConfiguration defaultConfiguration] is used. Specify a custom configuration if you need to configure timeout values, cookie policies, additional HTTP headers, etc. If alternativeRepresentationProvider is nil, the default is used (and supports FLAnimatedImage).
 @param configuration The configuration used to create the PI_NRemoteImageManager.
 @param alternateRepDelegate a delegate which conforms to the PI_NRemoteImageManagerAlternateRepresentationProvider protocol. Provide a delegate if you want to have non image results. @see PI_NRemoteImageManagerAlternateRepresentationProvider for an example.
 @param imageCache  Optional delegate which conforms to the PI_NRemoteImageCaching protocol. Provide a delegate if you want to control image caching. By default, image manager will use most appropriate implementation available (based on PI_NCache or NSCache, depending on subspec)@see PI_NRemoteImageBasicCache for an example.
 @return A PI_NRemoteImageManager with the specified configuration.
 */
- (nonnull instancetype)initWithSessionConfiguration:(nullable NSURLSessionConfiguration *)configuration
                   alternativeRepresentationProvider:(nullable id <PI_NRemoteImageManagerAlternateRepresentationProvider>)alternateRepDelegate
                                          imageCache:(nullable id<PI_NRemoteImageCaching>)imageCache NS_DESIGNATED_INITIALIZER;


/**
 Get the shared instance of PI_NRemoteImageManager
 
 @return Shared instance of PI_NRemoteImageManager
 */
+ (nonnull instancetype)sharedImageManager;

/**
 Sets the shared instance of PI_NRemoteImageManager to an instance with the supplied configuration. If configuration is nil, [NSURLSessionConfiguration ephemeralSessionConfiguration] is used. You specify a custom configuration if you need to configure timeout values, cookie policies, additional HTTP headers, etc. This method should not be used if the shared instance has already been created.
...
/**
 The result of this method is assigned to self.cache in init. If you wish to provide a customized cache to the manager you can subclass PI_NRemoteImageManager and return a custom object, implementing PI_NRemoteImageCaching protocol from this method. Same effect could be achieved by using initWithSessionConfiguration:alternativeRepresentationProvider:imageCache: initializer.
  @warning This method is meant only for override. It will be called *once* by an instance of PI_NRemoteImageManager. The default implementation creates a new cache on every call. If you're looking to access the cache being used by an instance of PI_NRemoteImageManager, @c cache.
 @return An instance of a object, implementing PI_NRemoteImageCaching protocol.
 */
- (nonnull id<PI_NRemoteImageCaching>)defaultImageCache;
...
/**
 ...
 @param configurationBlock A PI_NRemoteImageManagerRequestConfigurationHandler block.
 */
- (void)setRequestConfiguration:(nullable PI_NRemoteImageManagerRequestConfigurationHandler)configurationBlock;

/**
 Set the Authentication Challenge Block.
 
 @param challengeBlock A PI_NRemoteImageManagerAuthenticationChallenge block.
 */
- (void)setAuthenticationChallenge:(nullable PI_NRemoteImageManagerAuthenticationChallenge)challengeBlock;
...
/**
 Sets whether PI_NRemoteImage should blur progressive render results
 
 @param shouldBlur A bool value indicating whether PI_NRemoteImage should blur progressive render results
 @param completion Completion to be called once progressThresholds is set.
 */
...
/**
 Sets the maximum size of an image that PI_NRemoteImage will render progessively. If the image is too large, progressive rendering is skipped.
 
 @param maxProgressiveRenderSize A CGSize which indicates the max size PI_NRemoteImage will render a progressive image. If an image is larger in either dimension, progressive rendering will be skipped
...
/**
 ...
 @param url NSURL where the image to prefetch resides.
 @param options PI_NRemoteImageManagerDownloadOptions options with which to pefetch the image.
 */
- (nullable NSUUID *)prefetchImageWithURL:(nonnull NSURL *)url options:(PI_NRemoteImageManagerDownloadOptions)options;
...
/**
 ...
 @param options PI_NRemoteImageManagerDownloadOptions options with which to pefetch the image.
 */
- (nonnull NSArray<NSUUID *> *)prefetchImagesWithURLs:(nonnull NSArray <NSURL *> *)urls options:(PI_NRemoteImageManagerDownloadOptions)options;
...
/**
 ...
 @param completion PI_NRemoteImageManagerImageCompletion block to call when image has been fetched from the cache or downloaded.
 ...
 */
- (nullable NSUUID *)downloadImageWithURL:(nonnull NSURL *)url completion:(nullable PI_NRemoteImageManagerImageCompletion)completion;

/**
 Download or retrieve from cache the image found at the url. All completions are called on an arbitrary callback queue unless called on the main thread and the result is in the memory cache (this is an optimization to allow synchronous results for the UI when an object is cached in memory).
 
 @param url NSURL where the image to download resides.
 @param options PI_NRemoteImageManagerDownloadOptions options with which to fetch the image.
 @param completion PI_NRemoteImageManagerImageCompletion block to call when image has been fetched from the cache or downloaded.
 @return An NSUUID which uniquely identifies this request. To be used for canceling requests and verifying that the callback is for the request you expect (see categories for example).
 */
...
/**
 ...
 @param url NSURL where the image to download resides.
 @param options PI_NRemoteImageManagerDownloadOptions options with which to fetch the image.
 @param progressImage PI_NRemoteImageManagerImageCompletion block which will be called to update progress of the image download.
 @param completion PI_NRemoteImageManagerImageCompletion block to call when image has been fetched from the cache or downloaded.
...
/**
 ...
 @param processor PI_NRemoteImageManagerImageProcessor block which will be called to post-process downloaded image.
 @param completion PI_NRemoteImageManagerImageCompletion block to call when image has been fetched from the cache or downloaded.
...
/**
 ...
 @param completion PI_NRemoteImageManagerImageCompletion block to call when image has been fetched from the cache.
 */
- (void)imageFromCacheWithCacheKey:(nonnull NSString *)cacheKey completion:(nonnull PI_NRemoteImageManagerImageCompletion)completion __attribute__((deprecated));
...
/**
 ...
 @param completion PI_NRemoteImageManagerImageCompletion block to call when image has been fetched from the cache.
 */
- (void)imageFromCacheWithCacheKey:(nonnull NSString *)cacheKey options:(PI_NRemoteImageManagerDownloadOptions)options completion:(nonnull PI_NRemoteImageManagerImageCompletion)completion __attribute__((deprecated));
...
/**
 ...
 @param completion PI_NRemoteImageManagerImageCompletion block to call when image has been fetched from the cache.
 */
- (void)imageFromCacheWithURL:(nonnull NSURL *)url processorKey:(nullable NSString *)processorKey options:(PI_NRemoteImageManagerDownloadOptions)options completion:(nonnull PI_NRemoteImageManagerImageCompletion)completion;
...
/**
 ...
 @return A PI_NRemoteImageManagerResult
 */
- (nonnull PI_NRemoteImageManagerResult *)synchronousImageFromCacheWithCacheKey:(nonnull NSString *)cacheKey options:(PI_NRemoteImageManagerDownloadOptions)options __attribute__((deprecated));
...
/**
 ...
 @return A PI_NRemoteImageManagerResult
 */
- (nonnull PI_NRemoteImageManagerResult *)synchronousImageFromCacheWithURL:(nonnull NSURL *)url processorKey:(nullable NSString *)processorKey options:(PI_NRemoteImageManagerDownloadOptions)options;
...
/**
 ...
 @see PI_NRemoteImageCategoryManager
 ...
 */
- (void)setPriority:(PI_NRemoteImageManagerPriority)priority ofTaskWithUUID:(nonnull NSUUID *)UUID;
 ...
 *
 * @param progressImageCallback a PI_NRemoteImageManagerImageCompletion block to be called with a progress update
 * @param UUID NSUUID of the task to set the priority on.
 */
...
/**
 ...
 @param retryStrategyCreationBlock a block which should create new instance of PI_NRequestRetryStrategy when called.
 */
- (void)setRetryStrategyCreationBlock:(_Nonnull id<PI_NRequestRetryStrategy> (^_Nonnull)(void))retryStrategyCreationBlock;

@property (nonatomic, readonly) _Nonnull id<PI_NRequestRetryStrategy> (^_Nonnull retryStrategyCreationBlock)(void);
