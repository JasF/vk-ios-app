...
final class DemoCellNode: A_SCellNode {
	let childA = A_SDisplayNode()
	let childB = A_SDisplayNode()
	...
	override func layoutSpecThatFits(constrainedSize: A_SSizeRange) -> A_SLayoutSpec {
		let specA = A_SRatioLayoutSpec(ratio: 1, child: childA)
		specA.style.flexBasis = A_SDimensionMakeWithPoints(1)
		specA.style.flexGrow = 1.0
		let specB = A_SRatioLayoutSpec(ratio: 1, child: childB)
		specB.style.flexBasis = A_SDimensionMakeWithPoints(1)
		specB.style.flexGrow = 1.0
		let children = state.isReverse ? [ specB, specA ] : [ specA, specB ]
		let direction: A_SStackLayoutDirection = state.isVertical ? .Vertical : .Horizontal
		return A_SStackLayoutSpec(direction: direction,
			spacing: 20,
	...
	override func animateLayoutTransition(context: A_SContextTransitioning) {
