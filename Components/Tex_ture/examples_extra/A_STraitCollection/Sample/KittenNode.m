...
- (instancetype)init
{
  ...
  [_imageNode addTarget:self action:@selector(imageTapped:) forControlEvents:A_SControlNodeEventTouchUpInside];
...
- (NSDictionary *)textStyle
{
  ...
  return @{ NSFontAttributeName: font,
            ...
            A_STextNodeWordKerningAttributeName : @.5};
}

- (A_SLayoutSpec *)layoutSpecThatFits:(A_SSizeRange)constrainedSize
{
  A_SStackLayoutSpec *stackSpec = [[A_SStackLayoutSpec alloc] init];
  ...
  if (self.asyncTraitCollection.horizontalSizeClass == UIUserInterfaceSizeClassRegular) {
    _imageNode.style.alignSelf = A_SStackLayoutAlignSelfStart;
    stackSpec.direction = A_SStackLayoutDirectionHorizontal;
  } else {
    _imageNode.style.alignSelf = A_SStackLayoutAlignSelfCenter;
    stackSpec.direction = A_SStackLayoutDirectionVertical;
  }
  
  return [A_SInsetLayoutSpec insetLayoutSpecWithInsets:UIEdgeInsetsMake(kOuterPadding, kOuterPadding, kOuterPadding, kOuterPadding) child:stackSpec];
}
...
+ (void)defaultImageTappedAction:(A_SViewController *)sourceViewController
{
  ...
  overrideVC.overrideDisplayTraitsWithTraitCollection = ^(UITraitCollection *traitCollection) {
    A_STraitCollection *asyncTraitCollection = [A_STraitCollection traitCollectionWithDisplayScale:traitCollection.displayScale
