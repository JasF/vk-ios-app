...
class ShopViewController: A_SViewController<A_STableNode> {
...
extension ShopViewController: A_STableDataSource, A_STableDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let tableViewAction = UIAlertAction(title: "A_STableNode", style: .default, handler: { (action) in
            let viewController = ProductsTableViewController(products: products)
            self.navigationController?.pushViewController(viewController, animated: true)
        })
        let collectionViewAction = UIAlertAction(title: "A_SCollectionNode", style: .default, handler: { (action) in
    ...
    func tableView(_ tableView: A_STableView, constrainedSizeForRowAt indexPath: IndexPath) -> A_SSizeRange {
        let width = UIScreen.main.bounds.width
        return A_SSizeRangeMakeExactSize(CGSize(width: width, height: 175))
    }
