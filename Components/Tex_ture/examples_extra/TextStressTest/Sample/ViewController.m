...
{
  NSMutableArray <A_STextNode *> *_textNodes;
...
- (void)viewDidLoad
{
  ...
  for (int i = 0; i < NUMBER_ELEMENTS; i++) {
    ...
    A_STextNode *node = [self createNodeForIndex:i];
...
- (A_STextNode *)createNodeForIndex:(NSUInteger)index
{
  ...
  switch (index) {
    case 0:                             // top justification (A_SDK) vs. center justification (UILabel)
      node.maximumNumberOfLines = 3;
      return node;
      
    case 1:                             // default truncation attributed string color shouldn't match attributed text color (A_SDK) vs. match (UIKit)
      node.maximumNumberOfLines = 3;
