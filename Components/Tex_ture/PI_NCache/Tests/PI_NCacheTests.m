...
#import "PI_NCacheTests.h"
#import <PI_NCache/PI_NCache.h>
#import <PI_NOperation/PI_NOperation.h>


#if TARGET_OS_IPHONE || TARGET_IPHONE_SIMULATOR
  typedef UIImage PI_NImage;
#else
  typedef NSImage PI_NImage;
#endif

static NSString * const PI_NCacheTestName = @"PI_NCacheTest";
const NSTimeInterval PI_NCacheTestBlockTimeout = 20.0;
...
@interface PI_NCacheTests ()
@property (strong, nonatomic) PI_NCache *cache;
@end

@implementation PI_NCacheTests
...
{
    [super setUp];
    self.cache = [[PI_NCache alloc] initWithName:[[NSUUID UUID] UUIDString]];
    ...
    if (!image) {
        ...
        NSData *imageData = [[NSData alloc] initWithContentsOfURL:imageURL
                                                          options:NSDataReadingUncached
                                                            error:&error];
		image = [[PI_NImage alloc] initWithData:imageData];
    }
...
{
    return dispatch_time(DISPATCH_TIME_NOW, (int64_t)(PI_NCacheTestBlockTimeout * NSEC_PER_SEC));
}
...
{
    PI_NCache *cache = [[PI_NCache alloc] initWithName:PI_NCacheTestName];
    XCTAssertTrue([cache.name isEqualToString:PI_NCacheTestName], @"wrong name");
...
- (void)testDiskCacheURL
{
    ...
    dispatch_group_t group = dispatch_group_create();
    dispatch_group_enter(group);
    [self.cache objectForKeyAsync:@"" completion:^(PI_NCache * _Nonnull cache, NSString * _Nonnull key, id  _Nullable object) {
      dispatch_group_leave(group);
    }];
...
- (void)testObjectSet
{
    ...
    [self.cache setObjectAsync:[self image] forKey:key completion:^(PI_NCache *cache, NSString *key, id object) {
        image = (PI_NImage *)object;
...
- (void)testObjectSetWithCost
{
    ...
    __block PI_NImage *image = nil;
    dispatch_semaphore_t semaphore = dispatch_semaphore_create(0);
    PI_NImage *srcImage = [self image];
    NSUInteger cost = (NSUInteger)(srcImage.size.width * srcImage.size.height);
    
    [self.cache setObjectAsync:srcImage forKey:key withCost:cost completion:^(PI_NCache *cache, NSString *key, id object) {
        image = (PI_NImage *)object;
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)testObjectSetWithDuplicateKey
{
    ...
    [self.cache objectForKeyAsync:key completion:^(PI_NCache *cache, NSString *key, id object) {
...
- (void)testObjectGetWithInvalidKey
{
    ...
    [self.cache objectForKeyAsync:invalidKey completion:^(PI_NCache *cache, NSString *key, id object) {
        image = (PI_NImage *)object;
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)testObjectRemove
{
    ...
    [self.cache removeObjectForKeyAsync:key completion:^(PI_NCache *cache, NSString *key, id object) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)testObjectRemoveAll
{
    ...
    [self.cache removeAllObjectsAsync:^(PI_NCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
#pragma clang diagnostic ignored "-Warc-retain-cycles"
        dispatch_group_enter(group);
        [self.cache setObjectAsync:obj forKey:key completion:^(PI_NCache * _Nonnull cache, NSString * _Nonnull key, id  _Nullable object) {
            dispatch_async(queue, ^{
                [self.cache objectForKeyAsync:key completion:^(PI_NCache * _Nonnull cache, NSString * _Nonnull key, id  _Nullable object) {
                    NSString *obj = [[NSString alloc] initWithFormat:@"obj %lu", (unsigned long)i];
...
- (void)testMemoryWarningBlock
{
    ...
    self.cache.memoryCache.didReceiveMemoryWarningBlock = ^(PI_NMemoryCache *cache) {
        blockDidExecute = YES;
        dispatch_semaphore_signal(semaphore);
    };
...
- (void)testBackgroundBlock
{
    ...
    self.cache.memoryCache.didEnterBackgroundBlock = ^(PI_NMemoryCache *cache) {
        blockDidExecute = YES;
        dispatch_semaphore_signal(semaphore);
    };
...
- (void)testMemoryCacheEnumerationWithWarning
{
    ...
    self.cache.memoryCache.didReceiveMemoryWarningBlock = ^(PI_NMemoryCache *cache) {
        [cache enumerateObjectsWithBlockAsync:^(PI_NMemoryCache *cache, NSString *key, id object, BOOL *stop) {
            @synchronized (self) {
                enumCount++;
            }
        } completionBlock:^(PI_NMemoryCache *cache) {
            dispatch_semaphore_signal(semaphore);
        }];
    };
...
- (void)testDiskCacheEnumeration
{
    ...
    dispatch_apply(objectCount, dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(size_t index) {
        ...
        NSString *obj = [[NSString alloc] initWithFormat:@"obj %zd", index];
        dispatch_group_enter(group);
        [self.cache.diskCache setObjectAsync:obj forKey:key completion:^(PI_NDiskCache *cache, NSString *key, id<NSCoding> object) {
            dispatch_group_leave(group);
        }];
    });
    ...
    } completionBlock:^(PI_NDiskCache *cache) {
        dispatch_semaphore_signal(semaphore);
    }];
...
- (void)testAgeLimit
{
    ...
    dispatch_group_enter(group);
    [self.cache.memoryCache objectForKeyAsync:key1 completion:^(PI_NMemoryCache *cache, NSString *key, id object) {
        memObj1 = object;
        dispatch_group_leave(group);
    }];
    
    dispatch_group_enter(group);
    [self.cache.memoryCache objectForKeyAsync:key2 completion:^(PI_NMemoryCache *cache, NSString *key, id object) {
        memObj2 = object;
        dispatch_group_leave(group);
    }];
    
    dispatch_group_enter(group);
    [self.cache.diskCache objectForKeyAsync:key1 completion:^(PI_NDiskCache *cache, NSString *key, id<NSCoding> object) {
        diskObj1 = object;
        dispatch_group_leave(group);
    }];
    
    dispatch_group_enter(group);
    [self.cache.diskCache objectForKeyAsync:key2 completion:^(PI_NDiskCache *cache, NSString *key, id<NSCoding> object) {
        diskObj2 = object;
        dispatch_group_leave(group);
    }];
...
- (void)testByteLimit
{
    ...
    [self.cache.diskCache synchronouslyLockFileAccessWhileExecutingBlock:^(id<PI_NCaching>  _Nonnull cache) {
        NSError *error = nil;
...
- (void)testDiskReadingAfterCacheInit
{
    ...
    PI_NDiskCache *diskCache = [[PI_NDiskCache alloc] initWithName:cacheName];
    ...
    diskCache = nil;
    diskCache = [[PI_NDiskCache alloc] initWithName:cacheName];
...
- (void)testWritingProtectionOption
{
  ...
  [self.cache.diskCache setObjectAsync:[self image] forKey:key completion:^(PI_NDiskCache *cache, NSString *key, id<NSCoding> object) {
...
- (void)_testTTLCacheObjectAccess {
    ...
    dispatch_group_enter(group);
    [self.cache.memoryCache objectForKeyAsync:key completion:^(PI_NMemoryCache *cache, NSString *key, id object) {
        memObj = object;
        dispatch_group_leave(group);
    }];

    dispatch_group_enter(group);
    [self.cache.diskCache objectForKeyAsync:key completion:^(PI_NDiskCache *cache, NSString *key, id<NSCoding> object) {
        diskObj = object;
        dispatch_group_leave(group);
    }];
...
- (void)_testTTLCacheObjectEnumeration {
    ...
    dispatch_group_t group = dispatch_group_create();
    dispatch_group_enter(group);
    [self.cache.diskCache objectForKeyAsync:key completion:^(PI_NDiskCache * _Nonnull cache, NSString * _Nonnull key, id<NSCoding>  _Nullable object) {
        dispatch_group_leave(group);
    }];
    dispatch_group_wait(group, DISPATCH_TIME_FOREVER);
    ...
    objCount = 0;
    [self.cache.memoryCache enumerateObjectsWithBlock:^(PI_NMemoryCache *cache, NSString *key, id _Nullable object, BOOL *stop) {
      objCount++;
    }];
    ...
    objCount = 0;
    [self.cache.memoryCache enumerateObjectsWithBlock:^(PI_NMemoryCache * _Nonnull cache, NSString * _Nonnull key, id  _Nullable object, BOOL *stop) {
      objCount++;
    }];
...
- (void)_testTTLCacheFileURLForKey {
    ...
    [self.cache.diskCache setObjectAsync:[self image] forKey:key completion:^(PI_NDiskCache * _Nonnull cache, NSString * _Nonnull key, id<NSCoding>  _Nullable object) {
        [cache fileURLForKeyAsync:key completion:^(NSString * _Nonnull key, NSURL * _Nullable fileURL) {
            objectURL = fileURL;
            dispatch_group_leave(group);
        }];
...
- (void)testAsyncDiskInitialization
{
    ...
    PI_NDiskCache *testCache = [[PI_NDiskCache alloc] initWithName:cacheName];
    ...
    testCache = [[PI_NDiskCache alloc] initWithName:cacheName];
...
{
  PI_NDiskCache *testCache = [[PI_NDiskCache alloc] initWithName:@"testDiskCacheSet"];
...
{
  PI_NDiskCache *testCache = [[PI_NDiskCache alloc] initWithName:@"textDiskCacheHit"];
  const NSUInteger objectCount = 100;
  for (NSUInteger idx = 0; idx < objectCount; idx++) {
    [testCache setObject:[@(idx) stringValue] forKey:[@(idx) stringValue]];
  }
...
{
  PI_NDiskCache *testCache = [[PI_NDiskCache alloc] initWithName:@"testDiskCacheMiss"];
  const NSUInteger objectCount = 100;
  [self measureBlock:^{
    for (NSUInteger idx = 0; idx < objectCount; idx++) {
      [testCache objectForKey:[@(idx) stringValue]];
...
- (void)testDiskCacheEmptyTrash
{
    ...
    dispatch_group_enter(group);
    [self.cache.diskCache removeAllObjectsAsync:^(PI_NDiskCache * _Nonnull cache) {
...
- (void)testCustomEncoderDecoder {
    ...
    PI_NDiskCacheKeyEncoderBlock encoder = ^NSString *(NSString *decodedKey) {
        return decodedKey;
    };
    PI_NDiskCacheKeyDecoderBlock decoder = ^NSString *(NSString *encodedKey) {
        return encodedKey;
    };
    PI_NDiskCache *testCache = [[PI_NDiskCache alloc] initWithName:@"testCustomEncoder"
                                                    ...
                                                    deserializer:NULL
                                                      keyEncoder:encoder
                                                      keyDecoder:decoder
                                                  operationQueue:[PI_NOperationQueue sharedOperationQueue]];
