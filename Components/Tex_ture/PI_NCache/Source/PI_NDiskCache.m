...
#define PI_NDiskCacheError(error) if (error) { NSLog(@"%@ (%d) ERROR: %@", \
[[NSString stringWithUTF8String:__FILE__] lastPathComponent], \
__LINE__, [error localizedDescription]); }

#define PI_NDiskCacheException(exception) if (exception) { NSAssert(NO, [exception reason]); }

NSString * const PI_NDiskCachePrefix = @"com.pinterest.PI_NDiskCache";
static NSString * const PI_NDiskCacheSharedName = @"PI_NDiskCacheShared";

static NSString * const PI_NDiskCacheOperationIdentifierTrimToDate = @"PI_NDiskCacheOperationIdentifierTrimToDate";
static NSString * const PI_NDiskCacheOperationIdentifierTrimToSize = @"PI_NDiskCacheOperationIdentifierTrimToSize";
static NSString * const PI_NDiskCacheOperationIdentifierTrimToSizeByDate = @"PI_NDiskCacheOperationIdentifierTrimToSizeByDate";

typedef NS_ENUM(NSUInteger, PI_NDiskCacheCondition) {
    PI_NDiskCacheConditionNotReady = 0,
    PI_NDiskCacheConditionReady = 1,
};

static PI_NOperationDataCoalescingBlock PI_NDiskTrimmingSizeCoalescingBlock = ^id(NSNumber *existingSize, NSNumber *newSize) {
    NSComparisonResult result = [existingSize compare:newSize];
    return (result == NSOrderedDescending) ? newSize : existingSize;
};

static PI_NOperationDataCoalescingBlock PI_NDiskTrimmingDateCoalescingBlock = ^id(NSDate *existingDate, NSDate *newDate) {
    NSComparisonResult result = [existingDate compare:newDate];
    return (result == NSOrderedDescending) ? newDate : existingDate;
};

@interface PI_NDiskCacheMetadata : NSObject
@property (nonatomic, strong) NSDate *date;
@property (nonatomic, strong) NSNumber *size;
@end

@interface PI_NDiskCache () {
    PI_NDiskCacheSerializerBlock _serializer;
    PI_NDiskCacheDeserializerBlock _deserializer;
    
    PI_NDiskCacheKeyEncoderBlock _keyEncoder;
    PI_NDiskCacheKeyDecoderBlock _keyDecoder;
}
...
@property (strong, nonatomic) NSMutableDictionary <NSString *, PI_NDiskCacheMetadata *> *metadata;
...
- (instancetype)init
{
    @throw [NSException exceptionWithName:@"Must initialize with a name" reason:@"PI_NDiskCache must be initialized with a name. Call initWithName: instead." userInfo:nil];
...
- (instancetype)initWithName:(NSString *)name rootPath:(NSString *)rootPath serializer:(PI_NDiskCacheSerializerBlock)serializer deserializer:(PI_NDiskCacheDeserializerBlock)deserializer
{
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    return [self initWithName:name rootPath:rootPath serializer:serializer deserializer:deserializer operationQueue:[PI_NOperationQueue sharedOperationQueue]];
#pragma clang diagnostic pop
}

- (instancetype)initWithName:(NSString *)name
                    rootPath:(NSString *)rootPath
                  serializer:(PI_NDiskCacheSerializerBlock)serializer
                deserializer:(PI_NDiskCacheDeserializerBlock)deserializer
              operationQueue:(PI_NOperationQueue *)operationQueue
{
  return [self initWithName:name
                     prefix:PI_NDiskCachePrefix
                   rootPath:rootPath
...
- (instancetype)initWithName:(NSString *)name
                  ...
                  serializer:(PI_NDiskCacheSerializerBlock)serializer
                deserializer:(PI_NDiskCacheDeserializerBlock)deserializer
                  keyEncoder:(PI_NDiskCacheKeyEncoderBlock)keyEncoder
                  keyDecoder:(PI_NDiskCacheKeyDecoderBlock)keyDecoder
              operationQueue:(PI_NOperationQueue *)operationQueue
{
    if (!name)
        return nil;
    ...
    NSAssert(((!serializer && !deserializer) || (serializer && deserializer)),
             @"PI_NDiskCache must be initialized with a serializer AND deserializer.");
    
    NSAssert(((!keyEncoder && !keyDecoder) || (keyEncoder && keyDecoder)),
              @"PI_NDiskCache must be initialized with a encoder AND decoder.");
...
{
    return [[NSString alloc] initWithFormat:@"%@.%@.%p", PI_NDiskCachePrefix, _name, (void *)self];
}

+ (PI_NDiskCache *)sharedCache
{
    static PI_NDiskCache *cache;
    static dispatch_once_t predicate;
    
    dispatch_once(&predicate, ^{
        cache = [[PI_NDiskCache alloc] initWithName:PI_NDiskCacheSharedName];
    });
...
- (PI_NDiskCacheSerializerBlock)defaultSerializer
{
...
- (PI_NDiskCacheDeserializerBlock)defaultDeserializer
{
...
- (PI_NDiskCacheKeyEncoderBlock)defaultKeyEncoder
{
...
- (PI_NDiskCacheKeyEncoderBlock)defaultKeyDecoder
{
...
+ (dispatch_queue_t)sharedTrashQueue
{
    ...
    dispatch_once(&predicate, ^{
        NSString *queueName = [[NSString alloc] initWithFormat:@"%@.trash", PI_NDiskCachePrefix];
        ...
        if (_sharedTrashURL == nil) {
            ...
            [[NSFileManager defaultManager] createDirectoryAtURL:_sharedTrashURL
                                     withIntermediateDirectories:YES
                                                      attributes:nil
                                                           error:&error];
            PI_NDiskCacheError(error);
        }
        trashURL = _sharedTrashURL;
    [[PI_NDiskCache sharedLock] unlock];
...
+ (BOOL)moveItemAtURLToTrash:(NSURL *)itemURL
{
    ...
    NSError *error = nil;
    NSString *uniqueString = [[NSProcessInfo processInfo] globallyUniqueString];
    NSURL *uniqueTrashURL = [[PI_NDiskCache sharedTrashURL] URLByAppendingPathComponent:uniqueString isDirectory:NO];
...
+ (void)emptyTrash
{
    dispatch_async([PI_NDiskCache sharedTrashQueue], ^{
        NSURL *trashURL = nil;
        ...
        if (trashURL != nil) {
            ...
            PI_NDiskCacheError(removeTrashedItemError);
        }
    });
}
    ...
    for (NSURL *fileURL in files) {
        ...
        [self lock];
            ...
            if (_metadata[key] == nil) {
                _metadata[key] = [[PI_NDiskCacheMetadata alloc] init];
            }
...
- (BOOL)removeFileAndExecuteBlocksForKey:(NSString *)key
{
    ...
    [self lockForWriting];
        ...
        BOOL trashed = [PI_NDiskCache moveItemAtURLToTrash:fileURL];
        if (!trashed) {
            [self unlock];
            return NO;
        }
    
        [PI_NDiskCache emptyTrash];
...
- (void)trimDiskToSize:(NSUInteger)trimByteCount
{
    ...
    [self lockForWriting];
        if (_byteCount > trimByteCount) {
            ...
            NSArray *keysSortedBySize = [_metadata keysSortedByValueUsingComparator:^NSComparisonResult(PI_NDiskCacheMetadata * _Nonnull obj1, PI_NDiskCacheMetadata * _Nonnull obj2) {
...
- (void)trimDiskToSizeByDate:(NSUInteger)trimByteCount
{
    ...
    [self lockForWriting];
        if (_byteCount > trimByteCount) {
            ...
            NSArray *keysSortedByDate = [_metadata keysSortedByValueUsingComparator:^NSComparisonResult(PI_NDiskCacheMetadata * _Nonnull obj1, PI_NDiskCacheMetadata * _Nonnull obj2) {
...
- (void)containsObjectForKeyAsync:(NSString *)key completion:(PI_NDiskCacheContainsBlock)block
{
    if (!key || !block)
        return;
...
- (void)objectForKeyAsync:(NSString *)key completion:(PI_NDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        NSURL *fileURL = nil;
...
- (void)setObjectAsync:(id <NSCoding>)object forKey:(NSString *)key completion:(PI_NDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        NSURL *fileURL = nil;
...
{
    [self setObjectAsync:object forKey:key completion:(PI_NDiskCacheObjectBlock)block];
}

- (void)removeObjectForKeyAsync:(NSString *)key completion:(PI_NDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        NSURL *fileURL = nil;
...
- (void)trimToSizeAsync:(NSUInteger)trimByteCount completion:(PI_NCacheBlock)block
{
    PI_NOperationBlock operation = ^(id data) {
    ...
    [self.operationQueue scheduleOperation:operation
                              withPriority:PI_NOperationQueuePriorityLow
                                identifier:PI_NDiskCacheOperationIdentifierTrimToSize
                            coalescingData:[NSNumber numberWithUnsignedInteger:trimByteCount]
                       dataCoalescingBlock:PI_NDiskTrimmingSizeCoalescingBlock
...
- (void)trimToDateAsync:(NSDate *)trimDate completion:(PI_NCacheBlock)block
{
    ...
    [self.operationQueue scheduleOperation:operation
                              withPriority:PI_NOperationQueuePriorityLow
                                identifier:PI_NDiskCacheOperationIdentifierTrimToDate
                            coalescingData:trimDate
                       dataCoalescingBlock:PI_NDiskTrimmingDateCoalescingBlock
                                completion:completion];
}

- (void)trimToSizeByDateAsync:(NSUInteger)trimByteCount completion:(PI_NCacheBlock)block
{
    PI_NOperationBlock operation = ^(id data){
    ...
    [self.operationQueue scheduleOperation:operation
                              withPriority:PI_NOperationQueuePriorityLow
                                identifier:PI_NDiskCacheOperationIdentifierTrimToSizeByDate
                            coalescingData:[NSNumber numberWithUnsignedInteger:trimByteCount]
                       dataCoalescingBlock:PI_NDiskTrimmingSizeCoalescingBlock
                                completion:completion];
}
...
- (void)enumerateObjectsWithBlockAsync:(PI_NDiskCacheFileURLEnumerationBlock)block completionBlock:(PI_NCacheBlock)completionBlock
{
    [self.operationQueue scheduleOperation:^{
...
- (void)synchronouslyLockFileAccessWhileExecutingBlock:(PI_NCacheBlock)block
{
    if (block) {
        [self lockForWriting];
            block(self);
        ...
        if (!self->_ttlCache || self->_ageLimit <= 0 || fabs([_metadata[key].date timeIntervalSinceDate:now]) < self->_ageLimit) {
            ...
            if (objectData) {
              ...
              @catch (NSException *exception) {
                  ...
                  [self unlock];
                  PI_NDiskCacheError(error)
                  PI_NDiskCacheException(exception);
              }
              [self lock];
            }
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key fileURL:(NSURL **)outFileURL
{
    ...
    [self lockForWriting];
        PI_NCacheObjectBlock willAddObjectBlock = self->_willAddObjectBlock;
        ...
        PI_NCacheObjectBlock didAddObjectBlock = self->_didAddObjectBlock;
...
- (void)removeAllObjects
{
    ...
    [self lockForWriting];
        PI_NCacheBlock willRemoveAllObjectsBlock = self->_willRemoveAllObjectsBlock;
        ...
        [PI_NDiskCache moveItemAtURLToTrash:self->_cacheURL];
        [PI_NDiskCache emptyTrash];
        ...
        PI_NCacheBlock didRemoveAllObjectsBlock = self->_didRemoveAllObjectsBlock;
...
- (void)enumerateObjectsWithBlock:(PI_NDiskCacheFileURLEnumerationBlock)block
{
    if (!block)
        return;
...
- (PI_NDiskCacheObjectBlock)willAddObjectBlock
{
    PI_NDiskCacheObjectBlock block = nil;
...
- (void)setWillAddObjectBlock:(PI_NDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        [self lock];
...
- (PI_NDiskCacheObjectBlock)willRemoveObjectBlock
{
    PI_NDiskCacheObjectBlock block = nil;
...
- (void)setWillRemoveObjectBlock:(PI_NDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        [self lock];
...
- (PI_NDiskCacheObjectBlock)didAddObjectBlock
{
    PI_NDiskCacheObjectBlock block = nil;
...
- (void)setDidAddObjectBlock:(PI_NDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        [self lock];
...
- (PI_NDiskCacheObjectBlock)didRemoveObjectBlock
{
    PI_NDiskCacheObjectBlock block = nil;
...
- (void)setDidRemoveObjectBlock:(PI_NDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        [self lock];
...
- (void)lock
{
    ...
    NSAssert(result == 0, @"Failed to lock PI_NDiskCache %@. Code: %d", self, result);
}

- (void)unlock
{
    __unused int result = pthread_mutex_unlock(&_mutex);
    NSAssert(result == 0, @"Failed to unlock PI_NDiskCache %@. Code: %d", self, result);
}

@end

@implementation PI_NDiskCache (Deprecated)

- (void)lockFileAccessWhileExecutingBlock:(nullable PI_NCacheBlock)block
{
    [self lockFileAccessWhileExecutingBlockAsync:block];
}

- (void)containsObjectForKey:(NSString *)key block:(PI_NDiskCacheContainsBlock)block
{
    [self containsObjectForKeyAsync:key completion:block];
}

- (void)objectForKey:(NSString *)key block:(nullable PI_NDiskCacheObjectBlock)block
{
    [self objectForKeyAsync:key completion:block];
}

- (void)fileURLForKey:(NSString *)key block:(nullable PI_NDiskCacheFileURLBlock)block
{
    [self fileURLForKeyAsync:key completion:block];
}

- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key block:(nullable PI_NDiskCacheObjectBlock)block
{
    [self setObjectAsync:object forKey:key completion:block];
}

- (void)removeObjectForKey:(NSString *)key block:(nullable PI_NDiskCacheObjectBlock)block
{
    [self removeObjectForKeyAsync:key completion:block];
}

- (void)trimToDate:(NSDate *)date block:(nullable PI_NDiskCacheBlock)block
{
    [self trimToDateAsync:date completion:block];
}

- (void)trimToSize:(NSUInteger)byteCount block:(nullable PI_NDiskCacheBlock)block
{
    [self trimToSizeAsync:byteCount completion:block];
}

- (void)trimToSizeByDate:(NSUInteger)byteCount block:(nullable PI_NDiskCacheBlock)block
{
    [self trimToSizeAsync:byteCount completion:block];
}

- (void)removeAllObjects:(nullable PI_NDiskCacheBlock)block
{
    [self removeAllObjectsAsync:block];
}

- (void)enumerateObjectsWithBlock:(PI_NDiskCacheFileURLBlock)block completionBlock:(nullable PI_NDiskCacheBlock)completionBlock
{
...
@implementation PI_NDiskCacheMetadata
@end
