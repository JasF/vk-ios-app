...
@interface ViewController () <A_SPagerDataSource, A_SPagerDelegate>
{
  A_SPagerNode *_pagerNode;
}
...
- (instancetype)init
{
  ...
  A_SDisplayNode.shouldShowRangeDebugOverlay = YES;
  
  // Could implement A_SCollectionDelegate if we wanted extra callbacks, like from UIScrollView.
  //_pagerNode.delegate = self;
...
- (A_SSizeRange)pagerNode:(A_SPagerNode *)pagerNode constrainedSizeForNodeAtIndex:(NSInteger)index;
{
  return A_SSizeRangeMake(pagerNode.bounds.size);
}
