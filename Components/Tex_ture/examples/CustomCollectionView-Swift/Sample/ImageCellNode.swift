...
class ImageCellNode: A_SCellNode {
  ...
  override func layoutSpecThatFits(_ constrainedSize: A_SSizeRange) -> A_SLayoutSpec {
    ...
    let imagePlace = A_SRatioLayoutSpec(ratio: imageRatio, child: imageNode)
    
    let stackLayout = A_SStackLayoutSpec.horizontal()
    stackLayout.justifyContent = .start
    stackLayout.alignItems = .start
    stackLayout.style.flexShrink = 1.0
    stackLayout.children = [imagePlace]
    
    return  A_SInsetLayoutSpec(insets: UIEdgeInsets.zero, child: stackLayout)
  }
