...
//  distributed under the License is distributed on an "A_S IS" BA_SIS,
...
protocol MosaicCollectionViewLayoutDelegate: A_SCollectionDelegate {
...
class MosaicCollectionViewLayoutInspector: NSObject, A_SCollectionViewLayoutInspecting
{
  func collectionView(_ collectionView: A_SCollectionView, constrainedSizeForNodeAt indexPath: IndexPath) -> A_SSizeRange {
    let layout = collectionView.collectionViewLayout as! MosaicCollectionViewLayout
    return A_SSizeRangeMake(CGSize.zero, layout._itemSizeAtIndexPath(indexPath: indexPath))
  }
  
  func collectionView(_ collectionView: A_SCollectionView, constrainedSizeForSupplementaryNodeOfKind: String, at atIndexPath: IndexPath) -> A_SSizeRange
  {
    let layout = collectionView.collectionViewLayout as! MosaicCollectionViewLayout
    return A_SSizeRange.init(min: CGSize.zero, max: layout._headerSizeForSection(section: atIndexPath.section))
  }
  
  /**
   * Asks the inspector for the number of supplementary sections in the collection view for the given kind.
   */
  func collectionView(_ collectionView: A_SCollectionView, numberOfSectionsForSupplementaryNodeOfKind kind: String) -> UInt {
   ...
   */
  func collectionView(_ collectionView: A_SCollectionView, supplementaryNodesOfKind kind: String, inSection section: UInt) -> UInt {
    if (kind == UICollectionElementKindSectionHeader) {
      return 1
    } else {
      return 0
  ...
  func scrollableDirections() -> A_SScrollDirection {
