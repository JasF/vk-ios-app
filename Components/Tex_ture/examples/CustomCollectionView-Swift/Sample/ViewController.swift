...
class ViewController: A_SViewController<A_SCollectionNode>, MosaicCollectionViewLayoutDelegate, A_SCollectionDataSource, A_SCollectionDelegate {
  
  var _sections = [[UIImage]]()
  let _collectionNode: A_SCollectionNode
  ...
  init() {
    ...
    _collectionNode = A_SCollectionNode(frame: CGRect.zero, collectionViewLayout: layout)
  ...
  func collectionNode(_ collectionNode: A_SCollectionNode, nodeForItemAt indexPath: IndexPath) -> A_SCellNode {
    let image = _sections[indexPath.section][indexPath.item]
    return ImageCellNode(with: image)
  }
  
  
  func collectionNode(_ collectionNode: A_SCollectionNode, nodeForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> A_SCellNode {
    let textAttributes : NSDictionary = [
      NSFontAttributeName: UIFont.preferredFont(forTextStyle: UIFontTextStyle.headline),
      NSForegroundColorAttributeName: UIColor.gray
    ]
    let textInsets = UIEdgeInsets(top: 11, left: 0, bottom: 11, right: 0)
    let textCellNode = A_STextCellNode(attributes: textAttributes as! [AnyHashable : Any], insets: textInsets)
    textCellNode.text = String(format: "Section %zd", indexPath.section + 1)
    return textCellNode
  }
  

  func numberOfSections(in collectionNode: A_SCollectionNode) -> Int {
    return _sections.count
  }

  func collectionNode(_ collectionNode: A_SCollectionNode, numberOfItemsInSection section: Int) -> Int {
