...
@interface ViewController()<A_SVideoNodeDelegate>
@property (nonatomic, strong) A_SDisplayNode *rootNode;
@property (nonatomic, strong) A_SVideoNode *guitarVideoNode;
@end
...
- (void)viewDidLoad
{
  ...
  A_SVideoNode *guitarVideoNode = self.guitarVideoNode;
  [_rootNode addSubnode:self.guitarVideoNode];
  
  A_SVideoNode *nicCageVideoNode = self.nicCageVideoNode;
  [_rootNode addSubnode:nicCageVideoNode];
  
  // Video node with custom play button
  A_SVideoNode *simonVideoNode = self.simonVideoNode;
  [_rootNode addSubnode:simonVideoNode];
  
  A_SVideoNode *hlsVideoNode = self.hlsVideoNode;
  [_rootNode addSubnode:hlsVideoNode];
  
  CGSize mainScreenBoundsSize = [UIScreen mainScreen].bounds.size;
  
  _rootNode.layoutSpecBlock = ^A_SLayoutSpec *(A_SDisplayNode * _Nonnull node, A_SSizeRange constrainedSize) {
...
- (A_SVideoNode *)guitarVideoNode;
{
  if (_guitarVideoNode) {
    return _guitarVideoNode;
  }
  
  _guitarVideoNode = [[A_SVideoNode alloc] init];
...
- (A_SVideoNode *)nicCageVideoNode;
{
  A_SVideoNode *nicCageVideoNode = [[A_SVideoNode alloc] init];
...
- (A_SVideoNode *)simonVideoNode
{
  A_SVideoNode *simonVideoNode = [[A_SVideoNode alloc] init];
...
- (A_SVideoNode *)hlsVideoNode;
{
  A_SVideoNode *hlsVideoNode = [[A_SVideoNode alloc] init];
...
- (A_SButtonNode *)playButton;
{
  A_SButtonNode *playButtonNode = [[A_SButtonNode alloc] init];
  ...
  //playButtonNode.contentHorizontalAlignment = A_SHorizontalAlignmentStart;
  //playButtonNode.contentVerticalAlignment = A_SVerticalAlignmentCenter;
...
- (void)didTapVideoNode:(A_SVideoNode *)videoNode
{
  if (videoNode == self.guitarVideoNode) {
    if (videoNode.playerState == A_SVideoNodePlayerStatePlaying) {
      [videoNode pause];
    } else if(videoNode.playerState == A_SVideoNodePlayerStateLoading) {
      [videoNode pause];
    } else {
      [videoNode play];
    }
...
- (void)videoNode:(A_SVideoNode *)videoNode willChangePlayerState:(A_SVideoNodePlayerState)state toState:(A_SVideoNodePlayerState)toState
{
  ...
  } else if (toState == A_SVideoNodePlayerStateFinished) {
    NSLog(@"guitarVideoNode finished");
  } else if (toState == A_SVideoNodePlayerStateLoading) {
