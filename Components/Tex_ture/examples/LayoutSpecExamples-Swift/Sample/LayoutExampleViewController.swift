...
class LayoutExampleViewController: A_SViewController<A_SDisplayNode> {
  ...
  init(layoutExampleType: LayoutExampleNode.Type) {
    ...
    super.init(node: A_SDisplayNode())
    ...
    self.node.layoutSpecBlock = { [weak self] node, constrainedSize in
      guard let customNode = self?.customNode else { return A_SLayoutSpec() }
      return A_SCenterLayoutSpec(centeringOptions: needsOnlyYCentering ? .Y : .XY,
