...
#import "A_SListKitTestAdapterDataSource.h"
#import "A_SXCTExtensions.h"
#import <JGMethodSwizzler/JGMethodSwizzler.h>
#import "A_STestCase.h"

@interface A_SListKitTests : A_STestCase

@property (nonatomic, strong) A_SCollectionNode *collectionNode;
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) IGListAdapter *adapter;
@property (nonatomic, strong) A_SListKitTestAdapterDataSource *dataSource;
...
@implementation A_SListKitTests

- (void)setUp
{
  [super setUp];

  [A_SCollectionView swizzleInstanceMethod:@selector(reloadData) withReplacement:JGMethodReplacementProviderBlock {
    return JGMethodReplacement(void, A_SCollectionView *) {
  ...
  self.collectionNode = [[A_SCollectionNode alloc] initWithCollectionViewLayout:self.layout];
  ...
  self.dataSource = [[A_SListKitTestAdapterDataSource alloc] init];
  self.adapter = [[IGListAdapter alloc] initWithUpdater:updater
                                         viewController:nil
                                       workingRangeSize:0];
  self.adapter.dataSource = self.dataSource;
  [self.adapter setA_SDKCollectionNode:self.collectionNode];
  XCTAssertNotNil(self.adapter.collectionView, @"Adapter was not bound to collection view. You may have a stale copy of Async_DisplayKit that was built without IG_LIST_KIT. Clean Builder Folder IMO.");
}

- (void)tearDown
{
  [super tearDown];
  XCTAssert([A_SCollectionView deswizzleAllMethods]);
