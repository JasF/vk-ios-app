...
#import <Async_DisplayKit/A_SImageNode+CGExtras.h>
...
typedef void (^A_SImageNodeDrawParametersBlock)(A_SWeakMapEntry *entry);

@interface A_SImageNodeDrawParameters : NSObject {
@package
  UIImage *_image;
  BOOL _opaque;
  CGRect _bounds;
  ...
  A_SImageNodeDrawParametersBlock _didDrawBlock;
}

@end

@implementation A_SImageNodeDrawParameters
 ...
 */
@interface A_SImageNodeContentsKey : NSObject
...
@property (nonatomic, copy) A_SDisplayNodeContextModifier willDisplayNodeContentWithRenderingContext;
@property (nonatomic, copy) A_SDisplayNodeContextModifier didDisplayNodeContentWithRenderingContext;
@property (nonatomic, copy) asimagenode_modification_block_t imageModificationBlock;

@end

@implementation A_SImageNodeContentsKey
...
- (BOOL)isEqual:(id)object
{
  ...
  if ([object isKindOfClass:[A_SImageNodeContentsKey class]]) {
    A_SImageNodeContentsKey *other = (A_SImageNodeContentsKey *)object;
...
@implementation A_SImageNode
{
@private
  UIImage *_image;
  A_SWeakMapEntry *_weakCacheEntry;  // Holds a reference that keeps our contents in cache.


  void (^_displayCompletionBlock)(BOOL canceled);
  
  // Drawing
  A_STextNode *_debugLabelNode;
...
- (instancetype)init
{
  ...
  self.contentsScale = A_SScreenScale();
  ...
  _animatedImageRunLoopMode = A_SAnimatedImageDefaultRunLoopMode;
...
{
  if (A_SObjectIsEqual(_image, image)) {
    return;
  }
  ...
  if (image != nil) {
    ...
    if ([A_SImageNode shouldShowImageScalingOverlay] && _debugLabelNode == nil) {
      A_SPerformBlockOnMainThread(^{
        _debugLabelNode = [[A_STextNode alloc] init];
  ...
  if (shouldReleaseImageOnBackgroundThread) {
    A_SPerformBackgroundDeallocation(&oldImage);
  }
}
...
- (NSObject *)drawParametersForAsyncLayer:(_A_SDisplayLayer *)layer
{
  ...
  A_SImageNodeDrawParameters *drawParameters = [[A_SImageNodeDrawParameters alloc] init];
  ...
  drawParameters->_didDrawBlock = ^(A_SWeakMapEntry *entry){
    A_SDN::MutexLocker l(__instanceLock__);
...
{
  A_SImageNodeDrawParameters *drawParameter = (A_SImageNodeDrawParameters *)parameter;
  ...
  A_SDisplayNodeContextModifier willDisplayNodeContentWithRenderingContext = drawParameter->_willDisplayNodeContentWithRenderingContext;
  A_SDisplayNodeContextModifier didDisplayNodeContentWithRenderingContext  = drawParameter->_didDisplayNodeContentWithRenderingContext;
  ...
  A_SDisplayNodeAssert(contentsScale > 0, @"invalid contentsScale at display time");
  ...
  if (!cropEnabled || !contentModeSupported || stretchable) {
    ...
    A_SCroppedImageBackingSizeAndDrawRectInBounds(imageSizeInPixels,
                                                 boundsSizeInPixels,
                                                 contentMode,
                                                 cropRect,
                                                 forceUpscaling,
  ...
  A_SImageNodeContentsKey *contentsKey = [[A_SImageNodeContentsKey alloc] init];
  ...
  A_SWeakMapEntry<UIImage *> *entry = [self.class contentsForkey:contentsKey
...
static A_SWeakMap<A_SImageNodeContentsKey *, UIImage *> *cache = nil;
// Allocate cacheLock on the heap to prevent destruction at app exit (https://github.com/Tex_tureGroup/Tex_ture/issues/136)
static A_SDN::StaticMutex& cacheLock = *new A_SDN::StaticMutex;

+ (A_SWeakMapEntry *)contentsForkey:(A_SImageNodeContentsKey *)key drawParameters:(id)drawParameters isCancelled:(asdisplaynode_iscancelled_block_t)isCancelled
{
  {
    A_SDN::StaticMutexLocker l(cacheLock);
    if (!cache) {
      cache = [[A_SWeakMap alloc] init];
    }
    A_SWeakMapEntry *entry = [cache entryForKey:key];
    if (entry != nil) {
      return entry;
    }
  }
...
+ (UIImage *)createContentsForkey:(A_SImageNodeContentsKey *)key drawParameters:(id)drawParameters isCancelled:(asdisplaynode_iscancelled_block_t)isCancelled
{
  ...
  // Use contentsScale of 1.0 and do the contentsScale handling in boundsSizeInPixels so A_SCroppedImageBackingSizeAndDrawRectInBounds
  ...
  // as well as iOS games, and a small number of A_SDK apps that provide the same image reference
  // to many separate A_SImageNodes.  A workaround is to set .displaysAsynchronously = NO for the nodes
  ...
  // Another option is to have A_SDisplayNode+AsyncDisplay coordinate these cases, and share the decoded buffer.
  // Details tracked in https://github.com/facebook/Async_DisplayKit/issues/1068
  
  UIImage *image = key.image;
  BOOL canUseCopy = (contextIsClean || A_SImageAlphaInfoIsOpaque(CGImageGetAlphaInfo(image.CGImage)));
...
- (void)layout
{
  ...
  if (_debugLabelNode) {
    CGSize boundsSize        = self.bounds.size;
    CGSize debugLabelSize    = [_debugLabelNode layoutThatFits:A_SSizeRangeMake(CGSizeZero, boundsSize)].size;
...
extern asimagenode_modification_block_t A_SImageNodeRoundBorderModificationBlock(CGFloat borderWidth, UIColor *borderColor)
{
...
extern asimagenode_modification_block_t A_SImageNodeTintColorModificationBlock(UIColor *color)
{
  return ^(UIImage *originalImage) {
    UIGraphicsBeginImageContextWithOptions(originalImage.size, NO, originalImage.scale);
