...
@protocol A_SVideoNodeDelegate;

typedef NS_ENUM(NSInteger, A_SVideoNodePlayerState) {
  A_SVideoNodePlayerStateUnknown,
  A_SVideoNodePlayerStateInitialLoading,
  A_SVideoNodePlayerStateReadyToPlay,
  A_SVideoNodePlayerStatePlaybackLikelyToKeepUpButNotPlaying,
  A_SVideoNodePlayerStatePlaying,
  A_SVideoNodePlayerStateLoading,
  A_SVideoNodePlayerStatePaused,
  A_SVideoNodePlayerStateFinished
};

NS_ASSUME_NONNULL_BEGIN

// IMPORTANT NOTES:
// 1. Applications using A_SVideoNode must link AVFoundation! (this provides the AV* classes below)
// 2. This is a relatively new component of Async_DisplayKit.  It has many useful features, but
//    there is room for further expansion and optimization.  Please report any issues or requests
//    in an issue on GitHub: https://github.com/facebook/Async_DisplayKit/issues

@interface A_SVideoNode : A_SNetworkImageNode
...
@property (nullable, nonatomic, weak, readwrite) id<A_SVideoNodeDelegate, A_SNetworkImageNodeDelegate> delegate;

@end

@protocol A_SVideoNodeDelegate <A_SNetworkImageNodeDelegate>
@optional
/**
 ...
 */
- (void)videoNode:(A_SVideoNode *)videoNode didFailToLoadValueForKey:(NSString *)key asset:(AVAsset *)asset error:(NSError *)error;

@end

@interface A_SVideoNode (Unavailable)
