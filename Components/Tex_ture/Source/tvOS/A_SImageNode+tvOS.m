...
#if TARGET_OS_TV
#import <Async_DisplayKit/A_SImageNode+tvOS.h>
...
@implementation A_SImageNode (tvOS)
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event
{
  ...
  if (!self.isDefaultFocusAppearance) {
    ...
    // or our superview if we are in a A_SCellNode
    UIView *view = [self getView];
...
- (UIView *)getView
{
  ...
  // If we are inside a A_SCellNode, then we need to apply our focus effects to the A_SCellNode view/layer rather than the A_SImageNode view/layer.
  return A_SDisplayNodeUltimateParentOfNode(self).view;
}
