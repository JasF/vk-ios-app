...
#if !A_STEXTNODE_EXPERIMENT_GLOBAL_ENABLE
...
#import <Async_DisplayKit/A_STextKitCoreTextAdditions.h>
 ...
 * testing.
 */
#define A_S_TEXTNODE_RECORD_ATTRIBUTED_STRINGS 0

static const NSTimeInterval A_STextNodeHighlightFadeOutDuration = 0.15;
static const NSTimeInterval A_STextNodeHighlightFadeInDuration = 0.1;
...
#pragma mark - A_STextKitRenderer

@interface A_STextNodeRendererKey : NSObject
@property (assign, nonatomic) A_STextKitAttributes attributes;
@property (assign, nonatomic) CGSize constrainedSize;
@end

@implementation A_STextNodeRendererKey
...
- (BOOL)isEqual:(A_STextNodeRendererKey *)object
{
  if (self == object) {
    return YES;
  }
...
static A_STextKitRenderer *rendererForAttributes(A_STextKitAttributes attributes, CGSize constrainedSize)
{
  NSCache *cache = sharedRendererCache();
  
  A_STextNodeRendererKey *key = [[A_STextNodeRendererKey alloc] init];
  key.attributes = attributes;
  key.constrainedSize = constrainedSize;

  A_STextKitRenderer *renderer = [cache objectForKey:key];
  if (renderer == nil) {
    renderer = [[A_STextKitRenderer alloc] initWithTextKitAttributes:attributes constrainedSize:constrainedSize];
...
#pragma mark - A_STextNodeDrawParameter

@interface A_STextNodeDrawParameter : NSObject {
@package
  A_STextKitAttributes _rendererAttributes;
  UIColor *_backgroundColor;
  UIEdgeInsets _textContainerInsets;
}
@end

@implementation A_STextNodeDrawParameter

- (instancetype)initWithRendererAttributes:(A_STextKitAttributes)rendererAttributes
...
- (A_STextKitRenderer *)rendererForBounds:(CGRect)bounds
{
...
@interface A_STextNode () <UIGestureRecognizerDelegate>

@end

@implementation A_STextNode {
...
- (NSMutableArray<NSDictionary *> *)propertiesForDescription
{
  ...
  if (plainString.length > 0) {
    [result addObject:@{ (id)kCFNull : A_SStringWithQuotesIfMultiword(plainString) }];
  }
  return result;
}
...
- (A_STextKitAttributes)_locked_rendererAttributes
{
  return {
...
- (CGSize)calculateSizeThatFits:(CGSize)constrainedSize
{
  ...
  A_STextKitRenderer *renderer = [self _locked_rendererWithBounds:{.size = constrainedSize}];
...
- (void)setAttributedText:(NSAttributedString *)attributedText
{
  ...
  // Don't hold textLock for too long.
  {
    ...
    _attributedText = A_SCleanseAttributedStringOfCoreTextAttributes(attributedText);
#if A_S_TEXTNODE_RECORD_ATTRIBUTED_STRINGS
...
{
  {
    ...
    if (A_SObjectIsEqual(exclusionPaths, _exclusionPaths)) {
      return;
    }
...
- (NSObject *)drawParametersForAsyncLayer:(_A_SDisplayLayer *)layer
{
  ...
  return [[A_STextNodeDrawParameter alloc] initWithRendererAttributes:[self _locked_rendererAttributes]
...
{
  A_STextNodeDrawParameter *drawParameter = (A_STextNodeDrawParameter *)parameters;
  ...
  A_STextKitRenderer *renderer = [drawParameter rendererForBounds:bounds];
                 ...
                 forHighlighting:(BOOL)highlighting
{
  ...
  A_STextKitRenderer *renderer = [self _locked_renderer];
  ...
  if (!NSEqualRanges(highlightRange, _highlightRange) && ((0 != highlightRange.length) || (0 != _highlightRange.length))) {
    ...
    if (_activeHighlightLayer) {
      if (animated) {
        ...
        fadeOut.duration = A_STextNodeHighlightFadeOutDuration;
    ...
    if (0 != highlightRange.length) {
      ...
      if (highlightTargetLayer != nil) {
        A_SDN::MutexLocker l(__instanceLock__);
        A_STextKitRenderer *renderer = [self _renderer];

        NSArray *highlightRects = [renderer rectsForTextRange:highlightRange measureOption:A_STextKitRendererMeasureOptionBlock];
        ...
        for (NSValue *rectValue in highlightRects) {
          ...
          CGRect rendererRect = A_STextNodeAdjustRenderRectForShadowPadding(rectValue.CGRectValue, shadowPadding);
        ...
        A_SHighlightOverlayLayer *overlayLayer = [[A_SHighlightOverlayLayer alloc] initWithRects:converted];
        ...
        if (animated) {
          ...
          fadeIn.duration = A_STextNodeHighlightFadeInDuration;
...
static CGRect A_STextNodeAdjustRenderRectForShadowPadding(CGRect rendererRect, UIEdgeInsets shadowPadding) {
...
{
  return [self _rectsForTextRange:textRange measureOption:A_STextKitRendererMeasureOptionCapHeight];
}

- (NSArray *)highlightRectsForTextRange:(NSRange)textRange
{
  return [self _rectsForTextRange:textRange measureOption:A_STextKitRendererMeasureOptionBlock];
}

- (NSArray *)_rectsForTextRange:(NSRange)textRange measureOption:(A_STextKitRendererMeasureOption)measureOption
{
  A_SDN::MutexLocker l(__instanceLock__);
  ...
  for (NSValue *rectValue in rects) {
    ...
    rect = A_STextNodeAdjustRenderRectForShadowPadding(rect, self.shadowPadding);
...
- (CGRect)trailingRect
{
  ...
  return A_STextNodeAdjustRenderRectForShadowPadding(rect, self.shadowPadding);
}
...
- (CGRect)frameForTextRange:(NSRange)textRange
{
  ...
  return A_STextNodeAdjustRenderRectForShadowPadding(frame, self.shadowPadding);
}
...
- (UIImage *)placeholderImage
{
  ...
  NSArray *lineRects = [self _rectsForTextRange:visibleRange measureOption:A_STextKitRendererMeasureOptionLineHeight];
...
{
  truncationString = A_SCleanseAttributedStringOfCoreTextAttributes(truncationString);
...
// Allocate _experimentLock on the heap to prevent destruction at app exit (https://github.com/Tex_tureGroup/Tex_ture/issues/136)
static A_SDN::StaticMutex& _experimentLock = *new A_SDN::StaticMutex;
static A_STextNodeExperimentOptions _experimentOptions;
...
+ (void)setExperimentOptions:(A_STextNodeExperimentOptions)options
{
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    A_SDN::StaticMutexLocker lock(_experimentLock);
    
    // They must call this before allocating any text nodes.
    A_SDisplayNodeAssertFalse(_hasAllocatedNode);
    
    _experimentOptions = options;
    
    // Set superclass of all subclasses to A_STextNode2
    if (options & A_STextNodeExperimentSubclasses) {
      unsigned int classCount;
      Class originalClass = [A_STextNode class];
      Class newClass = [A_STextNode2 class];
...
#pragma clang diagnostic pop
    ...
    if (options & A_STextNodeExperimentDebugging) {
      [A_STextNode2 enableDebugging];
    }
  });
}
...
+ (id)allocWithZone:(struct _NSZone *)zone
{
  ...
  BOOL useExperiment = (_experimentOptions & A_STextNodeExperimentAllInstances)
		|| ((_experimentOptions & A_STextNodeExperimentRandomInstances)
        && (arc4random_uniform(2) != 0));
  
  if (useExperiment) {
    return (A_STextNode *)[A_STextNode2 allocWithZone:zone];
  } else {
...
@implementation A_STextNode (Deprecated)
