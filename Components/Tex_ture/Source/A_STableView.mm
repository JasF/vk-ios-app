...
static NSString * const kCellReuseIdentifier = @"_A_STableViewCell";

//#define LOG(...) NSLog(__VA_ARGS__)
#define LOG(...)

/**
 * See note at the top of A_SCollectionView.mm near declaration of macro GET_COLLECTIONNODE_OR_RETURN
 */
#define GET_TABLENODE_OR_RETURN(__var, __val) \
  A_STableNode *__var = self.tableNode; \
  if (__var == nil) { \
    return __val; \
  }
...
#pragma mark -
#pragma mark A_SCellNode<->UITableViewCell bridging.

@class _A_STableViewCell;

@protocol _A_STableViewCellDelegate <NSObject>
- (void)didLayoutSubviewsOfTableViewCell:(_A_STableViewCell *)tableViewCell;
@end

@interface _A_STableViewCell : UITableViewCell
@property (nonatomic, weak) id<_A_STableViewCellDelegate> delegate;
@property (nonatomic, strong, readonly) A_SCellNode *node;
@property (nonatomic, strong) A_SCollectionElement *element;
@end

@implementation _A_STableViewCell
...
- (void)setElement:(A_SCollectionElement *)element
{
  ...
  if (node) {
    ...
    // overlaps other cells if size of A_SCellNode has changed.)
...
@interface A_STableView () <A_SRangeControllerDataSource, A_SRangeControllerDelegate, A_SDataControllerSource, _A_STableViewCellDelegate, A_SCellNodeInteractionDelegate, A_SDelegateProxyInterceptor, A_SBatchFetchingScrollView>
{
  A_STableViewProxy *_proxyDataSource;
  A_STableViewProxy *_proxyDelegate;

  A_STableLayoutController *_layoutController;
  ...
  __weak A_SCellNode *_contentOffsetAdjustmentTopVisibleNode;
  ...
  NSHashTable<_A_STableViewCell *> *_cellsForVisibilityUpdates;
  ...
  // See documentation on same property in A_SCollectionView
   ...
   */
  _A_SHierarchyChangeSet *_changeSet;
...
@property (nonatomic, strong, readwrite) A_SDataController *dataController;

@property (nonatomic, weak)   A_STableNode *tableNode;

@property (nonatomic) BOOL test_enableSuperUpdateCallLogging;
@end

@implementation A_STableView
{
  __weak id<A_STableDelegate> _asyncDelegate;
  __weak id<A_STableDataSource> _asyncDataSource;
}

// Using _A_SDisplayLayer ensures things like -layout are properly forwarded to A_STableNode.
+ (Class)layerClass
{
...
{
  return [A_SDataController class];
}
...
- (instancetype)_initWithFrame:(CGRect)frame style:(UITableViewStyle)style dataControllerClass:(Class)dataControllerClass owningNode:(A_STableNode *)tableNode eventLog:(A_SEventLog *)eventLog
{
  ...
  _layoutController = [[A_STableLayoutController alloc] initWithTableView:self];
  ...
  _proxyDelegate = [[A_STableViewProxy alloc] initWithTarget:nil interceptor:self];
  super.delegate = (id<UITableViewDelegate>)_proxyDelegate;
  
  _proxyDataSource = [[A_STableViewProxy alloc] initWithTarget:nil interceptor:self];
  super.dataSource = (id<UITableViewDataSource>)_proxyDataSource;
  
  [self registerClass:_A_STableViewCell.class forCellReuseIdentifier:kCellReuseIdentifier];
...
{
  NSLog(@"Warning: Async_DisplayKit is not designed to be used with Interface Builder.  Table properties set in IB will be lost.");
...
- (void)dealloc
{
  ...
  A_SDisplayNodeCAssert(_batchUpdateCount == 0, @"A_STableView deallocated in the middle of a batch update.");
...
- (void)setDataSource:(id<UITableViewDataSource>)dataSource
{
  ...
  A_SDisplayNodeAssert(dataSource == nil, @"A_STableView uses asyncDataSource, not UITableView's dataSource property.");
}

- (void)setDelegate:(id<UITableViewDelegate>)delegate
{
  // Our UIScrollView superclass sets its delegate to nil on dealloc. Only assert if we get a non-nil value here.
  A_SDisplayNodeAssert(delegate == nil, @"A_STableView uses asyncDelegate, not UITableView's delegate property.");
}

- (id<A_STableDataSource>)asyncDataSource
{
  return _asyncDataSource;
}

- (void)setAsyncDataSource:(id<A_STableDataSource>)asyncDataSource
{
  ...
  // reference to the old dataSource in this case because calls to A_STableViewProxy will start failing and cause crashes.
  NS_VALID_UNTIL_END_OF_SCOPE id oldDataSource = self.dataSource;
  
  if (asyncDataSource == nil) {
    _asyncDataSource = nil;
    _proxyDataSource = _isDeallocating ? nil : [[A_STableViewProxy alloc] initWithTarget:nil interceptor:self];
    
    memset(&_asyncDataSourceFlags, 0, sizeof(_asyncDataSourceFlags));
  } else {
    _asyncDataSource = asyncDataSource;
    _proxyDataSource = [[A_STableViewProxy alloc] initWithTarget:_asyncDataSource interceptor:self];
    ...
    A_SDisplayNodeAssert(_asyncDataSourceFlags.tableViewNodeBlockForRow
    ...
    A_SDisplayNodeAssert(_asyncDataSourceFlags.tableNodeNumberOfRowsInSection || _asyncDataSourceFlags.tableViewNumberOfRowsInSection, @"Data source must implement tableNode:numberOfRowsInSection:");
  }
...
- (id<A_STableDelegate>)asyncDelegate
{
  return _asyncDelegate;
}

- (void)setAsyncDelegate:(id<A_STableDelegate>)asyncDelegate
{
  ...
  // will return as nil (ARC magic) even though the _proxyDataSource still exists. It's really important to hold a strong
  // reference to the old delegate in this case because calls to A_STableViewProxy will start failing and cause crashes.
  NS_VALID_UNTIL_END_OF_SCOPE id oldDelegate = super.delegate;
  
  if (asyncDelegate == nil) {
    _asyncDelegate = nil;
    _proxyDelegate = _isDeallocating ? nil : [[A_STableViewProxy alloc] initWithTarget:nil interceptor:self];
    
    memset(&_asyncDelegateFlags, 0, sizeof(_asyncDelegateFlags));
  } else {
    _asyncDelegate = asyncDelegate;
    _proxyDelegate = [[A_STableViewProxy alloc] initWithTarget:_asyncDelegate interceptor:self];
...
- (nullable NSIndexPath *)validateIndexPath:(nullable NSIndexPath *)indexPath
{
  ...
  if (section >= self.numberOfSections) {
    A_SDisplayNodeFailAssert(@"Table view index path has invalid section %lu, section count = %lu", (unsigned long)section, (unsigned long)self.numberOfSections);
    return nil;
  }
  ...
  if (item != NSNotFound && item >= [self numberOfRowsInSection:section]) {
    A_SDisplayNodeFailAssert(@"Table view index path has invalid item %lu in section %lu, item count = %lu", (unsigned long)indexPath.item, (unsigned long)section, (unsigned long)[self numberOfRowsInSection:section]);
    return nil;
  }
...
- (nullable NSIndexPath *)indexPathForNode:(A_SCellNode *)cellNode waitingIfNeeded:(BOOL)wait
{
  if (cellNode == nil) {
    return nil;
  }
...
- (NSArray<A_SCellNode *> *)visibleNodes
{
  auto elements = [self visibleElementsForRangeController:_rangeController];
  return A_SArrayByFlatMapping(elements, A_SCollectionElement *e, e.node);
}
...
- (void)layoutSubviews
{
  ...
  if (constrainedWidth > 0 && _nodesConstrainedWidth != constrainedWidth) {
    ...
    [self beginUpdates];
    [_dataController relayoutAllNodesWithInvalidationBlock:nil];
    [self endUpdatesAnimated:(A_SDisplayNodeLayerHasAnimations(self.layer) == NO) completion:nil];
  } else {
...
- (void)beginAdjustingContentOffset
{
  ...
  if (firstVisibleIndexPath) {
    A_SCellNode *node = [self nodeForRowAtIndexPath:firstVisibleIndexPath];
    if (node) {
...
- (void)setTableHeaderView:(UIView *)tableHeaderView
{
  ...
  [self.tableHeaderView.asyncdisplaykit_node exitHierarchyState:A_SHierarchyStateRangeManaged];
  [super setTableHeaderView:tableHeaderView];
  [self.tableHeaderView.asyncdisplaykit_node enterHierarchyState:A_SHierarchyStateRangeManaged];
}

- (void)setTableFooterView:(UIView *)tableFooterView
{
  // Typically the view will be nil before setting it, but reset state if it is being re-hosted.
  [self.tableFooterView.asyncdisplaykit_node exitHierarchyState:A_SHierarchyStateRangeManaged];
  [super setTableFooterView:tableFooterView];
  [self.tableFooterView.asyncdisplaykit_node enterHierarchyState:A_SHierarchyStateRangeManaged];
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  _A_STableViewCell *cell = [self dequeueReusableCellWithIdentifier:kCellReuseIdentifier forIndexPath:indexPath];
  cell.delegate = self;
...
{
  A_SCellNode *node = [_dataController.visibleMap elementForItemAtIndexPath:indexPath].node;
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(_A_STableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
...
#pragma clang diagnostic pop
  ...
  if (A_SSubclassOverridesSelector([A_SCellNode class], [cellNode class], @selector(cellNodeVisibilityEvent:inScrollView:withCellFrame:))) {
    [_cellsForVisibilityUpdates addObject:cell];
  }
}

- (void)tableView:(UITableView *)tableView didEndDisplayingCell:(_A_STableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath
{
  A_SCollectionElement *element = cell.element;
  ...
  A_SDisplayNodeAssertNotNil(cellNode, @"Expected node associated with removed cell not to be nil.");
  if (_asyncDelegateFlags.tableNodeDidEndDisplayingNodeForRow) {
    if (A_STableNode *tableNode = self.tableNode) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
  ...
  for (_A_STableViewCell *tableCell in _cellsForVisibilityUpdates) {
    [[tableCell node] cellNodeVisibilityEvent:A_SCellNodeVisibilityEventVisibleRectChanged
...
- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView
{
  ...
  for (_A_STableViewCell *tableViewCell in _cellsForVisibilityUpdates) {
    [[tableViewCell node] cellNodeVisibilityEvent:A_SCellNodeVisibilityEventWillBeginDragging
                                          inScrollView:scrollView
...
- (void)scrollViewDidEndDragging:(UIScrollView *)scrollView willDecelerate:(BOOL)decelerate
{
  ...
  for (_A_STableViewCell *tableViewCell in _cellsForVisibilityUpdates) {
    [[tableViewCell node] cellNodeVisibilityEvent:A_SCellNodeVisibilityEventDidEndDragging
                                          inScrollView:scrollView
                                         withCellFrame:tableViewCell.frame];
  }
...
{
  A_SScrollDirection scrollableDirection = A_SScrollDirectionNone;
  ...
  if (self.alwaysBounceHorizontal || totalContentWidth > self.bounds.size.width) { // Can scroll horizontally.
    scrollableDirection |= A_SScrollDirectionHorizontalDirections;
  }
  if (self.alwaysBounceVertical || totalContentHeight > self.bounds.size.height) { // Can scroll vertically.
    scrollableDirection |= A_SScrollDirectionVerticalDirections;
  }
...
{
  if (A_SDisplayShouldFetchBatchForScrollView(self, self.scrollDirection, A_SScrollDirectionVerticalDirections, contentOffset, velocity)) {
...
{
  return A_SInterfaceStateForDisplayNode(self.tableNode, self.window);
}
...
- (void)rangeController:(A_SRangeController *)rangeController updateWithChangeSet:(_A_SHierarchyChangeSet *)changeSet updates:(dispatch_block_t)updates
{
  ...
  for (_A_SHierarchyItemChange *change in [changeSet itemChangesOfType:_A_SHierarchyChangeTypeReload]) {
    ...
    A_SPerformBlockWithoutAnimation(preventAnimation, ^{
      if (self.test_enableSuperUpdateCallLogging) {
...
- (A_SCellNodeBlock)dataController:(A_SDataController *)dataController nodeBlockAtIndexPath:(NSIndexPath *)indexPath {
  ...
  if (_asyncDataSourceFlags.tableNodeNodeBlockForRow) {
    ...
    if ([node isKindOfClass:[A_SCellNode class]]) {
      block = ^{
        return node;
      };
    } else {
      A_SDisplayNodeFailAssert(@"Data source returned invalid node from tableNode:nodeForRowAtIndexPath:. Node: %@", node);
    }
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    A_SCellNode *node = [_asyncDataSource tableView:self nodeForRowAtIndexPath:indexPath];
#pragma clang diagnostic pop
    if ([node isKindOfClass:[A_SCellNode class]]) {
      block = ^{
        return node;
      ...
      A_SDisplayNodeFailAssert(@"Data source returned invalid node from tableView:nodeForRowAtIndexPath:. Node: %@", node);
    }
  }

  // Handle nil node block
  if (block == nil) {
    A_SDisplayNodeFailAssert(@"A_STableNode could not get a node block for row at index path %@", indexPath);
    block = ^{
...
{
  A_SSizeRange constrainedSize = A_SSizeRangeZero;
  if (_asyncDelegateFlags.tableNodeConstrainedSizeForRow) {
    GET_TABLENODE_OR_RETURN(tableNode, constrainedSize);
    A_SSizeRange delegateConstrainedSize = [_asyncDelegate tableNode:tableNode constrainedSizeForRowAtIndexPath:indexPath];
    // ignore widths in the returned size range (for TableView)
    constrainedSize = A_SSizeRangeMake(CGSizeMake(_nodesConstrainedWidth, delegateConstrainedSize.min.height),
...
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    A_SSizeRange delegateConstrainedSize = [_asyncDelegate tableView:self constrainedSizeForRowAtIndexPath:indexPath];
#pragma clang diagnostic pop
    // ignore widths in the returned size range (for TableView)
    constrainedSize = A_SSizeRangeMake(CGSizeMake(_nodesConstrainedWidth, delegateConstrainedSize.min.height),
                                      CGSizeMake(_nodesConstrainedWidth, delegateConstrainedSize.max.height));
  } else {
    constrainedSize = A_SSizeRangeMake(CGSizeMake(_nodesConstrainedWidth, 0),
...
- (BOOL)dataController:(A_SDataController *)dataController presentedSizeForElement:(A_SCollectionElement *)element matchesSize:(CGSize)size
{
  ...
  if (self.separatorStyle != UITableViewCellSeparatorStyleNone) {
    rect.size.height -= 1.0 / A_SScreenScale();
  }

  return (fabs(rect.size.height - size.height) < FLT_EPSILON);
}

#pragma mark - _A_STableViewCellDelegate

- (void)didLayoutSubviewsOfTableViewCell:(_A_STableViewCell *)tableViewCell
{
  A_SCellNode *node = tableViewCell.node;
  if (node == nil || _asyncDataSource == nil) {
    return;
  }
  
  CGFloat contentViewWidth = tableViewCell.contentView.bounds.size.width;
  A_SSizeRange constrainedSize = node.constrainedSizeForCalculatedLayout;
  ...
  if (CGSizeEqualToSize(node.calculatedSize, CGSizeZero) == NO && contentViewWidth != constrainedSize.max.width) {
    ...
    // is the same for all cells (because there is no easy way to get that individual value before the node being assigned to a _A_STableViewCell).
    ...
    // To avoid premature optimization and making such assumption, as well as to keep A_STableView simple, re-measurement is strictly done on main.
...
- (void)willMoveToWindow:(UIWindow *)newWindow
{
  ...
  A_SDisplayNode *node = self.tableNode;
