//
//  IGListAdapter+Async_DisplayKit.m
//  Tex_ture
//
...
#import "IGListAdapter+Async_DisplayKit.h"
#import "A_SIGListAdapterBasedDataSource.h"
#import "A_SAssert.h"
#import <objc/runtime.h>

@implementation IGListAdapter (Async_DisplayKit)
...
- (void)setA_SDKCollectionNode:(A_SCollectionNode *)collectionNode
{
  ...
  A_SIGListAdapterBasedDataSource *dataSource = objc_getAssociatedObject(self, _cmd);
  // Bomb if we already made one.
  if (dataSource != nil) {
    A_SDisplayNodeFailAssert(@"Attempt to call %@ multiple times on the same list adapter. Not currently allowed!", NSStringFromSelector(_cmd));
    return;
  }

  // Make a data source and retain it.
  dataSource = [[A_SIGListAdapterBasedDataSource alloc] initWithListAdapter:self];
  ...
  [collectionNode onDidLoad:^(__kindof A_SCollectionNode * _Nonnull collectionNode) {
#if IG_LIST_COLLECTION_VIEW
    // We manually set the superclass of A_SCollectionView to IGListCollectionView at runtime if needed.
