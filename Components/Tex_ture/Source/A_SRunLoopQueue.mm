...
#define A_SRunLoopQueueLoggingEnabled 0
#define A_SRunLoopQueueVerboseLoggingEnabled 0

static void runLoopSourceCallback(void *info) {
  // No-op
#if A_SRunLoopQueueVerboseLoggingEnabled
  NSLog(@"<%@> - Called runLoopSourceCallback", info);
#endif
}

#pragma mark - A_SDeallocQueue

@implementation A_SDeallocQueue {
  NSThread *_thread;
  NSCondition *_condition;
  std::deque<id> _queue;
  A_SDN::RecursiveMutex _queueLock;
}

+ (A_SDeallocQueue *)sharedDeallocationQueue
{
  static A_SDeallocQueue *deallocQueue = nil;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    deallocQueue = [[A_SDeallocQueue alloc] init];
  });
  return deallocQueue;
}
...
- (void)releaseObjectInBackground:(id  _Nullable __strong *)objectPtr
{
  ...
  if (objectPtr != NULL && *objectPtr != nil) {
    A_SDN::MutexLocker l(_queueLock);
...
- (void)threadMain
{
  @autoreleasepool {
    ...
    CFRunLoopTimerRef timer = CFRunLoopTimerCreateWithHandler(NULL, -1, 0.1, 0, 0, ^(CFRunLoopTimerRef timer) {
      ...
      NSInteger count;
      @autoreleasepool {
#if A_SRunLoopQueueLoggingEnabled
        NSLog(@"A_SDeallocQueue Processing: %lu objects destroyed", weakSelf->_queue.size());
#endif
        ...
        A_SSignpostStartCustom(A_SSignpostDeallocQueueDrain, self, count);
        weakSelf->_queue = std::deque<id>();
        weakSelf->_queueLock.unlock();
        currentQueue.clear();
      }
      A_SSignpostEndCustom(A_SSignpostDeallocQueueDrain, self, count, A_SSignpostColorDefault);
    });
...
@implementation A_SRunLoopQueue
...
+ (void)registerCATransactionObservers
{
  ...
  dispatch_once(&onceToken, ^{
    ...
    preLayoutHandler = ^{
      A_SSignpostStartCustom(A_SSignpostCATransactionLayout, 0, [CATransaction currentState]);
    };
    preCommitHandler = ^{
      int state = [CATransaction currentState];
      A_SSignpostEndCustom(A_SSignpostCATransactionLayout, 0, state, A_SSignpostColorDefault);
      A_SSignpostStartCustom(A_SSignpostCATransactionCommit, 0, state);
    };
    postCommitHandler = ^{
      A_SSignpostEndCustom(A_SSignpostCATransactionCommit, 0, [CATransaction currentState], A_SSignpostColorDefault);
...
- (instancetype)initWithRunLoop:(CFRunLoopRef)runloop retainObjects:(BOOL)retainsObjects handler:(void (^)(id _Nullable, BOOL))handlerBlock
{
  if (self = [super init]) {
    ...
    _rootActivity = as_activity_create("Process run loop queue items", OS_ACTIVITY_NONE, OS_ACTIVITY_FLAG_DEFAULT);
    {
      ...
      as_log_verbose(A_SDisplayLog(), "Created run loop queue: %@", self);
    }
...
- (void)processQueue
{
  ...
  {
    A_SDN::MutexLocker l(_internalQueueLock);
    ...
    A_SSignpostStart(A_SSignpostRunLoopQueueBatch);
  ...
  if (count > 0) {
    ...
    for (auto iterator = itemsToProcess.begin(); iterator < itemsEnd; iterator++) {
      ...
      as_log_verbose(A_SDisplayLog(), "processed %@", value);
    }
    if (count > 1) {
      as_log_verbose(A_SDisplayLog(), "processed %lu items", (unsigned long)count);
    }
  }
  ...
  A_SSignpostEnd(A_SSignpostRunLoopQueueBatch);
}
