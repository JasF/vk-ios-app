...
static inline BOOL A_SDisplayNodeThreadIsMain()
{
 ...
 * 
 * This flag also enable locking assertions (e.g A_SDisplayNodeAssertLockUnownedByCurrentThread(node)).
...
#define A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(x_) do { \
  _Pragma("clang diagnostic push"); \
  _Pragma("clang diagnostic ignored \"-Wunused-variable\""); \
  volatile int res = (x_); \
  A_SDisplayNodeCAssert(res == 0, @"Expected %@ to return 0, got %d instead", @#x_, res); \
...
#define A_SDisplayNodeAssertLockUnownedByCurrentThread(lock) A_SDisplayNodeAssertFalse(lock.ownedByCurrentThread())
#else
#define A_SDisplayNodeAssertLockUnownedByCurrentThread(lock)
#endif
...
namespace A_SDN {
  ...
  public:
    ...
    Locker (T &l) A_SDISPLAYNODE_NOTHROW : _l (l) {
      _l.lock ();
    }
    ...
    Locker (T &l, const char *name = NULL) A_SDISPLAYNODE_NOTHROW : _l (l), _name(name) {
  ...
  public:
    ...
    SharedLocker (std::shared_ptr<T> const& l) A_SDISPLAYNODE_NOTHROW : _l (l) {
      A_SDisplayNodeCAssertTrue(_l != nullptr);
      _l->lock ();
    }
    ...
    SharedLocker (std::shared_ptr<T> const& l, const char *name = NULL) A_SDISPLAYNODE_NOTHROW : _l (l), _name(name) {
      _ti = CACurrentMediaTime();
      _l->lock ();
    }
  ...
  class Unlocker
  {
    T &_l;
  public:
    Unlocker (T &l) A_SDISPLAYNODE_NOTHROW : _l (l) { _l.unlock (); }
    ~Unlocker () {_l.lock ();}
  ...
  class SharedUnlocker
  {
    std::shared_ptr<T> _l;
  public:
    SharedUnlocker (std::shared_ptr<T> const& l) A_SDISPLAYNODE_NOTHROW : _l (l) { _l->unlock (); }
  ...
  struct Mutex
  {
    ...
    ~Mutex () {
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_mutex_destroy (&_m));
#if CHECK_LOCKING_SAFETY
      _owner = 0;
      _count = 0;
#endif
    ...
    void lock () {
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_mutex_lock (this->mutex()));
#if CHECK_LOCKING_SAFETY
      ...
      if (thread_id != _owner) {
        ...
        A_SDisplayNodeCAssertTrue(0 == _owner);
        A_SDisplayNodeCAssertTrue(0 == _count);
        _owner = thread_id;
      } else {
        // Existing owner tries to reacquire this (recursive) mutex. _count must already be positive.
        A_SDisplayNodeCAssertTrue(_count > 0);
      }
      ++_count;
#endif
    }
    ...
    void unlock () {
      ...
      A_SDisplayNodeCAssertTrue(thread_id == _owner);
...
#endif
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_mutex_unlock (this->mutex()));
    }
  ...
  protected:
    explicit Mutex (bool recursive) {
      if (!recursive) {
        A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_mutex_init (&_m, NULL));
      } else {
        pthread_mutexattr_t attr;
        A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_mutexattr_init (&attr));
        A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_mutexattr_settype (&attr, PTHREAD_MUTEX_RECURSIVE));
        A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_mutex_init (&_m, &attr));
        A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_mutexattr_destroy (&attr));
      }
#if CHECK_LOCKING_SAFETY
      _owner = 0;
      _count = 0;
  ...
  {
    Condition () {
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_cond_init(&_c, NULL));
    }

    ~Condition () {
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_cond_destroy(&_c));
    }
    ...
    void signal() {
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_cond_signal(&_c));
    }

    void wait(Mutex &m) {
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_cond_wait(&_c, m.mutex()));
    }
  ...
  {
    ReadWriteLock() {
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_rwlock_init(&_rwlock, NULL));
    }

    ~ReadWriteLock() {
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_rwlock_destroy(&_rwlock));
    }
    ...
    void readlock() {
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_rwlock_rdlock(&_rwlock));
    }

    void writelock() {
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_rwlock_wrlock(&_rwlock));
    }

    void unlock() {
      A_SDISPLAYNODE_THREAD_A_SSERT_ON_ERROR(pthread_rwlock_unlock(&_rwlock));
    }
  ...
  class ReadWriteLockReadLocker
  {
    ReadWriteLock &_lock;
  public:
    ReadWriteLockReadLocker(ReadWriteLock &lock) A_SDISPLAYNODE_NOTHROW : _lock(lock) {
      _lock.readlock();
    }
  ...
  class ReadWriteLockWriteLocker
  {
    ReadWriteLock &_lock;
  public:
    ReadWriteLockWriteLocker(ReadWriteLock &lock) A_SDISPLAYNODE_NOTHROW : _lock(lock) {
      _lock.writelock();
    }
