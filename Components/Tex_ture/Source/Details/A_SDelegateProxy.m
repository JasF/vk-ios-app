...
@implementation A_STableViewProxy

- (BOOL)interceptsSelector:(SEL)selector
{
  return (
          // handled by A_STableView node<->cell machinery
          ...
          // used for A_SCellNode visibility
          selector == @selector(scrollViewDidScroll:) ||

          // used for A_SCellNode user interaction
          selector == @selector(scrollViewWillBeginDragging:) ||
          selector == @selector(scrollViewDidEndDragging:willDecelerate:) ||
          
          // used for A_SRangeController visibility updates
...
@implementation A_SCollectionViewProxy

- (BOOL)interceptsSelector:(SEL)selector
{
  return (
          // handled by A_SCollectionView node<->cell machinery
...
@implementation A_SPagerNodeProxy

- (BOOL)interceptsSelector:(SEL)selector
{
  return (
          // handled by A_SPagerDataSource node<->cell machinery
...
@implementation A_SDelegateProxy {
  id <A_SDelegateProxyInterceptor> __weak _interceptor;
...
- (instancetype)initWithTarget:(id <NSObject>)target interceptor:(id <A_SDelegateProxyInterceptor>)interceptor
{
  ...
  A_SDisplayNodeAssert(interceptor, @"interceptor must not be nil");
...
- (id)forwardingTargetForSelector:(SEL)aSelector
{
  if ([self interceptsSelector:aSelector]) {
    ...
    if (_target) {
      ...
      id <A_SDelegateProxyInterceptor> interceptor = _interceptor;
      _interceptor = nil;
...
- (BOOL)interceptsSelector:(SEL)selector
{
  A_SDisplayNodeAssert(NO, @"This method must be overridden by subclasses.");
  return NO;
}
