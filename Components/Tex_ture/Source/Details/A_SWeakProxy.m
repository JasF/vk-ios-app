...
{
  return [[A_SWeakProxy alloc] initWithTarget:target];
}
...
{
  return A_SObjectDescriptionMake(self, @[@{ @"target": _target ?: (id)kCFNull }]);
}

- (NSMethodSignature *)methodSignatureForSelector:(SEL)sel
{
  A_SDisplayNodeAssertNil(_target, @"A_SWeakProxy got %@ when its target is still alive, which is unexpected.", NSStringFromSelector(_cmd));
