...
@interface A_SMainSerialQueue ()
{
  A_SDN::Mutex _serialQueueLock;
  NSMutableArray *_blocks;
}

@end

@implementation A_SMainSerialQueue
...
{
  A_SDN::MutexLocker l(_serialQueueLock);
...
- (void)performBlockOnMainThread:(dispatch_block_t)block
{
  ...
  {
    A_SDN::MutexUnlocker u(_serialQueueLock);
    [self runBlocks];
  }
}
...
- (void)runBlocks
{
  ...
  A_SPerformBlockOnMainThread(mainThread);
}
