...
@property (nonatomic, strong, readwrite) A_SDisplayNode *keepalive_node;
@end

@implementation _A_SDisplayView
{
  BOOL _inHitTest;
  BOOL _inPointInside;
...
- (NSString *)description
{
  ...
  A_SDisplayNode *node = _asyncdisplaykit_node;
...
{
  A_SDisplayNode *node = _asyncdisplaykit_node; // Create strong reference to weak ivar.
...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
  ...
  if (newSuperview) {
    A_SDisplayNode *supernode = node.supernode;
    BOOL supernodeLoaded = supernode.nodeLoaded;
    A_SDisplayNodeAssert(!supernode.isLayerBacked, @"Shouldn't be possible for _A_SDisplayView's supernode to be layer-backed.");
...
- (void)didMoveToSuperview
{
  ...
  A_SDisplayNode *supernode = node.supernode;
  A_SDisplayNodeAssert(!supernode.isLayerBacked, @"Shouldn't be possible for superview's node to be layer-backed.");
  
  if (supernode) {
    A_SDisplayNodeAssertTrue(node.nodeLoaded);
    BOOL supernodeLoaded = supernode.nodeLoaded;
    ...
    if (superview) {
      ...
      BOOL nodeIsRasterized = ((node.hierarchyState & A_SHierarchyStateRasterized) == A_SHierarchyStateRasterized);
...
{
  A_SDisplayNode *node = _asyncdisplaykit_node; // Create strong reference to weak ivar.
  return node ? [node layoutThatFits:A_SSizeRangeMake(size)].size : [super sizeThatFits:size];
}
...
{
  return A_SDisplayNodeUIContentModeFromCAContentsGravity(self.layer.contentsGravity);
}

- (void)setContentMode:(UIViewContentMode)contentMode
{
  A_SDisplayNodeAssert(contentMode != UIViewContentModeRedraw, @"Don't do this. Use needsDisplayOnBoundsChange instead.");
  ...
  self.layer.contentsGravity = (contentMode != UIViewContentModeRedraw) ? A_SDisplayNodeCAContentsGravityFromUIContentMode(contentMode) : kCAGravityResize;
}
...
{
  A_SDisplayNode *node = _asyncdisplaykit_node; // Create strong reference to weak ivar.
  if (node.methodOverrides & A_SDisplayNodeMethodOverrideTouchesBegan) {
...
{
  A_SDisplayNode *node = _asyncdisplaykit_node; // Create strong reference to weak ivar.
  if (node.methodOverrides & A_SDisplayNodeMethodOverrideTouchesMoved) {
...
{
  A_SDisplayNode *node = _asyncdisplaykit_node; // Create strong reference to weak ivar.
  if (node.methodOverrides & A_SDisplayNodeMethodOverrideTouchesEnded) {
...
{
  A_SDisplayNode *node = _asyncdisplaykit_node; // Create strong reference to weak ivar.
  if (node.methodOverrides & A_SDisplayNodeMethodOverrideTouchesCancelled) {
...
{
  // REVIEW: We should optimize these types of messages by setting a boolean in the associated A_SDisplayNode subclass if
  ...
  // hitTest:, it will call it on the view, which is _A_SDisplayView.  After calling into the node, any additional calls
