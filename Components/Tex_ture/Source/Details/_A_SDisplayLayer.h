...
@interface _A_SDisplayLayer : CALayer

/**
 @discussion This property overrides the CALayer category method which implements this via associated objects.
 This should result in much better performance for _A_SDisplayLayers.
 */
@property (nonatomic, weak) A_SDisplayNode *asyncdisplaykit_node;
 ...
 */
@property (atomic, weak) id<_A_SDisplayLayerDelegate> asyncDelegate;
...
/**
 ...
 @desc Used by A_SDisplayNode to display the layer synchronously on-demand (must be called on the main thread).
 */
...
/**
 * Optional methods that the view associated with an _A_SDisplayLayer can implement. 
 * This is distinguished from _A_SDisplayLayerDelegate in that it points to the _view_
 * not the node. Unfortunately this is required by A_SCollectionView, since we currently
 * can't guarantee that an A_SCollectionNode exists for it.
 */
@protocol A_SCALayerExtendedDelegate
 ...
 */
@protocol _A_SDisplayLayerDelegate <NSObject>
 ...
 */
+ (void)drawRect:(CGRect)bounds withParameters:(id)parameters isCancelled:(A_S_NOESCAPE asdisplaynode_iscancelled_block_t)isCancelledBlock isRasterizing:(BOOL)isRasterizing;
 ...
 */
+ (UIImage *)displayWithParameters:(id<NSObject>)parameters isCancelled:(A_S_NOESCAPE asdisplaynode_iscancelled_block_t)isCancelledBlock;
