...
FOUNDATION_EXPORT A_SRangeTuningParameters const A_SRangeTuningParametersZero;

FOUNDATION_EXPORT BOOL A_SRangeTuningParametersEqualToRangeTuningParameters(A_SRangeTuningParameters lhs, A_SRangeTuningParameters rhs);
 ...
 */
typedef NS_ENUM(NSInteger, A_SLayoutRangeMode) {
  A_SLayoutRangeModeUnspecified = -1,
   ...
   * still be able to see the expected content.  This mode is automatically set on all A_SRangeControllers when the app suspends,
   ...
   */
  A_SLayoutRangeModeLowMemory
};

static NSInteger const A_SLayoutRangeModeCount = 4;

typedef NS_ENUM(NSInteger, A_SLayoutRangeType) {
  A_SLayoutRangeTypeDisplay,
  A_SLayoutRangeTypePreload
};

static NSInteger const A_SLayoutRangeTypeCount = 2;
