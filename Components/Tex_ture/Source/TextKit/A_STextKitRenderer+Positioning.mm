//
//  A_STextKitRenderer+Positioning.mm
//  Tex_ture
//
...
static const CGFloat A_STextKitRendererGlyphTouchHitSlop = 5.0;
static const CGFloat A_STextKitRendererTextCapHeightPadding = 1.3;

@implementation A_STextKitRenderer (Tracking)

- (NSArray *)rectsForTextRange:(NSRange)textRange measureOption:(A_STextKitRendererMeasureOption)measureOption
{
...
/**
 ...
 prevent a deadlock
 */
- (NSArray *)unlockedRectsForTextRange:(NSRange)textRange measureOptions:(A_STextKitRendererMeasureOption)measureOption layoutManager:(NSLayoutManager *)layoutManager textStorage:(NSTextStorage *)textStorage textContainer:(NSTextContainer *)textContainer
{
  ...
  [layoutManager enumerateLineFragmentsForGlyphRange:totalGlyphRange usingBlock:^(CGRect rect,
    ...
    if (!CGRectIsNull(lineRect)) {
      if (measureOption == A_STextKitRendererMeasureOptionBlock) {
...
- (CGRect)_internalRectForGlyphAtIndex:(NSUInteger)glyphIndex
                         measureOption:(A_STextKitRendererMeasureOption)measureOption
                           ...
                           textStorage:(NSTextStorage *)textStorage
{
  ...
  if (measureOption == A_STextKitRendererMeasureOptionCapHeight
      || measureOption == A_STextKitRendererMeasureOptionBlock) {
    ...
    // the text.
    CGFloat topPadding = A_STextKitRendererTextCapHeightPadding * descent;
...
- (void)enumerateTextIndexesAtPosition:(CGPoint)externalPosition usingBlock:(as_text_component_index_block_t)block
{
  ...
  A_STextKitContext *lockingContext = self.context;
  ...
  NSInteger pointsOnA_Side = 3;

  // The distance between any 2 of the adjacent points
  CGFloat pointSeparation = squareSide / pointsOnA_Side;
  // These are for tracking which point we're on.  We start with -pointsOnA_Side/2 and go to pointsOnA_Side/2.  So if
  // pointsOnA_Side=3, we go from -1 to 1.
  NSInteger endIndex = pointsOnA_Side / 2;
    ...
    for (NSInteger j = startIndex; j <= endIndex && !stop; j++) {
      ...
      [lockingContext performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager, NSTextStorage *textStorage, NSTextContainer *textContainer) {
        ...
        glyphRect = [self _internalRectForGlyphAtIndex:glyphIndex
                                         measureOption:A_STextKitRendererMeasureOptionLineHeight
                                         layoutManager:layoutManager
                                         textContainer:textContainer
                                           textStorage:textStorage];
      }];
      ...
      // Silly TextKit...
      if (!isValidGlyph || !CGRectContainsPoint(CGRectInset(glyphRect, -A_STextKitRendererGlyphTouchHitSlop, -A_STextKitRendererGlyphTouchHitSlop), currentPoint)) {
        continue;
      }
...
- (CGRect)trailingRect
{
  ...
  [self.context performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager, NSTextStorage *textStorage, NSTextContainer *textContainer) {
    ...
    NSArray *finalRects = [self unlockedRectsForTextRange:textRange measureOptions:A_STextKitRendererMeasureOptionLineHeight layoutManager:layoutManager textStorage:textStorage textContainer:textContainer];
...
- (CGRect)frameForTextRange:(NSRange)textRange
{
  ...
  [self.context performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager, NSTextStorage *textStorage, NSTextContainer *textContainer) {
    ...
    if (NSMaxRange(textRange) > [textStorage length]) {
      A_SDisplayNodeCFailAssert(@"Invalid range");
      return;
    }
