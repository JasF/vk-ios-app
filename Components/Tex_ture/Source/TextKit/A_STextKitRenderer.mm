...
#import <Async_DisplayKit/A_STextKitFontSizeAdjuster.h>
...
@implementation A_STextKitRenderer {
...
- (instancetype)initWithTextKitAttributes:(const A_STextKitAttributes &)attributes
                          constrainedSize:(const CGSize)constrainedSize
{
  if (self = [super init]) {
    ...
    _shadower = [A_STextKitShadower shadowerWithShadowOffset:attributes.shadowOffset
    ...
    _context = [[A_STextKitContext alloc] initWithAttributedString:attributes.attributedString
    ...
    _truncater = [[A_STextKitTailTruncater alloc] initWithContext:[self context]
                                      truncationAttributedString:attributes.truncationAttributedString
                                          avoidTailTruncationSet:avoidTailTruncationSet];
      
    A_STextKitAttributes attributes = _attributes;
    // We must inset the constrained size by the size of the shadower.
    _fontSizeAdjuster = [[A_STextKitFontSizeAdjuster alloc] initWithContext:[self context]
                                                           constrainedSize:shadowConstrainedSize
...
- (NSStringDrawingContext *)stringDrawingContext
{
  ...
  if (isinf(_constrainedSize.width) == NO && _attributes.maximumNumberOfLines > 0) {
    A_SDisplayNodeAssert(_attributes.maximumNumberOfLines != 1, @"Max line count 1 is not supported in fast-path.");
...
- (void)_calculateSize
{
  ...
  if (isScaled) {
    ...
    [[self context] performBlockWithLockedTextKitComponents:^(NSLayoutManager *layoutManager, NSTextStorage *textStorage, NSTextContainer *textContainer) {
      ...
      [A_STextKitFontSizeAdjuster adjustFontSizeForAttributeString:scaledString withScaleFactor:_currentScaleFactor];
...
- (void)drawInContext:(CGContextRef)context bounds:(CGRect)bounds;
{
  ...
  A_SDisplayNodeAssertNotNil(context, @"This is no good without a context.");
...
@implementation A_STextKitRenderer (A_STextKitRendererConvenience)
