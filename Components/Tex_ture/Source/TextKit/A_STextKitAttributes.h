...
extern NSString *const A_STextKitTruncationAttributeName;
/**
 Use A_STextKitEntityAttribute as the value of this attribute to embed a link or other interactable content inside the
 text.
 */
extern NSString *const A_STextKitEntityAttributeName;

/**
 All NSObject values in this struct should be copied when passed into the TextComponent.
 */
struct A_STextKitAttributes {
  /**
   The string to be drawn.  A_STextKit will not augment this string with default colors, etc. so this must be complete.
   */
  NSAttributedString *attributedString;
  /**
   The string to use as the truncation string, usually just "...".  If you have a range of text you would like to
   restrict highlighting to (for instance if you have "... Continue Reading", use the A_STextKitTruncationAttributeName
   to mark the specific range of the string that should be highlightable.
   */
  NSAttributedString *truncationAttributedString;
  /**
   This is the character set that A_STextKit should attempt to avoid leaving as a trailing character before your
   ...
   */
  const A_STextKitAttributes copy() const
  {
    return {
  ...
  bool operator==(const A_STextKitAttributes &other) const
  {
    ...
    && A_SObjectIsEqual(exclusionPaths, other.exclusionPaths)
    && A_SObjectIsEqual(avoidTailTruncationSet, other.avoidTailTruncationSet)
    && A_SObjectIsEqual(shadowColor, other.shadowColor)
    && A_SObjectIsEqual(attributedString, other.attributedString)
    && A_SObjectIsEqual(truncationAttributedString, other.truncationAttributedString);
  }
