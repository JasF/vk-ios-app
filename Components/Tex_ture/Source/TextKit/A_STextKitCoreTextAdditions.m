...
#pragma mark - Public
BOOL A_SAttributeWithNameIsUnsupportedCoreTextAttribute(NSString *attributeName)
{
...
NSDictionary *NSAttributedStringAttributesForCoreTextAttributes(NSDictionary *coreTextAttributes)
{
  ...
  [coreTextAttributes enumerateKeysAndObjectsUsingBlock:^(NSString *coreTextKey, id coreTextValue, BOOL *stop) {
    ...
    if ([coreTextKey isEqualToString:(NSString *)kCTFontAttributeName]) {
      ...
      A_SDisplayNodeCAssertNotNil(font, @"unable to load font %@ with size %f", fontName, fontSize);
      if (font == nil) {
    ...
    else if (!A_SAttributeWithNameIsUnsupportedCoreTextAttribute(coreTextKey)){
...
NSAttributedString *A_SCleanseAttributedStringOfCoreTextAttributes(NSAttributedString *dirtyAttributedString)
{
  ...
  [dirtyAttributedString enumerateAttributesInRange:NSMakeRange(0, dirtyAttributedString.length)
                                         ...
                                         usingBlock:^(NSDictionary *dirtyAttributes, NSRange range, BOOL *stop) {
                                           [dirtyAttributes enumerateKeysAndObjectsUsingBlock:^(NSString *coreTextKey, id coreTextValue, BOOL *innerStop) {
                                             if (A_SAttributeWithNameIsUnsupportedCoreTextAttribute(coreTextKey)) {
...
#pragma mark -
#pragma mark -
@implementation NSParagraphStyle (A_STextKitCoreTextAdditions)
