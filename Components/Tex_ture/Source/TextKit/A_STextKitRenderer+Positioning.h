//
//  A_STextKitRenderer+Positioning.h
//  Tex_ture
//
...
/**
 ...
 A_STextNodeRendererMeasureOptionLineHeight is faster and will give the height from the baseline to the next line.

 A_STextNodeRendererMeasureOptionCapHeight is a more nuanced measure of the glyphs in the given range that attempts to
 produce a visually balanced rectangle above and below the glyphs to produce nice looking text highlights.

 A_STextNodeRendererMeasureOptionBlock uses the cap height option to generate each glyph index, but combines all but the
 first and last line rect into a single block.  Looks nice for multiline selection.
 */
typedef NS_ENUM(NSUInteger, A_STextKitRendererMeasureOption) {
  A_STextKitRendererMeasureOptionLineHeight,
  A_STextKitRendererMeasureOptionCapHeight,
  A_STextKitRendererMeasureOptionBlock
};

@interface A_STextKitRenderer (Positioning)
...
/**
 ...
 attributedString of this renderer.
 @param measureOption The measure option to use for construction of the rects. See A_STextKitRendererMeasureOption
 docs for usage.
