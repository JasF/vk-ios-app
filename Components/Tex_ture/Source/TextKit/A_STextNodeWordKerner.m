...
@implementation A_STextNodeWordKerner
...
- (CGFloat)_wordKernedSpaceWidthForCharacterAtIndex:(NSUInteger)characterIndex atGlyphPosition:(CGPoint)glyphPosition forTextContainer:(NSTextContainer *)textContainer layoutManager:(NSLayoutManager *)layoutManager
{
  ...
  dispatch_once(&onceToken, ^{
    ...
    mapQueue = dispatch_queue_create("org.Async_DisplayKit.wordKerningQueue", DISPATCH_QUEUE_SERIAL);
  });
  ...
  CGFloat wordKerning = [[layoutManager.textStorage attribute:A_STextNodeWordKerningAttributeName atIndex:characterIndex effectiveRange:NULL] floatValue];
