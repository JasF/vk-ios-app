...
@interface A_STextKitComponentsTextView ()
@property (atomic, assign) CGRect threadSafeBounds;
@end

@implementation A_STextKitComponentsTextView
...
@interface A_STextKitComponents ()
...
@implementation A_STextKitComponents
...
{
  A_STextKitComponents *components = [[self alloc] init];
...
{
  A_STextKitComponents *components = self;
  ...
  if (CGRectGetWidth(components.textView.threadSafeBounds) != constrainedWidth) {
    components = [A_STextKitComponents componentsWithAttributedSeedString:components.textStorage textContainerSize:CGSizeMake(constrainedWidth, CGFLOAT_MAX)];
  }
