...
/**
 * The subclass header _A_SControlNode+Subclasses_ defines methods to be
 * overridden by custom nodes that subclass A_SControlNode.
 *
 * These methods should never be called directly by other classes.
 */

@interface A_SControlNode (Subclassing)

/**
 @abstract Sends action messages for the given control events.
 @param controlEvents A bitmask whose set flags specify the control events for which action messages are sent. See "Control Events" in A_SControlNode.h for bitmask constants.
 @param touchEvent An event object encapsulating the information specific to the user event.
 @discussion A_SControlNode implements this method to send all action messages associated with controlEvents. The list of targets is constructed from prior invocations of addTarget:action:forControlEvents:.
 */
- (void)sendActionsForControlEvents:(A_SControlNodeEvent)controlEvents withEvent:(nullable UIEvent *)touchEvent;
