...
- (instancetype)init
{
  if (self = [super init]) {
    ...
    _contentHorizontalAlignment = A_SHorizontalAlignmentMiddle;
    _contentVerticalAlignment = A_SVerticalAlignmentCenter;
    _contentEdgeInsets = UIEdgeInsetsZero;
    _imageAlignment = A_SButtonNodeImageAlignmentBeginning;
    self.accessibilityTraits = UIAccessibilityTraitButton;
  }
  return self;
}

- (A_STextNode *)titleNode
{
  if (!_titleNode) {
...
- (A_SImageNode *)backgroundImageNode
{
  if (!_backgroundImageNode) {
    _backgroundImageNode = [[A_SImageNode alloc] init];
...
{
  A_SDisplayNodeAssert(!layerBacked, @"A_SButtonNode must not be layer backed!");
...
- (A_SVerticalAlignment)contentVerticalAlignment
{
  A_SDN::MutexLocker l(__instanceLock__);
  return _contentVerticalAlignment;
}

- (void)setContentVerticalAlignment:(A_SVerticalAlignment)contentVerticalAlignment
{
  A_SDN::MutexLocker l(__instanceLock__);
  _contentVerticalAlignment = contentVerticalAlignment;
}

- (A_SHorizontalAlignment)contentHorizontalAlignment
{
  A_SDN::MutexLocker l(__instanceLock__);
  return _contentHorizontalAlignment;
}

- (void)setContentHorizontalAlignment:(A_SHorizontalAlignment)contentHorizontalAlignment
{
  A_SDN::MutexLocker l(__instanceLock__);
...
- (A_SButtonNodeImageAlignment)imageAlignment
{
  A_SDN::MutexLocker l(__instanceLock__);
  return _imageAlignment;
}

- (void)setImageAlignment:(A_SButtonNodeImageAlignment)imageAlignment
{
  A_SDN::MutexLocker l(__instanceLock__);
...
- (A_SLayoutSpec *)layoutSpecThatFits:(A_SSizeRange)constrainedSize
{
  ...
  {
    A_SDN::MutexLocker l(__instanceLock__);
    stack.direction = _laysOutHorizontally ? A_SStackLayoutDirectionHorizontal : A_SStackLayoutDirectionVertical;
  ...
  if (_titleNode.attributedText.length > 0) {
    if (imageAlignment == A_SButtonNodeImageAlignmentBeginning) {
  ...
  if (UIEdgeInsetsEqualToEdgeInsets(UIEdgeInsetsZero, contentEdgeInsets) == NO) {
    spec = [A_SInsetLayoutSpec insetLayoutSpecWithInsets:contentEdgeInsets child:spec];
  }

  if (_backgroundImageNode.image) {
    spec = [A_SBackgroundLayoutSpec backgroundLayoutSpecWithChild:spec background:_backgroundImageNode];
  }
