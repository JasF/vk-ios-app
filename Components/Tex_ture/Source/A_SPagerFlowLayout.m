...
@interface A_SPagerFlowLayout () {
  __weak A_SCellNode *_currentCellNode;
}

@end

//TODO make this an A_SCollectionViewLayout
@implementation A_SPagerFlowLayout

- (A_SCollectionView *)asCollectionView
{
  // Dynamic cast is too slow and not worth it.
  return (A_SCollectionView *)self.collectionView;
}
...
- (void)_updateCurrentNode
{
  ...
  if (indexPath) {
    A_SCellNode *node = [self.asCollectionView nodeForItemAtIndexPath:indexPath];
    if (node) {
