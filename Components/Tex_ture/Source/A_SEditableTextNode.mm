...
#import <Async_DisplayKit/A_STextNodeWordKerner.h>
#import <Async_DisplayKit/A_SThread.h>

/**
 @abstract Object to hold UITextView's pending UITextInputTraits
**/
@interface _A_STextInputTraitsPendingState : NSObject
...
@implementation _A_STextInputTraitsPendingState
...
/**
 ...
 See issue: https://github.com/facebook/Async_DisplayKit/issues/1063
 */
@interface A_SPanningOverriddenUITextView : A_STextKitComponentsTextView
{
  BOOL _shouldBlockPanGesture;
}
@end

@implementation A_SPanningOverriddenUITextView
...
#pragma mark -
@interface A_SEditableTextNode () <UITextViewDelegate, NSLayoutManagerDelegate>
{
  @private
  // Configuration.
  ...
  // Core.
  id <A_SEditableTextNodeDelegate> __weak _delegate;
  BOOL _delegateDidUpdateEnqueued;

  // TextKit.
  A_SDN::RecursiveMutex _textKitLock;
  A_STextKitComponents *_textKitComponents;
  A_STextKitComponents *_placeholderTextKitComponents;
  // Forwards NSLayoutManagerDelegate methods related to word kerning
  A_STextNodeWordKerner *_wordKerner;
  
  // UITextInputTraits
  A_SDN::RecursiveMutex _textInputTraitsLock;
  _A_STextInputTraitsPendingState *_textInputTraits;
...
@property (nonatomic, strong, readonly) _A_STextInputTraitsPendingState *textInputTraits;

@end

@implementation A_SEditableTextNode

#pragma mark - NSObject Overrides
- (instancetype)init
{
  return [self initWithTextKitComponents:[A_STextKitComponents componentsWithAttributedSeedString:nil textContainerSize:CGSizeZero]
            placeholderTextKitComponents:[A_STextKitComponents componentsWithAttributedSeedString:nil textContainerSize:CGSizeZero]];
}

- (instancetype)initWithTextKitComponents:(A_STextKitComponents *)textKitComponents
             placeholderTextKitComponents:(A_STextKitComponents *)placeholderTextKitComponents
{
  if (!(self = [super init]))
    return nil;
  ...
  _wordKerner = [[A_STextNodeWordKerner alloc] init];
...
- (void)didLoad
{
  ...
  void (^configureTextView)(UITextView *) = ^(UITextView *textView) {
    ...
    {
      A_SDN::MutexLocker l(_textInputTraitsLock);
      if (_textInputTraits) {
  ...
  A_SDN::MutexLocker l(_textKitLock);

  // Create and configure the placeholder text view.
  _placeholderTextKitComponents.textView = [[A_STextKitComponentsTextView alloc] initWithFrame:CGRectZero textContainer:_placeholderTextKitComponents.textContainer];
  ...
  _textKitComponents.textView = [[A_SPanningOverriddenUITextView alloc] initWithFrame:CGRectZero textContainer:_textKitComponents.textContainer];
...
{
  A_STextKitComponents *displayedComponents = [self isDisplayingPlaceholder] ? _placeholderTextKitComponents : _textKitComponents;
...
{
  A_SDisplayNodeAssert(!layerBacked, @"Cannot set layerBacked to YES on A_SEditableTextNode – instances must be view-backed in order to ensure touch events can be passed to the internal UITextView during editing.");
...
- (UITextView *)textView
{
  A_SDisplayNodeAssertMainThread();
  [self view];
  A_SDisplayNodeAssert(_textKitComponents.textView != nil, @"UITextView must be created in -[A_SEditableTextNode didLoad]");
...
{
  if (A_SObjectIsEqual(typingAttributes, _typingAttributes))
    return;
...
- (void)setAttributedPlaceholderText:(NSAttributedString *)attributedPlaceholderText
{
  ...
  if (A_SObjectIsEqual(_placeholderTextKitComponents.textStorage, attributedPlaceholderText))
    return;
...
- (void)setAttributedText:(NSAttributedString *)attributedText
{
  ...
  if (A_SObjectIsEqual((_textKitComponents.textView.attributedText ? : _textKitComponents.textStorage), attributedText))
    return;
...
- (_A_STextInputTraitsPendingState *)textInputTraits
{
  if (!_textInputTraits) {
    _textInputTraits = [[_A_STextInputTraitsPendingState alloc] init];
  }
...
- (CGRect)frameForTextRange:(NSRange)textRange
{
  ...
  if (NSMaxRange(textRange) > [_textKitComponents.textStorage length]) {
    A_SDisplayNodeAssert(NO, @"Invalid range");
    return CGRectZero;
  }
