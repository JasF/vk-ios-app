...
@interface A_SWeakMap ()
@property (nonatomic, strong, readonly) NSMapTable<id, A_SWeakMapEntry *> *hashTable;
@end
...
- (A_SWeakMapEntry *)entryForKey:(id)key
{
  return [self.hashTable objectForKey:key];
}

- (A_SWeakMapEntry *)setObject:(id)value forKey:(id)key
{
  A_SWeakMapEntry *entry = [self.hashTable objectForKey:key];
  if (entry != nil) {
    // Update the value in the existing entry.
    entry.value = value;
  } else {
    entry = [[A_SWeakMapEntry alloc] initWithKey:key value:value];
