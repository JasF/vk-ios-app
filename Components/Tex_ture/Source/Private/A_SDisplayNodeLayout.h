...
/*
 * Represents a connection between an A_SLayout and a A_SDisplayNode
 * A_SDisplayNode uses this to store additional information that are necessary besides the layout
 */
struct A_SDisplayNodeLayout {
  A_SLayout *layout;
  A_SSizeRange constrainedSize;
  CGSize parentSize;
   ...
   * @param version The version of the source layout data – see A_SDisplayNode's _layoutVersion. 
   */
  A_SDisplayNodeLayout(A_SLayout *layout, A_SSizeRange constrainedSize, CGSize parentSize, NSUInteger version)
   ...
   */
  BOOL isValid(A_SSizeRange constrainedSize, CGSize parentSize, NSUInteger version);
};
