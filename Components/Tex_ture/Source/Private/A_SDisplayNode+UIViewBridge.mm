//
//  A_SDisplayNode+UIViewBridge.mm
//  Tex_ture
//
...
/**
 * The following macros are conveniences to help in the common tasks related to the bridging that A_SDisplayNode does to UIView and CALayer.
 ...
 *   - Has differing types on views and layers, or custom A_SDisplayNode-specific behavior is desired
 *       manually implement
 *
...
#define _bridge_prologue_read A_SDN::MutexLocker l(__instanceLock__); A_SDisplayNodeAssertThreadAffinity(self)
#define _bridge_prologue_write A_SDN::MutexLocker l(__instanceLock__)
#else
#define _bridge_prologue_read A_SDisplayNodeAssertThreadAffinity(self)
#define _bridge_prologue_write
#endif

/// Returns YES if the property set should be applied to view/layer immediately.
/// Side Effect: Registers the node with the shared A_SPendingStateController if
...
/// to apply it.
A_SDISPLAYNODE_INLINE BOOL A_SDisplayNodeShouldApplyBridgedWriteToView(A_SDisplayNode *node) {
  BOOL loaded = __loaded(node);
  if (A_SDisplayNodeThreadIsMain()) {
    return loaded;
  } else {
    if (loaded && !A_SDisplayNodeGetPendingState(node).hasChanges) {
      [[A_SPendingStateController sharedInstance] registerNode:node];
    }
    return NO;
  }
};
...
#define _getFromViewOrLayer(layerProperty, viewAndPendingViewStateProperty) __loaded(self) ? \
 ...
 : A_SDisplayNodeGetPendingState(self).viewAndPendingViewStateProperty

#define _setToViewOrLayer(layerProperty, layerValueExpr, viewAndPendingViewStateProperty, viewAndPendingViewStateExpr) BOOL shouldApply = A_SDisplayNodeShouldApplyBridgedWriteToView(self); \
  if (shouldApply) { (_view ? _view.viewAndPendingViewStateProperty = (viewAndPendingViewStateExpr) : _layer.layerProperty = (layerValueExpr)); } else { A_SDisplayNodeGetPendingState(self).viewAndPendingViewStateProperty = (viewAndPendingViewStateExpr); }

#define _setToViewOnly(viewAndPendingViewStateProperty, viewAndPendingViewStateExpr) BOOL shouldApply = A_SDisplayNodeShouldApplyBridgedWriteToView(self); \
if (shouldApply) { _view.viewAndPendingViewStateProperty = (viewAndPendingViewStateExpr); } else { A_SDisplayNodeGetPendingState(self).viewAndPendingViewStateProperty = (viewAndPendingViewStateExpr); }

#define _getFromViewOnly(viewAndPendingViewStateProperty) __loaded(self) ? _view.viewAndPendingViewStateProperty : A_SDisplayNodeGetPendingState(self).viewAndPendingViewStateProperty

#define _getFromLayer(layerProperty) __loaded(self) ? _layer.layerProperty : A_SDisplayNodeGetPendingState(self).layerProperty

#define _setToLayer(layerProperty, layerValueExpr) BOOL shouldApply = A_SDisplayNodeShouldApplyBridgedWriteToView(self); \
if (shouldApply) { _layer.layerProperty = (layerValueExpr); } else { A_SDisplayNodeGetPendingState(self).layerProperty = (layerValueExpr); }

/**
 * This category implements certain frequently-used properties and methods of UIView and CALayer so that A_SDisplayNode clients can just call the view/layer methods on the node,
 ...
 * without any CALayer or UIView actually existing while still being able to set and read properties from A_SDisplayNode instances.
 */
@implementation A_SDisplayNode (UIViewBridge)
...
- (CGFloat)cornerRadius
{
  ...
  if (_cornerRoundingType == A_SCornerRoundingTypeDefaultSlowCALayer) {
...
- (A_SCornerRoundingType)cornerRoundingType
{
  A_SDN::MutexLocker l(__instanceLock__);
  return _cornerRoundingType;
}

- (void)setCornerRoundingType:(A_SCornerRoundingType)newRoundingType
{
  A_SDN::MutexLocker l(__instanceLock__);
...
//  A_SDisplayNodeAssert(CATransform3DIsIdentity(self.transform), @"-[A_SDisplayNode frame] - self.transform must be identity in order to use the frame property.  (From Apple's UIView documentation: If the transform property is not the identity transform, the value of this property is undefined and therefore should be ignored.)");
//#endif
...
- (void)setFrame:(CGRect)rect
{
  ...
  // For classes like A_STableNode, A_SCollectionNode, A_SScrollNode and similar - make sure UIView gets setFrame:
  struct A_SDisplayNodeFlags flags = _flags;
  BOOL specialPropertiesHandling = A_SDisplayNodeNeedsSpecialPropertiesHandling(checkFlag(Synchronous), flags.layerBacked);

  BOOL nodeLoaded = __loaded(self);
  BOOL isMainThread = A_SDisplayNodeThreadIsMain();
  if (!specialPropertiesHandling) {
    ...
    if (canReadProperties) {
      ...
      A_SBoundsAndPositionForFrame(rect, origin, anchorPoint, &newBounds, &newPosition);

      if (A_SIsCGRectValidForLayout(newBounds) == NO || A_SIsCGPositionValidForLayout(newPosition) == NO) {
        A_SDisplayNodeAssertNonFatal(NO, @"-[A_SDisplayNode setFrame:] - The new frame (%@) is invalid and unsafe to be set.", NSStringFromCGRect(rect));
        return;
      }
      ...
      _A_SPendingState *pendingState = A_SDisplayNodeGetPendingState(self);
      if (nodeLoaded && !pendingState.hasChanges) {
        [[A_SPendingStateController sharedInstance] registerNode:self];
      }
...
//#if DEBUG
//      // Checking if the transform is identity is expensive, so disable when unnecessary. We have assertions on in Release, so DEBUG is the only way I know of.
//      A_SDisplayNodeAssert(CATransform3DIsIdentity(self.transform), @"-[A_SDisplayNode setFrame:] - self.transform must be identity in order to set the frame property.  (From Apple's UIView documentation: If the transform property is not the identity transform, the value of this property is undefined and therefore should be ignored.)");
//#endif
      _view.frame = rect;
    } else {
...
- (void)setNeedsDisplay
{
  ...
  {
    _bridge_prologue_write;
    isRasterized = _hierarchyState & A_SHierarchyStateRasterized;
    shouldApply = A_SDisplayNodeShouldApplyBridgedWriteToView(self);
    ...
    if (isRasterized == NO && shouldApply == NO) {
      ...
      [A_SDisplayNodeGetPendingState(self) setNeedsDisplay];
    }
  }
  ...
  if (isRasterized) {
    A_SPerformBlockOnMainThread(^{
      ...
      // For this reason, this method should be avoided when possible.  Use _hierarchyState & A_SHierarchyStateRasterized.
      A_SDisplayNodeAssertMainThread();
      A_SDisplayNode *rasterizedContainerNode = self.supernode;
...
- (void)setNeedsLayout
{
  ...
  id viewOrLayer = nil;
  {
    ...
    if (shouldApply == NO && loaded) {
      ...
      // We can't release the lock before applying to pending state, or it may be flushed before it can be applied.
      [A_SDisplayNodeGetPendingState(self) setNeedsLayout];
    }
  }
...
- (void)layoutIfNeeded
{
  ...
  id viewOrLayer = nil;
  {
    ...
    if (shouldApply == NO && loaded) {
      // The node is loaded but we're not on main.
      // We will call layoutIfNeeded on the view or layer when we apply the pending state. __layout will in turn be called on us (see -[_A_SDisplayLayer layoutSublayers]).
      // We need to call it on main if the node is loaded to support automatic subnode management.
      // We can't release the lock before applying to pending state, or it may be flushed before it can be applied.
      [A_SDisplayNodeGetPendingState(self) layoutIfNeeded];
    }
  }
  
  if (shouldApply) {
    // The node is loaded and we're on main.
    // Message the view or layer which in turn will call __layout on us (see -[_A_SDisplayLayer layoutSublayers]).
...
- (void)setOpaque:(BOOL)newOpaque
{
  ...
  BOOL shouldApply = A_SDisplayNodeShouldApplyBridgedWriteToView(self);
  ...
  if (shouldApply) {
    ...
    A_SDisplayNodeGetPendingState(self).opaque = newOpaque;
  }
}
...
{
  _bridge_prologue_read;
  A_SDisplayNodeAssert(!_flags.layerBacked, @"Danger: this property is undefined on layer-backed nodes.");
...
- (UIViewContentMode)contentMode
{
  ...
  if (__loaded(self)) {
    if (_flags.layerBacked) {
      return A_SDisplayNodeUIContentModeFromCAContentsGravity(_layer.contentsGravity);
    } else {
      return _view.contentMode;
    }
  } else {
    return A_SDisplayNodeGetPendingState(self).contentMode;
  }
}
...
- (void)setContentMode:(UIViewContentMode)contentMode
{
  ...
  if (shouldApply) {
    if (_flags.layerBacked) {
      _layer.contentsGravity = A_SDisplayNodeCAContentsGravityFromUIContentMode(contentMode);
    } else {
      _view.contentMode = contentMode;
    }
  } else {
    A_SDisplayNodeGetPendingState(self).contentMode = contentMode;
  }
}
...
- (void)setBackgroundColor:(UIColor *)newBackgroundColor
{
  ...
  if (shouldApply) {
    ...
    A_SDisplayNodeGetPendingState(self).backgroundColor = newBackgroundCGColor;
  }
}
...
@implementation A_SDisplayNode (InternalPropertyBridge)
...
// A_SDK supports accessibility for view or layer backed nodes. To be able to provide support for layer backed
...
@implementation A_SDisplayNode (UIViewBridgeAccessibility)
...
#pragma mark - A_SAsyncTransactionContainer

@implementation A_SDisplayNode (A_SAsyncTransactionContainer)
