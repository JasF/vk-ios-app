//
//  A_SDisplayNode+AsyncDisplay.mm
//  Tex_ture
//
...
@interface A_SDisplayNode () <_A_SDisplayLayerDelegate>
@end

@implementation A_SDisplayNode (AsyncDisplay)

#if A_SDISPLAYNODE_DELAY_DISPLAY
  #define A_SDN_DELAY_FOR_DISPLAY() usleep( (long)(0.1 * USEC_PER_SEC) )
#else
  #define A_SDN_DELAY_FOR_DISPLAY()
#endif
...
- (void)_recursivelyRasterizeSelfAndSublayersWithIsCancelledBlock:(asdisplaynode_iscancelled_block_t)isCancelledBlock displayBlocks:(NSMutableArray *)displayBlocks
{
  ...
  __instanceLock__.lock();
  BOOL rasterizingFromAscendent = (_hierarchyState & A_SHierarchyStateRasterized);
  __instanceLock__.unlock();
  ...
  if (shouldDisplay) {
    dispatch_block_t pushAndDisplayBlock = ^{
      ...
      if (displayBlock) {
        ...
        if (image) {
          BOOL opaque = A_SImageAlphaInfoIsOpaque(CGImageGetAlphaInfo(image.CGImage));
                                                                         ...
                                                                         rasterizing:(BOOL)rasterizing
{
  ...
  A_SDisplayNodeAssert(contentsScaleForDisplay != 0.0, @"Invalid contents scale");
  A_SDisplayNodeAssert(rasterizing || !(_hierarchyState & A_SHierarchyStateRasterized),
  ...
  displayBlock = ^{
    A_SSignpostStartCustom(A_SSignpostLayerDisplay, ptrSelf, 0);
    id result = displayBlock();
    A_SSignpostEndCustom(A_SSignpostLayerDisplay, ptrSelf, 0, isCancelledBlock() ? A_SSignpostColorRed : A_SSignpostColorGreen);
    return result;
  };
#endif
...
- (void)__willDisplayNodeContentWithRenderingContext:(CGContextRef)context drawParameters:(id _Nullable)drawParameters
{
  if (context) {
    __instanceLock__.lock();
      A_SCornerRoundingType cornerRoundingType = _cornerRoundingType;
      CGFloat cornerRadius = _cornerRadius;
      A_SDisplayNodeContextModifier willDisplayNodeContentWithRenderingContext = _willDisplayNodeContentWithRenderingContext;
    __instanceLock__.unlock();

    if (cornerRoundingType == A_SCornerRoundingTypePrecomposited && cornerRadius > 0.0) {
      A_SDisplayNodeAssert(context == UIGraphicsGetCurrentContext(), @"context is expected to be pushed on UIGraphics stack %@", self);
...
- (void)__didDisplayNodeContentWithRenderingContext:(CGContextRef)context image:(UIImage **)image drawParameters:(id _Nullable)drawParameters backgroundColor:(UIColor *)backgroundColor borderWidth:(CGFloat)borderWidth borderColor:(CGColorRef)borderColor
{
  ...
  __instanceLock__.lock();
    ...
    A_SDisplayNodeContextModifier didDisplayNodeContentWithRenderingContext = _didDisplayNodeContentWithRenderingContext;
  __instanceLock__.unlock();
  ...
  if (cornerRoundingType == A_SCornerRoundingTypePrecomposited && cornerRadius > 0.0f) {
    ...
    A_SDisplayNodeAssert(UIGraphicsGetCurrentContext(), @"context is expected to be pushed on UIGraphics stack %@", self);
...
- (void)displayAsyncLayer:(_A_SDisplayLayer *)asyncLayer asynchronously:(BOOL)asynchronously
{
  A_SDisplayNodeAssertMainThread();
  
  __instanceLock__.lock();
  
  if (_hierarchyState & A_SHierarchyStateRasterized) {
    __instanceLock__.unlock();
    return;
  }
  ...
  if (asynchronously) {
    ...
    __weak A_SDisplayNode *weakSelf = self;
    isCancelledBlock = ^BOOL{
      __strong A_SDisplayNode *self = weakSelf;
  ...
  A_SDisplayNodeAssert(layer, @"Expect _layer to be not nil");
  ...
  asyncdisplaykit_async_transaction_operation_completion_block_t completionBlock = ^(id<NSObject> value, BOOL canceled){
    ...
    if (!canceled && !isCancelledBlock()) {
      ...
      if (stretchable) {
        A_SDisplayNodeSetResizableContents(layer, image);
      } else {
  ...
  if (asynchronously) {
    ...
    // this call will allocate the transaction and add it to _A_SAsyncTransactionGroup.
    // It will automatically commit the transaction at the end of the runloop.
    _A_SAsyncTransaction *transaction = containerLayer.asyncdisplaykit_asyncTransaction;
    ...
    [transaction addOperationWithBlock:displayBlock priority:self.drawingPriority queue:[_A_SDisplayLayer displayQueue] completion:completionBlock];
  } else {
...
- (void)cancelDisplayAsyncLayer:(_A_SDisplayLayer *)asyncLayer
{
  _displaySentinel.fetch_add(1);
}

- (A_SDisplayNodeContextModifier)willDisplayNodeContentWithRenderingContext
{
  A_SDN::MutexLocker l(__instanceLock__);
  return _willDisplayNodeContentWithRenderingContext;
}

- (A_SDisplayNodeContextModifier)didDisplayNodeContentWithRenderingContext
{
  A_SDN::MutexLocker l(__instanceLock__);
  return _didDisplayNodeContentWithRenderingContext;
}

- (void)setWillDisplayNodeContentWithRenderingContext:(A_SDisplayNodeContextModifier)contextModifier
{
  A_SDN::MutexLocker l(__instanceLock__);
  _willDisplayNodeContentWithRenderingContext = contextModifier;
}

- (void)setDidDisplayNodeContentWithRenderingContext:(A_SDisplayNodeContextModifier)contextModifier;
{
  A_SDN::MutexLocker l(__instanceLock__);
