...
const CGSize A_STextContainerMaxSize = (CGSize){0x100000, 0x100000};
...
static inline CGSize A_STextClipCGSize(CGSize size) {
  if (size.width > A_STextContainerMaxSize.width) size.width = A_STextContainerMaxSize.width;
  if (size.height > A_STextContainerMaxSize.height) size.height = A_STextContainerMaxSize.height;
  return size;
}
 ...
 */
static CGColorRef A_STextGetCGColor(CGColorRef color) {
...
@implementation A_STextLinePositionSimpleModifier
- (void)modifyLines:(NSArray *)lines fromText:(NSAttributedString *)text inContainer:(A_STextContainer *)container {
  if (container.verticalForm) {
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
      A_STextLine *line = lines[i];
...
- (id)copyWithZone:(NSZone *)zone {
  A_STextLinePositionSimpleModifier *one = [self.class new];
...
@implementation A_STextContainer {
  @package
  BOOL _readonly; ///< used only in A_STextLayout.implementation
  ...
  A_STextTruncationType _truncationType;
  NSAttributedString *_truncationToken;
  id<A_STextLinePositionModifier> _linePositionModifier;
}
...
+ (instancetype)containerWithSize:(CGSize)size insets:(UIEdgeInsets)insets {
  A_STextContainer *one = [self new];
  one.size = A_STextClipCGSize(size);
  one.insets = insets;
  return one;
}
...
- (id)copyWithZone:(NSZone *)zone {
  A_STextContainer *one = [self.class new];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  _truncationType = (A_STextTruncationType)[aDecoder decodeIntegerForKey:@"truncationType"];
...
reason:@"Cannot change the property of the 'container' in 'A_STextLayout'." userInfo:nil]; \
return; \
} \
dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER); \
__VA_ARGS__; \
...
- (void)setSize:(CGSize)size {
  Setter(if(!_path) _size = A_STextClipCGSize(size));
}
...
- (A_STextTruncationType)truncationType {
  Getter(A_STextTruncationType type = _truncationType) return type;
}

- (void)setTruncationType:(A_STextTruncationType)truncationType {
...
- (void)setLinePositionModifier:(id<A_STextLinePositionModifier>)linePositionModifier {
  Setter(_linePositionModifier = [(NSObject *)linePositionModifier copy]);
}

- (id<A_STextLinePositionModifier>)linePositionModifier {
  Getter(id<A_STextLinePositionModifier> m = _linePositionModifier) return m;
}
...
@interface A_STextLayout ()

@property (nonatomic, readwrite) A_STextContainer *container;
...
@property (nonatomic, readwrite) A_STextLine *truncatedLine;
...
@property (nonatomic, assign) A_SRowEdge *lineRowsEdge; ///< top-left origin

@end



@implementation A_STextLayout
...
+ (A_STextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text {
  A_STextContainer *container = [A_STextContainer containerWithSize:size];
  return [self layoutWithContainer:container text:text];
}

+ (A_STextLayout *)layoutWithContainer:(A_STextContainer *)container text:(NSAttributedString *)text {
  return [self layoutWithContainer:container text:text range:NSMakeRange(0, text.length)];
}

+ (A_STextLayout *)layoutWithContainer:(A_STextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
  A_STextLayout *layout = NULL;
  ...
  A_STextLine *truncatedLine = nil;
  A_SRowEdge *lineRowsEdge = NULL;
...
#define FAIL_AND_RETURN {\
  ...
  layout = [[A_STextLayout alloc] _init];
  layout.text = text;
  ...
  if (container.path == nil && container.exclusionPaths.count == 0) {
    ...
    if (needFixLayoutSizeBug) {
      ...
      if (container.isVerticalForm) {
        rect.size.width = A_STextContainerMaxSize.width;
      } else {
        rect.size.height = A_STextContainerMaxSize.height;
      }
    }
  ...
  ctFrame = CTFramesetterCreateFrame(ctSetter, A_STextCFRangeFromNSRange(range), cgPath, (CFDictionaryRef)frameAttrs);
  ...
  for (NSUInteger i = 0; i < lineCount; i++) {
    ...
    A_STextLine *line = [A_STextLine lineWithCTLine:ctLine position:position vertical:isVerticalForm];
  ...
  if (rowCount > 0) {
    if (maximumNumberOfRows > 0) {
      if (rowCount > maximumNumberOfRows) {
        ...
        do {
          A_STextLine *line = lines.lastObject;
          if (!line) break;
    ...
    A_STextLine *lastLine = lines.lastObject;
    ...
    lineRowsEdge = (A_SRowEdge *)calloc(rowCount, sizeof(A_SRowEdge));
    ...
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
      ...
      if ((NSInteger)line.row != lastRowIdx) {
        if (lastRowIdx >= 0) {
          lineRowsEdge[lastRowIdx] = (A_SRowEdge) {.head = lastHead, .foot = lastFoot };
        }
        lastRowIdx = line.row;
    ...
    for (NSUInteger i = 1; i < rowCount; i++) {
      A_SRowEdge v0 = lineRowsEdge[i - 1];
      A_SRowEdge v1 = lineRowsEdge[i];
  ...
  { // calculate bounding size
    ...
    if (container.path) {
      ...
      rect = UIEdgeInsetsInsetRect(rect,A_STextUIEdgeInsetsInvert(container.insets));
    }
    rect = CGRectStandardize(rect);
  ...
  visibleRange = A_STextNSRangeFromCFRange(CTFrameGetVisibleStringRange(ctFrame));
  if (needTruncation) {
    A_STextLine *lastLine = lines.lastObject;
    ...
    if (container.truncationType != A_STextTruncationTypeNone) {
      ...
      if (container.truncationToken) {
        ...
        truncationToken = [[NSAttributedString alloc] initWithString:A_STextTruncationToken attributes:attrs];
        truncationTokenLine = CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
      }
      if (truncationTokenLine) {
        CTLineTruncationType type = kCTLineTruncationEnd;
        if (container.truncationType == A_STextTruncationTypeStart) {
          type = kCTLineTruncationStart;
        } else if (container.truncationType == A_STextTruncationTypeMiddle) {
        ...
        if (ctLastLineExtend) {
          ...
          if (ctTruncatedLine) {
            truncatedLine = [A_STextLine lineWithCTLine:ctTruncatedLine position:lastLine.position vertical:isVerticalForm];
  ...
  if (isVerticalForm) {
    NSCharacterSet *rotateCharset = A_STextVerticalFormRotateCharacterSet();
    NSCharacterSet *rotateMoveCharset = A_STextVerticalFormRotateAndMoveCharacterSet();
    
    void (^lineBlock)(A_STextLine *) = ^(A_STextLine *line){
      ...
      for (NSUInteger r = 0; r < runCount; r++) {
        ...
        BOOL isColorGlyph = A_STextCTFontContainsColorBitmapGlyphs(font);
        
        NSUInteger prevIdx = 0;
        A_STextRunGlyphDrawMode prevMode = A_STextRunGlyphDrawModeHorizontal;
        ...
        for (NSUInteger g = 0; g < glyphCount; g++) {
          ...
          A_STextRunGlyphDrawMode mode = glyphRotateMove ? A_STextRunGlyphDrawModeVerticalRotateMove : (glyphRotate ? A_STextRunGlyphDrawModeVerticalRotate : A_STextRunGlyphDrawModeHorizontal);
          if (g == 0) {
            prevMode = mode;
          } else if (mode != prevMode) {
            A_STextRunGlyphRange *aRange = [A_STextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx) drawMode:prevMode];
        ...
        if (prevIdx < glyphCount) {
          A_STextRunGlyphRange *aRange = [A_STextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx) drawMode:prevMode];
          [runRanges addObject:aRange];
        }
        
      }
    };
    for (A_STextLine *line in lines) {
      lineBlock(line);
    }
  ...
  if (visibleRange.length > 0) {
    ...
    void (^block)(NSDictionary *attrs, NSRange range, BOOL *stop) = ^(NSDictionary *attrs, NSRange range, BOOL *stop) {
      if (attrs[A_STextHighlightAttributeName]) layout.containsHighlight = YES;
      if (attrs[A_STextBlockBorderAttributeName]) layout.needDrawBlockBorder = YES;
      if (attrs[A_STextBackgroundBorderAttributeName]) layout.needDrawBackgroundBorder = YES;
      if (attrs[A_STextShadowAttributeName] || attrs[NSShadowAttributeName]) layout.needDrawShadow = YES;
      if (attrs[A_STextUnderlineAttributeName]) layout.needDrawUnderline = YES;
      if (attrs[A_STextAttachmentAttributeName]) layout.needDrawAttachment = YES;
      if (attrs[A_STextInnerShadowAttributeName]) layout.needDrawInnerShadow = YES;
      if (attrs[A_STextStrikethroughAttributeName]) layout.needDrawStrikethrough = YES;
      if (attrs[A_STextBorderAttributeName]) layout.needDrawBorder = YES;
    };
  ...
  for (NSUInteger i = 0, max = lines.count; i < max; i++) {
    ...
    if (line.attachments.count > 0) {
      ...
      for (A_STextAttachment *attachment in line.attachments) {
...
+ (NSArray *)layoutWithContainers:(NSArray *)containers text:(NSAttributedString *)text range:(NSRange)range {
  ...
  for (NSUInteger i = 0, max = containers.count; i < max; i++) {
    A_STextContainer *container = containers[i];
    A_STextLayout *layout = [self layoutWithContainer:container text:text range:range];
...
- (NSUInteger)_rowIndexForEdge:(CGFloat)edge {
  ...
  while (lo <= hi) {
    mid = (lo + hi) / 2;
    A_SRowEdge oneEdge = _lineRowsEdge[mid];
    if (isVertical ?
 ...
 */
- (CTRunRef)_runForLine:(A_STextLine *)line position:(A_STextPosition *)position {
  if (!line || !position) return NULL;
  CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
  for (NSUInteger i = 0, max = CFArrayGetCount(runs); i < max; i++) {
    CTRunRef run = (CTRunRef)CFArrayGetValueAtIndex(runs, i);
    CFRange range = CTRunGetStringRange(run);
    if (position.affinity == A_STextAffinityBackward) {
...
/**
 ...
 next:  right position
 */
- (BOOL)_insideComposedCharacterSequences:(A_STextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
 ...
 */
- (BOOL)_insideEmoji:(A_STextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
  if (!line) return NO;
  CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
  for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
    ...
    if (!A_STextCTFontContainsColorBitmapGlyphs(font)) continue;
...
/**
 ...
 @return YES if RTL.
 */
- (BOOL)_isRightToLeftInLine:(A_STextLine *)line atPoint:(CGPoint)point {
  if (!line) return NO;
  // get write direction
  BOOL RTL = NO;
  CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
 ...
 */
- (A_STextRange *)_correctedRangeWithEdge:(A_STextRange *)range {
  NSRange visibleRange = self.visibleRange;
  A_STextPosition *start = range.start;
  A_STextPosition *end = range.end;
  
  if (start.offset == visibleRange.location && start.affinity == A_STextAffinityBackward) {
    start = [A_STextPosition positionWithOffset:start.offset affinity:A_STextAffinityForward];
  }
  
  if (end.offset == visibleRange.location + visibleRange.length && start.affinity == A_STextAffinityForward) {
    end = [A_STextPosition positionWithOffset:end.offset affinity:A_STextAffinityBackward];
  }
  
  if (start != range.start || end != range.end) {
    range = [A_STextRange rangeWithStart:start end:end];
  }
  return range;
}
...
- (NSUInteger)rowIndexForLine:(NSUInteger)line {
  ...
  return ((A_STextLine *)_lines[line]).row;
}
...
- (NSUInteger)lineIndexForPoint:(CGPoint)point {
  ...
  for (NSUInteger i = lineIdx0; i <= lineIdx1; i++) {
    CGRect bounds = ((A_STextLine *)_lines[i]).bounds;
...
- (CGFloat)offsetForTextPosition:(NSUInteger)position lineIndex:(NSUInteger)lineIndex {
  ...
  A_STextLine *line = _lines[lineIndex];
...
- (NSUInteger)textPositionForPoint:(CGPoint)point lineIndex:(NSUInteger)lineIndex {
  ...
  for (NSUInteger r = 0, max = CFArrayGetCount(runs); r < max; r++) {
    ...
    if (range.location <= idx && idx < range.location + range.length) {
      ...
      CFDictionaryRef attrs = CTRunGetAttributes(run);
      CTFontRef font = (CTFontRef)CFDictionaryGetValue(attrs, kCTFontAttributeName);
      if (!A_STextCTFontContainsColorBitmapGlyphs(font)) break;
...
- (A_STextPosition *)closestPositionToPoint:(CGPoint)point {
  BOOL isVertical = _container.verticalForm;
  ...
  if (position <= _visibleRange.location) {
    return [A_STextPosition positionWithOffset:_visibleRange.location affinity:A_STextAffinityForward];
  } else if (position >= _visibleRange.location + _visibleRange.length) {
    return [A_STextPosition positionWithOffset:_visibleRange.location + _visibleRange.length affinity:A_STextAffinityBackward];
  }
  
  A_STextAffinity finalAffinity = A_STextAffinityForward;
  BOOL finalAffinityDetected = NO;
  
  // binding range
  NSRange bindingRange;
  A_STextBinding *binding = [_text attribute:A_STextBindingAttributeName atIndex:position longestEffectiveRange:&bindingRange inRange:NSMakeRange(0, _text.length)];
  if (binding && bindingRange.length > 0) {
    NSUInteger headLineIdx = [self lineIndexForPosition:[A_STextPosition positionWithOffset:bindingRange.location]];
    NSUInteger tailLineIdx = [self lineIndexForPosition:[A_STextPosition positionWithOffset:bindingRange.location + bindingRange.length affinity:A_STextAffinityBackward]];
      ...
      if (left != CGFLOAT_MAX && right != CGFLOAT_MAX) {
        if (_container.isVerticalForm) {
          if (fabs(point.y - left) < fabs(point.y - right)) {
            ...
            finalAffinity = A_STextAffinityForward;
          } else {
            position = bindingRange.location + bindingRange.length;
            finalAffinity = A_STextAffinityBackward;
          }
        } else {
  ...
  if (line.range.length == 0) {
    ...
    return [A_STextPosition positionWithOffset:line.range.location affinity:behind ? A_STextAffinityBackward:A_STextAffinityForward];
  }
  ...
  if (line.range.length <= 2) {
    ...
    if (A_STextIsLinebreakString(str)) { // an empty line ("\r", "\n", "\r\n")
      return [A_STextPosition positionWithOffset:line.range.location];
    }
  }
  ...
  if (position >= line.range.location + line.range.length - 1) {
    if (position > line.range.location) {
      ...
      if (A_STextIsLinebreakChar(c1)) {
        position--;
        if (position > line.range.location) {
          unichar c0 = [_text.string characterAtIndex:position - 1];
          if (A_STextIsLinebreakChar(c0)) {
            position--;
          }
        }
      }
  ...
  if (position == line.range.location) {
    return [A_STextPosition positionWithOffset:position];
  }
  if (position == line.range.location + line.range.length) {
    return [A_STextPosition positionWithOffset:position affinity:A_STextAffinityBackward];
  }
  ...
  if (!finalAffinityDetected) {
    ...
    if (ofs != CGFLOAT_MAX) {
      ...
      if (position >= line.range.location + line.range.length) {
        finalAffinity = RTL ? A_STextAffinityForward : A_STextAffinityBackward;
      } else if (position <= line.range.location) {
        finalAffinity = RTL ? A_STextAffinityBackward : A_STextAffinityForward;
      } else {
        finalAffinity = (ofs < (isVertical ? point.y : point.x) && !RTL) ? A_STextAffinityForward : A_STextAffinityBackward;
      }
    }
  }
  
  return [A_STextPosition positionWithOffset:position affinity:finalAffinity];
}

- (A_STextPosition *)positionForPoint:(CGPoint)point
                         oldPosition:(A_STextPosition *)oldPosition
                       otherPosition:(A_STextPosition *)otherPosition {
  if (!oldPosition || !otherPosition) {
    return oldPosition;
  }
  A_STextPosition *newPos = [self closestPositionToPoint:point];
  ...
  A_STextLine *line = _lines[lineIndex];
  A_SRowEdge vertical = _lineRowsEdge[line.row];
  if (_container.verticalForm) {
  ...
  if (_container.isVerticalForm) {
    if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
      A_STextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionUp offset:1];
      if (range) return range.start;
    } else { // search forward
      A_STextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionDown offset:1];
      if (range) return range.end;
    }
  } else {
    if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
      A_STextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionLeft offset:1];
      if (range) return range.start;
    } else { // search forward
      A_STextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionRight offset:1];
      if (range) return range.end;
    }
  }
...
- (A_STextRange *)textRangeAtPoint:(CGPoint)point {
  ...
  A_STextPosition *pos = [self closestPositionToPoint:point];
  if (!pos) return nil;
  ...
  if (_container.verticalForm) {
    A_STextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.y >= point.y && !RTL) ? UITextLayoutDirectionUp:UITextLayoutDirectionDown offset:1];
    return range;
  } else {
    A_STextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.x >= point.x && !RTL) ? UITextLayoutDirectionLeft:UITextLayoutDirectionRight offset:1];
    return range;
  }
}

- (A_STextRange *)closestTextRangeAtPoint:(CGPoint)point {
  A_STextPosition *pos = [self closestPositionToPoint:point];
  if (!pos) return nil;
  ...
  A_STextRange *range = [self textRangeByExtendingPosition:pos inDirection:direction offset:1];
  return range;
}

- (A_STextRange *)textRangeByExtendingPosition:(A_STextPosition *)position {
  ...
  if (position.offset == visibleStart) {
    return [A_STextRange rangeWithRange:NSMakeRange(position.offset, 0)];
  } else if (position.offset == visibleEnd) {
    return [A_STextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:A_STextAffinityBackward];
  }
  
  // binding range
  NSRange tRange;
  A_STextBinding *binding = [_text attribute:A_STextBindingAttributeName atIndex:position.offset longestEffectiveRange:&tRange inRange:_visibleRange];
  if (binding && tRange.length > 0 && tRange.location < position.offset) {
    return [A_STextRange rangeWithRange:tRange];
  }
  ...
  if (lineIndex != NSNotFound) {
    ...
    if (emoji || seq) {
      return [A_STextRange rangeWithRange:NSMakeRange(_prev, _next - _prev)];
    }
  }
  ...
  if (position.offset > visibleStart && position.offset < visibleEnd) {
    ...
    if ((c0 == '\r') && position.offset < visibleEnd) {
      ...
      if (c1 == '\n') {
        return [A_STextRange rangeWithStart:[A_STextPosition positionWithOffset:position.offset - 1] end:[A_STextPosition positionWithOffset:position.offset + 1]];
      }
    }
    if (A_STextIsLinebreakChar(c0) && position.affinity == A_STextAffinityBackward) {
      NSString *str = [_text.string substringToIndex:position.offset];
      NSUInteger len = A_STextLinebreakTailLength(str);
      return [A_STextRange rangeWithStart:[A_STextPosition positionWithOffset:position.offset - len] end:[A_STextPosition positionWithOffset:position.offset]];
    }
  }
  
  return [A_STextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:position.affinity];
}
...
- (A_STextRange *)textRangeByExtendingPosition:(A_STextPosition *)position
  ...
  if (!forwardMove && position.offset == visibleStart) {
    return [A_STextRange rangeWithRange:NSMakeRange(_visibleRange.location, 0)];
  } else if (forwardMove && position.offset == visibleEnd) {
    return [A_STextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:A_STextAffinityBackward];
  }
  
  // extend from position
  A_STextRange *fromRange = [self textRangeByExtendingPosition:position];
  if (!fromRange) return nil;
  A_STextRange *allForward = [A_STextRange rangeWithStart:fromRange.start end:[A_STextPosition positionWithOffset:visibleEnd]];
  A_STextRange *allBackward = [A_STextRange rangeWithStart:[A_STextPosition positionWithOffset:visibleStart] end:fromRange.end];
  ...
  if (verticalMove) { // up/down in text layout
    ...
    A_STextLine *mostLeftLine = nil, *mostRightLine = nil;
    ...
    A_STextLine *insideLine = _lines[insideIndex];
    NSUInteger pos;
    if (isVerticalForm) {
    ...
    A_STextPosition *extPos;
    if (afinityEdge) {
      if (pos == insideLine.range.location + insideLine.range.length) {
        NSString *subStr = [_text.string substringWithRange:insideLine.range];
        NSUInteger lineBreakLen = A_STextLinebreakTailLength(subStr);
        extPos = [A_STextPosition positionWithOffset:pos - lineBreakLen];
      } else {
        extPos = [A_STextPosition positionWithOffset:pos];
      }
    } else {
      extPos = [A_STextPosition positionWithOffset:pos];
    }
    A_STextRange *ext = [self textRangeByExtendingPosition:extPos];
    if (!ext) return nil;
    if (forwardMove) {
      return [A_STextRange rangeWithStart:fromRange.start end:ext.end];
    } else {
      return [A_STextRange rangeWithStart:ext.start end:fromRange.end];
    }
    
  } else { // left/right in text layout
    A_STextPosition *toPosition = [A_STextPosition positionWithOffset:position.offset + (forwardMove ? offset : -offset)];
    if (toPosition.offset <= visibleStart) return allBackward;
    else if (toPosition.offset >= visibleEnd) return allForward;
    
    A_STextRange *toRange = [self textRangeByExtendingPosition:toPosition];
    ...
    return [A_STextRange rangeWithRange:NSMakeRange(start, end - start)];
  }
}

- (NSUInteger)lineIndexForPosition:(A_STextPosition *)position {
...
- (CGPoint)linePositionForPosition:(A_STextPosition *)position {
  NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)caretRectForPosition:(A_STextPosition *)position {
  NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)firstRectForRange:(A_STextRange *)range {
  ...
  A_STextLine *startLine = _lines[startLineIndex];
  A_STextLine *endLine = _lines[endLineIndex];
  NSMutableArray *lines = [NSMutableArray new];
  for (NSUInteger i = startLineIndex; i <= startLineIndex; i++) {
    A_STextLine *line = _lines[i];
  ...
  if (_container.verticalForm) {
    if (lines.count == 1) {
      ...
      if (top > bottom) A_STEXT_SWAP(top, bottom);
    ...
    if (lines.count == 1) {
      ...
      if (left > right) A_STEXT_SWAP(left, right);
...
- (CGRect)rectForRange:(A_STextRange *)range {
  NSArray *rects = [self selectionRectsForRange:range];
  if (rects.count == 0) return CGRectNull;
  CGRect rectUnion = ((A_STextSelectionRect *)rects.firstObject).rect;
  for (NSUInteger i = 1; i < rects.count; i++) {
    A_STextSelectionRect *rect = rects[i];
    rectUnion = CGRectUnion(rectUnion, rect.rect);
  }
  return rectUnion;
}

- (NSArray *)selectionRectsForRange:(A_STextRange *)range {
  range = [self _correctedRangeWithEdge:range];
  ...
  if (startLineIndex > endLineIndex) A_STEXT_SWAP(startLineIndex, endLineIndex);
  A_STextLine *startLine = _lines[startLineIndex];
  A_STextLine *endLine = _lines[endLineIndex];
  ...
  A_STextSelectionRect *start = [A_STextSelectionRect new];
  if (isVertical) {
  ...
  A_STextSelectionRect *end = [A_STextSelectionRect new];
  if (isVertical) {
  ...
  if (startLine.row == endLine.row) { // same row
    if (offsetStart > offsetEnd) A_STEXT_SWAP(offsetStart, offsetEnd);
    A_STextSelectionRect *rect = [A_STextSelectionRect new];
    if (isVertical) {
    ...
    A_STextSelectionRect *topRect = [A_STextSelectionRect new];
    ...
    A_STextSelectionRect *bottomRect = [A_STextSelectionRect new];
...
- (NSArray *)selectionRectsWithoutStartAndEndForRange:(A_STextRange *)range {
...
- (NSArray *)selectionRectsWithOnlyStartAndEndForRange:(A_STextRange *)range {
  NSMutableArray *rects = [self selectionRectsForRange:range].mutableCopy;
  for (NSInteger i = 0, max = rects.count; i < max; i++) {
    A_STextSelectionRect *rect = rects[i];
    if (!rect.containsStart && !rect.containsEnd) {
...
typedef NS_OPTIONS(NSUInteger, A_STextDecorationType) {
  A_STextDecorationTypeUnderline     = 1 << 0,
  A_STextDecorationTypeStrikethrough = 1 << 1,
};

typedef NS_OPTIONS(NSUInteger, A_STextBorderType) {
  A_STextBorderTypeBackgound = 1 << 0,
  A_STextBorderTypeNormal    = 1 << 1,
};

static CGRect A_STextMergeRectInSameLine(CGRect rect1, CGRect rect2, BOOL isVertical) {
  if (isVertical) {
...
static void A_STextGetRunsMaxMetric(CFArrayRef runs, CGFloat *xHeight, CGFloat *underlinePosition, CGFloat *lineThickness) {
...
static void A_STextDrawRun(A_STextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
  CGAffineTransform runTextMatrix = CTRunGetTextMatrix(run);
  BOOL runTextMatrixIsID = CGAffineTransformIsIdentity(runTextMatrix);
  
  CFDictionaryRef runAttrs = CTRunGetAttributes(run);
  NSValue *glyphTransformValue = (NSValue *)CFDictionaryGetValue(runAttrs, (__bridge const void *)(A_STextGlyphTransformAttributeName));
  if (!isVertical && !glyphTransformValue) { // draw run
    ...
    fillColor = A_STextGetCGColor(fillColor);
    ...
    CGContextSaveGState(context); {
      ...
      if (isVertical) {
        ...
        for (A_STextRunGlyphRange *oneRange in runRanges) {
          NSRange range = oneRange.glyphRangeInRun;
          NSUInteger rangeMax = range.location + range.length;
          A_STextRunGlyphDrawMode mode = oneRange.drawMode;
          ...
          for (NSUInteger g = range.location; g < rangeMax; g++) {
            CGContextSaveGState(context); {
              ...
              if (mode) { // CJK glyph, need rotated
                ...
                if (mode == A_STextRunGlyphDrawModeVerticalRotateMove) {
                  x += w;
                  y += w;
                }
              ...
              if (A_STextCTFontContainsColorBitmapGlyphs(runFont)) {
...
static void A_STextSetLinePatternInContext(A_STextLineStyle style, CGFloat width, CGFloat phase, CGContextRef context){
  ...
  } else if (pattern == A_STextLineStylePatternCircleDot) {
...
static void A_STextDrawBorderRects(CGContextRef context, CGSize size, A_STextBorder *border, NSArray *rects, BOOL isVertical) {
  if (rects.count == 0) return;
  
  A_STextShadow *shadow = border.shadow;
  if (shadow.color) {
    CGContextSaveGState(context);
  ...
  for (NSValue *value in rects) {
    ...
    rect = A_STextCGRectPixelRound(rect);
  ...
  if (border.strokeColor && border.lineStyle > 0 && border.strokeWidth > 0) {
    ...
    A_STextSetLinePatternInContext(border.lineStyle, border.strokeWidth, 0, context);
    CGFloat inset = -border.strokeWidth * 0.5;
    if ((border.lineStyle & 0xFF) == A_STextLineStyleThick) {
      inset *= 2;
    ...
    //------------------------- second line ------------------------------//
    if ((border.lineStyle & 0xFF) == A_STextLineStyleDouble) {
      CGContextSaveGState(context);
...
static void A_STextDrawLineStyle(CGContextRef context, CGFloat length, CGFloat lineWidth, A_STextLineStyle style, CGPoint position, CGColorRef color, BOOL isVertical) {
  ...
  CGContextSaveGState(context); {
    if (isVertical) {
      CGFloat x, y1, y2, w;
      y1 = A_SRoundPixelValue(position.y);
      y2 = A_SRoundPixelValue(position.y + length);
      w = (styleBase == A_STextLineStyleThick ? lineWidth * 2 : lineWidth);
      
      CGFloat linePixel = A_STextCGFloatToPixel(w);
      if (fabs(linePixel - floor(linePixel)) < 0.1) {
        int iPixel = linePixel;
        if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
          x = A_STextCGFloatPixelHalf(position.x);
        } else {
          x = A_SFloorPixelValue(position.x);
        }
      } else {
        x = position.x;
      }
      ...
      A_STextSetLinePatternInContext(style, lineWidth, position.y, context);
      CGContextSetLineWidth(context, w);
      if (styleBase == A_STextLineStyleSingle) {
        CGContextMoveToPoint(context, x, y1);
        CGContextAddLineToPoint(context, x, y2);
        CGContextStrokePath(context);
      } else if (styleBase == A_STextLineStyleThick) {
        CGContextMoveToPoint(context, x, y1);
        CGContextAddLineToPoint(context, x, y2);
        CGContextStrokePath(context);
      } else if (styleBase == A_STextLineStyleDouble) {
      ...
      CGFloat x1, x2, y, w;
      x1 = A_SRoundPixelValue(position.x);
      x2 = A_SRoundPixelValue(position.x + length);
      w = (styleBase == A_STextLineStyleThick ? lineWidth * 2 : lineWidth);
      ...
      if (fabs(linePixel - floor(linePixel)) < 0.1) {
        int iPixel = linePixel;
        if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
          y = A_STextCGFloatPixelHalf(position.y);
        } else {
          y = A_SFloorPixelValue(position.y);
        }
      } else {
        y = position.y;
      }
      ...
      CGContextSetStrokeColorWithColor(context, color);
      A_STextSetLinePatternInContext(style, lineWidth, position.x, context);
      CGContextSetLineWidth(context, w);
      if (styleBase == A_STextLineStyleSingle) {
...
static void A_STextDrawText(A_STextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
  CGContextSaveGState(context); {
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
      ...
      for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
        ...
        A_STextDrawRun(line, run, context, size, isVertical, lineRunRanges[r], verticalOffset);
      }
...
static void A_STextDrawBlockBorder(A_STextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
  CGContextSaveGState(context);
  CGContextTranslateCTM(context, point.x, point.y);
  ...
  for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
    ...
    for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
      ...
      A_STextBorder *border = attrs[A_STextBlockBorderAttributeName];
      if (!border) continue;
      
      NSUInteger lineStartIndex = line.index;
      while (lineStartIndex > 0) {
        if (((A_STextLine *)lines[lineStartIndex - 1]).row == line.row) lineStartIndex--;
        else break;
      }
      
      CGRect unionRect = CGRectZero;
      NSUInteger lineStartRow = ((A_STextLine *)lines[lineStartIndex]).row;
      NSUInteger lineContinueIndex = lineStartIndex;
      NSUInteger lineContinueRow = lineStartRow;
      do {
        A_STextLine *one = lines[lineContinueIndex];
        ...
        A_STextLine *next = lines[lineContinueIndex + 1];
        if (next.row != lineContinueRow) {
          A_STextBorder *nextBorder = [layout.text as_attribute:A_STextBlockBorderAttributeName atIndex:next.range.location];
      ...
      A_STextDrawBorderRects(context, size, border, @[[NSValue valueWithCGRect:unionRect]], isVertical);
...
static void A_STextDrawBorder(A_STextLayout *layout, CGContextRef context, CGSize size, CGPoint point, A_STextBorderType type, BOOL (^cancel)(void)) {
  CGContextSaveGState(context);
  CGContextTranslateCTM(context, point.x, point.y);
  ...
  NSArray *lines = layout.lines;
  NSString *borderKey = (type == A_STextBorderTypeNormal ? A_STextBorderAttributeName : A_STextBackgroundBorderAttributeName);
  ...
  for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
    ...
    for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
      ...
      NSDictionary *attrs = (id)CTRunGetAttributes(run);
      A_STextBorder *border = attrs[borderKey];
      if (!border) continue;
      ...
      for (NSInteger ll = l; ll < lMax; ll++) {
        ...
        for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
          ...
          A_STextBorder *iBorder = iAttrs[borderKey];
          ...
          if (isVertical) {
            A_STEXT_SWAP(iRunPosition.x, iRunPosition.y);
      ...
      for (NSInteger re = 0, reMax = runRects.count; re < reMax; re++) {
        ...
        if (isVertical) {
          if (fabs(rect.origin.x - curRect.origin.x) < 1) {
            curRect = A_STextMergeRectInSameLine(rect, curRect, isVertical);
          } else {
      ...
      A_STextDrawBorderRects(context, size, border, drawRects, isVertical);
...
static void A_STextDrawDecoration(A_STextLayout *layout, CGContextRef context, CGSize size, CGPoint point, A_STextDecorationType type, BOOL (^cancel)(void)) {
  NSArray *lines = layout.lines;
  ...
  for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
    ...
    for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
      ...
      NSDictionary *attrs = (id)CTRunGetAttributes(run);
      A_STextDecoration *underline = attrs[A_STextUnderlineAttributeName];
      A_STextDecoration *strikethrough = attrs[A_STextStrikethroughAttributeName];
      
      BOOL needDrawUnderline = NO, needDrawStrikethrough = NO;
      if ((type & A_STextDecorationTypeUnderline) && underline.style > 0) {
        needDrawUnderline = YES;
      }
      if ((type & A_STextDecorationTypeStrikethrough) && strikethrough.style > 0) {
      ...
      if (A_STextIsLinebreakString(runStr)) continue; // may need more checks...
      
      CGFloat xHeight, underlinePosition, lineThickness;
      A_STextGetRunsMaxMetric(runs, &xHeight, &underlinePosition, &lineThickness);
        ...
        while (shadow) {
          ...
          CGContextSaveGState(context); {
            ...
            CGContextSaveGState(context); {
              ...
              A_STextDrawLineStyle(context, length, thickness, underline.style, underlineStart, color, isVertical);
            } CGContextRestoreGState(context);
          } CGContextRestoreGState(context);
          shadow = shadow.subShadow;
        }
      ...
      if (needDrawStrikethrough) {
        ...
        A_STextDrawLineStyle(context, length, thickness, strikethrough.style, strikethroughStart, color, isVertical);
      }
    }
  }
  CGContextRestoreGState(context);
...
static void A_STextDrawAttachment(A_STextLayout *layout, CGContextRef context, CGSize size, CGPoint point, UIView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
  
  BOOL isVertical = layout.container.verticalForm;
  CGFloat verticalOffset = isVertical ? (size.width - layout.container.size.width) : 0;
  
  for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
    A_STextAttachment *a = layout.attachments[i];
    ...
    rect = A_STextCGRectFitWithContentMode(rect, asize, a.contentMode);
    rect = A_STextCGRectPixelRound(rect);
    rect = CGRectStandardize(rect);
...
static void A_STextDrawShadow(A_STextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
  ...
  CGContextSaveGState(context); {
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
      ...
      for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
        ...
        NSDictionary *attrs = (id)CTRunGetAttributes(run);
        A_STextShadow *shadow = attrs[A_STextShadowAttributeName];
        A_STextShadow *nsShadow = [A_STextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]]; // NSShadow compatible
        if (nsShadow) {
...
static void A_STextDrawInnerShadow(A_STextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
  CGContextSaveGState(context);
  CGContextTranslateCTM(context, point.x, point.y);
  CGContextTranslateCTM(context, 0, size.height);
  CGContextScaleCTM(context, 1, -1);
  ...
  for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
    ...
    for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
      ...
      CGContextSetTextPosition(context, linePosX, linePosY);
      NSDictionary *attrs = (id)CTRunGetAttributes(run);
      A_STextShadow *shadow = attrs[A_STextInnerShadowAttributeName];
      while (shadow) {
        if (!shadow.color) {
          shadow = shadow.subShadow;
          continue;
...
static void A_STextDrawDebug(A_STextLayout *layout, CGContextRef context, CGSize size, CGPoint point, A_STextDebugOption *op) {
  UIGraphicsPushContext(context);
  CGContextSaveGState(context);
  CGContextTranslateCTM(context, point.x, point.y);
  CGContextSetLineWidth(context, 1.0 / A_SScreenScale());
  ...
  if (op.CTFrameBorderColor || op.CTFrameFillColor) {
    ...
    if (!path) {
      ...
      if (op.CTFrameBorderColor) rect = A_STextCGRectPixelHalf(rect);
      else rect = A_STextCGRectPixelRound(rect);
  ...
  for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
    ...
    if (op.CTLineFillColor) {
      ...
      CGContextAddRect(context, A_STextCGRectPixelRound(lineBounds));
      CGContextFillPath(context);
    }
    if (op.CTLineBorderColor) {
      [op.CTLineBorderColor setStroke];
      CGContextAddRect(context, A_STextCGRectPixelHalf(lineBounds));
      CGContextStrokePath(context);
    }
    if (op.baselineColor) {
      [op.baselineColor setStroke];
      if (isVertical) {
        CGFloat x = A_STextCGFloatPixelHalf(line.position.x);
        CGFloat y1 = A_STextCGFloatPixelHalf(line.top);
        CGFloat y2 = A_STextCGFloatPixelHalf(line.bottom);
        CGContextMoveToPoint(context, x, y1);
        CGContextAddLineToPoint(context, x, y2);
        CGContextStrokePath(context);
      } else {
        CGFloat x1 = A_STextCGFloatPixelHalf(lineBounds.origin.x);
        CGFloat x2 = A_STextCGFloatPixelHalf(lineBounds.origin.x + lineBounds.size.width);
        CGFloat y = A_STextCGFloatPixelHalf(line.position.y);
        CGContextMoveToPoint(context, x1, y);
        CGContextAddLineToPoint(context, x2, y);
        CGContextStrokePath(context);
      }
    ...
    if (op.CTRunFillColor || op.CTRunBorderColor || op.CTRunNumberColor || op.CGGlyphFillColor || op.CGGlyphBorderColor) {
      ...
      for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
        ...
        if (isVertical) {
          A_STEXT_SWAP(runPosition.x, runPosition.y);
        ...
        if (op.CTRunFillColor) {
          ...
          CGContextAddRect(context, A_STextCGRectPixelRound(runTypoBounds));
          CGContextFillPath(context);
        }
        if (op.CTRunBorderColor) {
          [op.CTRunBorderColor setStroke];
          CGContextAddRect(context, A_STextCGRectPixelHalf(runTypoBounds));
          CGContextStrokePath(context);
        }
        ...
        if (op.CGGlyphBorderColor || op.CGGlyphFillColor) {
          for (NSUInteger g = 0; g < glyphCount; g++) {
            ...
            if (op.CGGlyphFillColor) {
              ...
              CGContextAddRect(context, A_STextCGRectPixelRound(rect));
              CGContextFillPath(context);
            }
            if (op.CGGlyphBorderColor) {
              [op.CGGlyphBorderColor setStroke];
              CGContextAddRect(context, A_STextCGRectPixelHalf(rect));
              CGContextStrokePath(context);
            }
          }
        }
...
- (void)drawInContext:(CGContextRef)context
                ...
                point:(CGPoint)point
                 view:(UIView *)view
                layer:(CALayer *)layer
                debug:(A_STextDebugOption *)debug
               cancel:(BOOL (^)(void))cancel{
  @autoreleasepool {
    if (self.needDrawBlockBorder && context) {
      if (cancel && cancel()) return;
      A_STextDrawBlockBorder(self, context, size, point, cancel);
    }
    if (self.needDrawBackgroundBorder && context) {
      if (cancel && cancel()) return;
      A_STextDrawBorder(self, context, size, point, A_STextBorderTypeBackgound, cancel);
    }
    if (self.needDrawShadow && context) {
      if (cancel && cancel()) return;
      A_STextDrawShadow(self, context, size, point, cancel);
    }
    if (self.needDrawUnderline && context) {
      if (cancel && cancel()) return;
      A_STextDrawDecoration(self, context, size, point, A_STextDecorationTypeUnderline, cancel);
    }
    if (self.needDrawText && context) {
      if (cancel && cancel()) return;
      A_STextDrawText(self, context, size, point, cancel);
    }
    if (self.needDrawAttachment && (context || view || layer)) {
      if (cancel && cancel()) return;
      A_STextDrawAttachment(self, context, size, point, view, layer, cancel);
    }
    if (self.needDrawInnerShadow && context) {
      if (cancel && cancel()) return;
      A_STextDrawInnerShadow(self, context, size, point, cancel);
    }
    if (self.needDrawStrikethrough && context) {
      if (cancel && cancel()) return;
      A_STextDrawDecoration(self, context, size, point, A_STextDecorationTypeStrikethrough, cancel);
    }
    if (self.needDrawBorder && context) {
      if (cancel && cancel()) return;
      A_STextDrawBorder(self, context, size, point, A_STextBorderTypeNormal, cancel);
    }
    if (debug.needDrawDebug && context) {
      if (cancel && cancel()) return;
      A_STextDrawDebug(self, context, size, point, debug);
    }
  }
}
