...
- (void)reloadBounds {
  ...
  for (NSUInteger r = 0; r < runCount; r++) {
    ...
    A_STextAttachment *attachment = attrs[A_STextAttachmentAttributeName];
    if (attachment) {
      ...
      NSRange runRange = A_STextNSRangeFromCFRange(CTRunGetStringRange(run));
...
- (NSString *)description {
  ...
  [desc appendFormat:@"<A_STextLine: %p> row:%zd range:%tu,%tu",self, self.row, range.location, range.length];
