...
static A_STextDebugOption *_sharedDebugOption = nil;
...
void _as_sharedDebugSetFunction(const void *value, void *context) {
  id<A_STextDebugTarget> target = (__bridge id<A_STextDebugTarget>)(value);
...
static void _setSharedDebugOption(A_STextDebugOption *option) {
  _initSharedDebug();
...
static A_STextDebugOption *_getSharedDebugOption() {
  _initSharedDebug();
  pthread_mutex_lock(&_sharedDebugLock);
  A_STextDebugOption *op = _sharedDebugOption;
  pthread_mutex_unlock(&_sharedDebugLock);
  return op;
}

static void _addDebugTarget(id<A_STextDebugTarget> target) {
  _initSharedDebug();
  pthread_mutex_lock(&_sharedDebugLock);
...
static void _removeDebugTarget(id<A_STextDebugTarget> target) {
  _initSharedDebug();
  pthread_mutex_lock(&_sharedDebugLock);
...
@implementation A_STextDebugOption

- (id)copyWithZone:(NSZone *)zone {
  A_STextDebugOption *op = [self.class new];
...
+ (void)addDebugTarget:(id<A_STextDebugTarget>)target {
  if (target) _addDebugTarget(target);
}

+ (void)removeDebugTarget:(id<A_STextDebugTarget>)target {
  if (target) _removeDebugTarget(target);
}

+ (A_STextDebugOption *)sharedDebugOption {
  return _getSharedDebugOption();
}

+ (void)setSharedDebugOption:(A_STextDebugOption *)option {
