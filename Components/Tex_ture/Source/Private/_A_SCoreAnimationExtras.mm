...
extern void A_SDisplayNodeSetupLayerContentsWithResizableImage(CALayer *layer, UIImage *image)
{
...
{
  if (image) {
    A_SDisplayNodeCAssert(image.resizingMode == UIImageResizingModeStretch || UIEdgeInsetsEqualToEdgeInsets(image.capInsets, UIEdgeInsetsZero),
                         @"Image insets must be all-zero or resizingMode has to be UIImageResizingModeStretch. XCode assets default value is UIImageResizingModeTile which is not supported by Tex_ture because of GPU-accelerated CALayer features.");
...
NSString *A_SDisplayNodeNSStringFromUIContentMode(UIViewContentMode contentMode)
{
...
UIViewContentMode A_SDisplayNodeUIContentModeFromNSString(NSString *string)
{
  for (int i=0; i < ARRAY_COUNT(UIContentModeDescriptionLUT); i++) {
    if (A_SObjectIsEqual(UIContentModeDescriptionLUT[i].string, string)) {
...
NSString *const A_SDisplayNodeCAContentsGravityFromUIContentMode(UIViewContentMode contentMode)
{
  ...
  A_SDisplayNodeCAssert(contentMode == UIViewContentModeRedraw, @"Encountered an unknown contentMode %zd. Is this a new version of iOS?", contentMode);
  // Redraw is ok to return nil.
  return nil;
}

#define ContentModeCacheSize 10
UIViewContentMode A_SDisplayNodeUIContentModeFromCAContentsGravity(NSString *const contentsGravity)
{
  ...
  for (int i = 0; i < ARRAY_COUNT(UIContentModeCAGravityLUT); i++) {
    if (A_SObjectIsEqual(UIContentModeCAGravityLUT[i].string, contentsGravity)) {
  ...
  A_SDisplayNodeCAssert(contentsGravity, @"Encountered an unknown contentsGravity \"%@\". Is this a new version of iOS?", contentsGravity);
  A_SDisplayNodeCAssert(!contentsGravity, @"You passed nil to A_SDisplayNodeUIContentModeFromCAContentsGravity. We're falling back to resize, but this is probably a bug.");
  // If asserts disabled, fall back to this
  return UIViewContentModeScaleToFill;
}

BOOL A_SDisplayNodeLayerHasAnimations(CALayer *layer)
{
