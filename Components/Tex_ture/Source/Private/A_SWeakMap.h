...
/**
 * This class is used in conjunction with A_SWeakMap.  Instances of this type are returned by an A_SWeakMap,
 * must retain this value for as long as they want the entry to exist in the map.
 */
A_S_SUBCLASSING_RESTRICTED
@interface A_SWeakMapEntry<Value> : NSObject
 ...
 * returned a A_SWeakMapEntry and must retain it for as long as it wishes the key/value to remain in the map.
 ...
 */
A_S_SUBCLASSING_RESTRICTED
@interface A_SWeakMap<__covariant Key, Value> : NSObject

/**
 * Read from the cache.  The Value object is accessible from the returned A_SWeakMapEntry.
 */
- (nullable A_SWeakMapEntry<Value> *)entryForKey:(Key)key A_S_WARN_UNUSED_RESULT;

/**
 * Put a value into the cache.  If an entry with an equal key already exists, then the value is updated on the existing entry.
 */
- (A_SWeakMapEntry<Value> *)setObject:(Value)value forKey:(Key)key A_S_WARN_UNUSED_RESULT;
