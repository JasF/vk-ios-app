...
static CGFloat resolveCrossDimensionMaxForStretchChild(const A_SStackLayoutSpecStyle &style,
                                                       const A_SStackLayoutSpecChild &child,
                                                       ...
                                                       const CGFloat crossMax)
{
  ...
  const CGFloat computedMax = (style.direction == A_SStackLayoutDirectionVertical ?
                               A_SLayoutElementSizeResolve(child.style.size, A_SLayoutElementParentSizeUndefined).max.width :
                               A_SLayoutElementSizeResolve(child.style.size, A_SLayoutElementParentSizeUndefined).max.height);
  return computedMax == INFINITY ? crossMax : computedMax;
}

static CGFloat resolveCrossDimensionMinForStretchChild(const A_SStackLayoutSpecStyle &style,
                                                       const A_SStackLayoutSpecChild &child,
                                                       const CGFloat stackMax,
                                                       const CGFloat crossMin)
{
  ...
  return (style.direction == A_SStackLayoutDirectionVertical ?
          A_SLayoutElementSizeResolve(child.style.size, A_SLayoutElementParentSizeUndefined).min.width :
          A_SLayoutElementSizeResolve(child.style.size, A_SLayoutElementParentSizeUndefined).min.height) ?: crossMin;
}

/**
 Sizes the child given the parameters specified, and returns the computed layout.
 */
static A_SLayout *crossChildLayout(const A_SStackLayoutSpecChild &child,
...
{
  const A_SStackLayoutAlignItems alignItems = alignment(child.style.alignSelf, style.alignItems);
  // stretched children will have a cross dimension of at least crossMin
  const CGFloat childCrossMin = (alignItems == A_SStackLayoutAlignItemsStretch ?
                                 resolveCrossDimensionMinForStretchChild(style, child, stackMax, crossMin) :
                                 0);
  const CGFloat childCrossMax = (alignItems == A_SStackLayoutAlignItemsStretch ?
                                 resolveCrossDimensionMaxForStretchChild(style, child, stackMax, crossMax) :
                                 crossMax);
  const A_SSizeRange childSizeRange = directionSizeRange(style.direction, stackMin, stackMax, childCrossMin, childCrossMax);
  A_SLayout *layout = [child.element layoutThatFits:childSizeRange parentSize:parentSize];
  A_SDisplayNodeCAssertNotNil(layout, @"A_SLayout returned from -layoutThatFits:parentSize: must not be nil: %@", child.element);
  return layout ? : [A_SLayout layoutWithLayoutElement:child.element size:{0, 0}];
}
...
static void dispatchApplyIfNeeded(size_t iterationCount, BOOL forced, void(^work)(size_t i))
{
  ...
  // TODO Once the locking situation in A_SDisplayNode has improved, always dispatch if on main
  if (forced == NO) {
  ...
  A_SDispatchApply(iterationCount, queue, 0, work);
}
 ...
 */
static CGFloat computeLinesCrossDimensionSum(const std::vector<A_SStackUnpositionedLine> &lines,
                                             const A_SStackLayoutSpecStyle &style)
{
  return std::accumulate(lines.begin(), lines.end(),
                         ...
                         [&](CGFloat x, const A_SStackUnpositionedLine &l) {
 ...
 */
CGFloat A_SStackUnpositionedLayout::computeCrossViolation(const CGFloat crossDimensionSum,
                                                         const A_SStackLayoutSpecStyle &style,
...
/**
 ...
 of the items once stretched though; A_SStackPositionedLayout will do centering etc.

 Finds the maximum cross dimension among child layouts.  If that dimension exceeds the minimum cross layout size then
 we must stretch any children whose alignItems specify A_SStackLayoutAlignItemsStretch.
 ...
 */
static void stretchItemsAlongCrossDimension(std::vector<A_SStackLayoutSpecItem> &items,
                                            const A_SStackLayoutSpecStyle &style,
                                            ...
                                            const CGFloat crossSize)
{
  dispatchApplyIfNeeded(items.size(), concurrent, ^(size_t i) {
    auto &item = items[i];
    const A_SStackLayoutAlignItems alignItems = alignment(item.child.style.alignSelf, style.alignItems);
    if (alignItems == A_SStackLayoutAlignItemsStretch) {
 ...
 */
static void stretchLinesAlongCrossDimension(std::vector<A_SStackUnpositionedLine> &lines,
                                            const A_SStackLayoutSpecStyle &style,
                                            const BOOL concurrent,
                                            const A_SSizeRange &sizeRange,
                                            const CGSize parentSize)
{
  A_SDisplayNodeCAssertFalse(lines.empty());
  const std::size_t numOfLines = lines.size();
  const CGFloat violation = A_SStackUnpositionedLayout::computeCrossViolation(computeLinesCrossDimensionSum(lines, style), style, sizeRange);
  ...
  const BOOL shouldStretchLines = (numOfLines > 1
                                   && style.alignContent == A_SStackLayoutAlignContentStretch
...
static BOOL itemIsBaselineAligned(const A_SStackLayoutSpecStyle &style,
                                  const A_SStackLayoutSpecItem &l)
{
  A_SStackLayoutAlignItems alignItems = alignment(l.child.style.alignSelf, style.alignItems);
  return alignItems == A_SStackLayoutAlignItemsBaselineFirst || alignItems == A_SStackLayoutAlignItemsBaselineLast;
}

CGFloat A_SStackUnpositionedLayout::baselineForItem(const A_SStackLayoutSpecStyle &style,
                                                   const A_SStackLayoutSpecItem &item)
{
 ...
 */
static void computeLinesCrossSizeAndBaseline(std::vector<A_SStackUnpositionedLine> &lines,
                                             const A_SStackLayoutSpecStyle &style,
                                             const A_SSizeRange &sizeRange)
{
  A_SDisplayNodeCAssertFalse(lines.empty());
  ...
  if (isSingleLine && definiteCrossSize) {
    ...
    for (const auto &item : line.items) {
      if (itemIsBaselineAligned(style, item)) {
        CGFloat baseline = A_SStackUnpositionedLayout::baselineForItem(style, item);
 ...
 */
static std::function<CGFloat(const A_SStackLayoutSpecItem &)> flexFactorInViolationDirection(const CGFloat violation)
{
  if (std::fabs(violation) < kViolationEpsilon) {
    return [](const A_SStackLayoutSpecItem &item) { return 0.0; };
  } else if (violation > 0) {
    return [](const A_SStackLayoutSpecItem &item) { return item.child.style.flexGrow; };
  } else {
    return [](const A_SStackLayoutSpecItem &item) { return item.child.style.flexShrink; };
  }
}

static inline CGFloat scaledFlexShrinkFactor(const A_SStackLayoutSpecItem &item,
                                             const A_SStackLayoutSpecStyle &style,
 ...
 */
static std::function<CGFloat(const A_SStackLayoutSpecItem &)> flexShrinkAdjustment(const std::vector<A_SStackLayoutSpecItem> &items,
                                                                                  const A_SStackLayoutSpecStyle &style,
                                                                                  const CGFloat violation,
                                                                                  const CGFloat flexFactorSum)
{
  const CGFloat scaledFlexShrinkFactorSum = std::accumulate(items.begin(), items.end(), 0.0, [&](CGFloat x, const A_SStackLayoutSpecItem &item) {
    return x + scaledFlexShrinkFactor(item, style, flexFactorSum);
  });
  return [style, scaledFlexShrinkFactorSum, violation, flexFactorSum](const A_SStackLayoutSpecItem &item) {
 ...
 */
static std::function<CGFloat(const A_SStackLayoutSpecItem &)> flexGrowAdjustment(const std::vector<A_SStackLayoutSpecItem> &items,
                                                                                const CGFloat violation,
                                                                                const CGFloat flexFactorSum)
{
  // To compute the flex grow adjustment distribute the violation proportionally based on each item's flex grow factor.
  return [violation, flexFactorSum](const A_SStackLayoutSpecItem &item) {
 ...
 */
static std::function<CGFloat(const A_SStackLayoutSpecItem &)> flexAdjustmentInViolationDirection(const std::vector<A_SStackLayoutSpecItem> &items,
                                                                                                const A_SStackLayoutSpecStyle &style,
                                                                                                const CGFloat violation,
                                                                                                const CGFloat flexFactorSum)
{
...
A_SDISPLAYNODE_INLINE BOOL isFlexibleInBothDirections(const A_SStackLayoutSpecChild &child)
{
 ...
 */
static void layoutFlexibleChildrenAtZeroSize(std::vector<A_SStackLayoutSpecItem> &items,
                                             const A_SStackLayoutSpecStyle &style,
                                             const BOOL concurrent,
                                             const A_SSizeRange &sizeRange,
                                             const CGSize parentSize)
...
/**
 ...
 @param style the layout style of the overall stack layout
 */
static CGFloat computeItemsStackDimensionSum(const std::vector<A_SStackLayoutSpecItem> &items,
                                             const A_SStackLayoutSpecStyle &style)
{
  ...
  const CGFloat childSpacingSum = std::accumulate(items.begin(), items.end(),
                                                  ...
                                                  [&](CGFloat x, const A_SStackLayoutSpecItem &l) {
...
/**
 ...
 @param sizeRange the range of allowable sizes for the stack layout spec
 */
CGFloat A_SStackUnpositionedLayout::computeStackViolation(const CGFloat stackDimensionSum,
                                                         const A_SStackLayoutSpecStyle &style,
                                                         const A_SSizeRange &sizeRange)
{
 ...
 */
A_SDISPLAYNODE_INLINE BOOL useOptimizedFlexing(const std::vector<A_SStackLayoutSpecChild> &children,
                                              const A_SStackLayoutSpecStyle &style,
                                              const A_SSizeRange &sizeRange)
{
 ...
 */
static void flexLinesAlongStackDimension(std::vector<A_SStackUnpositionedLine> &lines,
                                         const A_SStackLayoutSpecStyle &style,
                                         const BOOL concurrent,
                                         const A_SSizeRange &sizeRange,
                                         const CGSize parentSize,
                                         const BOOL useOptimizedFlexing)
{
  for (auto &line : lines) {
    ...
    const CGFloat violation = A_SStackUnpositionedLayout::computeStackViolation(computeItemsStackDimensionSum(items, style), style, sizeRange);
    std::function<CGFloat(const A_SStackLayoutSpecItem &)> flexFactor = flexFactorInViolationDirection(violation);
    ...
    const CGFloat flexFactorSum = std::accumulate(items.begin(), items.end(), 0.0, [&](CGFloat x, const A_SStackLayoutSpecItem &item) {
    ...
    std::function<CGFloat(const A_SStackLayoutSpecItem &)> flexAdjustment = flexAdjustmentInViolationDirection(items,
                                                                                                              style,
                                                                                                              violation,
                                                                                                              flexFactorSum);
    // Compute any remaining violation to the first flexible item.
    const CGFloat remainingViolation = std::accumulate(items.begin(), items.end(), violation, [&](CGFloat x, const A_SStackLayoutSpecItem &item) {
 ...
 */
static std::vector<A_SStackUnpositionedLine> collectChildrenIntoLines(const std::vector<A_SStackLayoutSpecItem> &items,
                                                                     const A_SStackLayoutSpecStyle &style,
                                                                     const A_SSizeRange &sizeRange)
{
  //TODO if infinite max stack size, fast path
  if (style.flexWrap == A_SStackLayoutFlexWrapNoWrap) {
    return std::vector<A_SStackUnpositionedLine> (1, {.items = std::move(items)});
  }
  
  std::vector<A_SStackUnpositionedLine> lines;
  std::vector<A_SStackLayoutSpecItem> lineItems;
  ...
  for(auto it = items.begin(); it != items.end(); ++it) {
    ...
    const BOOL negativeViolationIfAddItem = (A_SStackUnpositionedLayout::computeStackViolation(lineStackDimensionSum + interitemSpacing + itemAndSpacingStackDimension, style, sizeRange) < 0);
    const BOOL breakCurrentLine = negativeViolationIfAddItem && !lineItems.empty();
    
    if (breakCurrentLine) {
      lines.push_back({.items = std::vector<A_SStackLayoutSpecItem> (lineItems)});
      lineItems.clear();
...
/**
 ...
 stretched.
 */
static void layoutItemsAlongUnconstrainedStackDimension(std::vector<A_SStackLayoutSpecItem> &items,
                                                        const A_SStackLayoutSpecStyle &style,
                                                        const BOOL concurrent,
                                                        const A_SSizeRange &sizeRange,
                                                        const CGSize parentSize,
                                                        const BOOL useOptimizedFlexing)
{
  ...
  dispatchApplyIfNeeded(items.size(), concurrent, ^(size_t i) {
    ...
    if (useOptimizedFlexing && isFlexibleInBothDirections(item.child)) {
      item.layout = [A_SLayout layoutWithLayoutElement:item.child.element size:{0, 0}];
    } else {
      item.layout = crossChildLayout(item.child,
                                     style,
                                     A_SDimensionResolve(item.child.style.flexBasis, stackDimension(style.direction, parentSize), 0),
                                     A_SDimensionResolve(item.child.style.flexBasis, stackDimension(style.direction, parentSize), INFINITY),
                                     minCrossDimension,
                                     maxCrossDimension,
                                     parentSize);
    }
...
A_SStackUnpositionedLayout A_SStackUnpositionedLayout::compute(const std::vector<A_SStackLayoutSpecChild> &children,
                                                             const A_SStackLayoutSpecStyle &style,
                                                             const A_SSizeRange &sizeRange,
                                                             const BOOL concurrent)
{
  ...
  // Otherwise, we pass A_SLayoutElementParentDimensionUndefined since it will depend on the content.
  const CGSize parentSize = {
    (sizeRange.min.width == sizeRange.max.width) ? sizeRange.min.width : A_SLayoutElementParentDimensionUndefined,
    (sizeRange.min.height == sizeRange.max.height) ? sizeRange.min.height : A_SLayoutElementParentDimensionUndefined,
  };
  ...
  std::vector<A_SStackLayoutSpecItem> items = A_S::map(children, [&](const A_SStackLayoutSpecChild &child) -> A_SStackLayoutSpecItem {
    return {child, nil};
  });
  ...
  std::vector<A_SStackUnpositionedLine> lines = collectChildrenIntoLines(items, style, sizeRange);
  ...
  // This should be done before `lines` are moved to a new A_SStackUnpositionedLayout struct (i.e `std::move(lines)`)
