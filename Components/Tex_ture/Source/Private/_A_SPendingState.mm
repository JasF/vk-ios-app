...
} A_SPendingStateFlags;

@implementation _A_SPendingState
{
  @package //Expose all ivars for A_SDisplayNode to bypass getters for efficiency
 ...
 */
A_SDISPLAYNODE_INLINE void A_SPendingStateApplyMetricsToLayer(_A_SPendingState *state, CALayer *layer) {
  A_SPendingStateFlags flags = state->_flags;
  if (flags.setFrame) {
    CGRect _bounds = CGRectZero;
    CGPoint _position = CGPointZero;
    A_SBoundsAndPositionForFrame(state->frame, layer.bounds.origin, layer.anchorPoint, &_bounds, &_position);
...
{
  A_SDisplayNodeAssert(!isnan(newBounds.size.width) && !isnan(newBounds.size.height), @"Invalid bounds %@ provided to %@", NSStringFromCGRect(newBounds), self);
...
{
  A_SDisplayNodeAssert(!isnan(newPosition.x) && !isnan(newPosition.y), @"Invalid position %@ provided to %@", NSStringFromCGPoint(newPosition), self);
...
{
  if (! A_SObjectIsEqual(accessibilityLabel, newAccessibilityLabel)) {
...
{
  if (! A_SObjectIsEqual(accessibilityAttributedLabel, newAccessibilityAttributedLabel)) {
    _flags.setAccessibilityAttributedLabel = YES;
    _flags.setAccessibilityLabel = YES;
...
{
  if (! A_SObjectIsEqual(accessibilityHint, newAccessibilityHint)) {
...
{
  if (! A_SObjectIsEqual(accessibilityAttributedHint, newAccessibilityAttributedHint)) {
    _flags.setAccessibilityAttributedHint = YES;
    _flags.setAccessibilityHint = YES;
...
{
  if (! A_SObjectIsEqual(accessibilityValue, newAccessibilityValue)) {
...
{
  if (! A_SObjectIsEqual(accessibilityAttributedValue, newAccessibilityAttributedValue)) {
    _flags.setAccessibilityAttributedValue = YES;
    _flags.setAccessibilityValue = YES;
...
{
  A_SPendingStateFlags flags = _flags;
  ...
  if (flags.setContentMode)
    layer.contentsGravity = A_SDisplayNodeCAContentsGravityFromUIContentMode(contentMode);
  ...
  if (flags.setOpaque)
    A_SDisplayNodeAssert(layer.opaque == opaque, @"Didn't set opaque as desired");

  A_SPendingStateApplyMetricsToLayer(self, layer);
...
- (void)applyToView:(UIView *)view withSpecialPropertiesHandling:(BOOL)specialPropertiesHandling
{
  ...
  if (A_S_AT_LEA_ST_IOS11 && flags.setAccessibilityAttributedLabel)
    [view setValue:accessibilityAttributedLabel forKey:@"accessibilityAttributedLabel"];

  if (flags.setAccessibilityHint)
    view.accessibilityHint = accessibilityHint;

  if (A_S_AT_LEA_ST_IOS11 && flags.setAccessibilityAttributedHint)
  ...
  if (A_S_AT_LEA_ST_IOS11 && flags.setAccessibilityAttributedValue)
...
+ (_A_SPendingState *)pendingViewStateFromLayer:(CALayer *)layer
{
  if (!layer) {
    return nil;
  }
  _A_SPendingState *pendingState = [[_A_SPendingState alloc] init];
  ...
  pendingState.contentMode = A_SDisplayNodeUIContentModeFromCAContentsGravity(layer.contentsGravity);
...
// FIXME: Make this more efficient by tracking which properties are set rather than reading everything.
+ (_A_SPendingState *)pendingViewStateFromView:(UIView *)view
{
  if (!view) {
    return nil;
  }
