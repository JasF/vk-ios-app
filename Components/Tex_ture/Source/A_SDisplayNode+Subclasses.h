...
@class A_SLayoutSpec, _A_SDisplayLayer;

NS_ASSUME_NONNULL_BEGIN

/**
 * The subclass header _A_SDisplayNode+Subclasses_ defines the following methods that either must or can be overriden by
 * subclasses of A_SDisplayNode.
 *
...
@protocol A_SInterfaceStateDelegate <NSObject>
@required

/**
 * @abstract Called whenever any bit in the A_SInterfaceState bitfield is changed.
...
@interface A_SDisplayNode (Subclassing) <A_SInterfaceStateDelegate>
 ...
 * the A_SLayoutSpec implementation will automatically call -layoutThatFits: on all of the subnodes,
 * and the A_SDisplayNode base class implementation of -layout will automatically make use of .calculatedLayout on the subnodes.
 *
 ...
 */
@property (nullable, nonatomic, readonly, strong) A_SLayout *calculatedLayout;
 ...
 */
- (void)didLoad A_SDISPLAYNODE_REQUIRES_SUPER;
 ...
 */
- (void)layout A_SDISPLAYNODE_REQUIRES_SUPER;
 ...
 * out.
 */
- (void)layoutDidFinish A_SDISPLAYNODE_REQUIRES_SUPER;

/**
 * @abstract Called on a background thread if !isNodeLoaded - called on the main thread if isNodeLoaded.
 *
 * @discussion When the .calculatedLayout property is set to a new A_SLayout (directly from -calculateLayoutThatFits: or
 * calculated via use of -layoutSpecThatFits:), subclasses may inspect it here.
 */
- (void)calculatedLayoutDidChange A_SDISPLAYNODE_REQUIRES_SUPER;
 ...
 *
 * @note This method should not be called directly outside of A_SDisplayNode; use -layoutThatFits: or -calculatedLayout instead.
 */
- (A_SLayout *)calculateLayoutThatFits:(A_SSizeRange)constrainedSize;

/**
 * A_SDisplayNode's implementation of -layoutThatFits:parentSize: calls this method to resolve the node's size
 ...
 * is wrapped in an A_SLayout and cached for quick access during -layout. Other expensive work that needs to
 ...
 *
 * @note This method should not be called directly outside of A_SDisplayNode; use -layoutThatFits: or layoutThatFits:parentSize: instead.
 */
 ...
 * is used to calculate an A_SLayout and cached by A_SDisplayNode for quick access during -layout. Other expensive work that needs to
 * be done before display can be performed here, and using ivars to cache any valuable intermediate results is
 * encouraged.
 *
 * @note This method should not be called directly outside of A_SDisplayNode; use -layoutThatFits: instead.
 *
...
/**
  * Declare <A_SInterfaceStateDelegate> methods as requiring super calls (this can't be required in the protocol).
  * For descriptions, see <A_SInterfaceStateDelegate> definition.
  */
...
/**
 * @abstract Called when the node's A_STraitCollection changes
 *
...
+ (void)drawRect:(CGRect)bounds withParameters:(nullable id)parameters
                                   isCancelled:(A_S_NOESCAPE asdisplaynode_iscancelled_block_t)isCancelledBlock
 ...
 * @note Called on the main thread only
 */
- (void)displayWillStart A_SDISPLAYNODE_REQUIRES_SUPER A_SDISPLAYNODE_DEPRECATED_MSG("Use displayWillStartAsynchronously: instead.");
 ...
 * @note Called on the main thread only
 */
- (void)displayWillStartAsynchronously:(BOOL)asynchronously A_SDISPLAYNODE_REQUIRES_SUPER;
 ...
 * @note Called on the main thread only
 */
- (void)displayDidFinish A_SDISPLAYNODE_REQUIRES_SUPER;

/**
 * Called just before the view is added to a window.
 */
- (void)willEnterHierarchy A_SDISPLAYNODE_REQUIRES_SUPER;

/**
 * Called after the view is removed from the window.
 */
- (void)didExitHierarchy A_SDISPLAYNODE_REQUIRES_SUPER;
 ...
 */
- (void)clearContents A_SDISPLAYNODE_REQUIRES_SUPER;
 ...
 * about to begin.
 */
- (void)subnodeDisplayWillStart:(A_SDisplayNode *)subnode A_SDISPLAYNODE_REQUIRES_SUPER;
 ...
 * completed.
 */
- (void)subnodeDisplayDidFinish:(A_SDisplayNode *)subnode A_SDISPLAYNODE_REQUIRES_SUPER;
 ...
 *
 * @return Returns a UIView, not A_SDisplayNode, for two reasons:
...
#define A_SDisplayNodeAssertThreadAffinity(viewNode)   A_SDisplayNodeAssert(!viewNode || A_SMainThreadAssertionsAreDisabled() || A_SDisplayNodeThreadIsMain() || !(viewNode).nodeLoaded, @"Incorrect display node thread affinity - this method should not be called off the main thread after the A_SDisplayNode's view or layer have been created")
#define A_SDisplayNodeCAssertThreadAffinity(viewNode) A_SDisplayNodeCAssert(!viewNode || A_SMainThreadAssertionsAreDisabled() || A_SDisplayNodeThreadIsMain() || !(viewNode).nodeLoaded, @"Incorrect display node thread affinity - this method should not be called off the main thread after the A_SDisplayNode's view or layer have been created")
