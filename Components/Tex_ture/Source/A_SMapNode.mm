...
- (instancetype)init
{
  ...
  self.backgroundColor = A_SDisplayNodeDefaultPlaceholderColor();
  ...
  _liveMap = NO;
  _annotations = @[];
  _showAnnotationsOptions = A_SMapNodeShowAnnotationsOptionsIgnored;
  return self;
}
...
{
  A_SDisplayNodeAssert(!self.isLiveMap, @"A_SMapNode can not be layer backed whilst .liveMap = YES, set .liveMap = NO to use layer backing.");
...
{
  A_SDisplayNodeAssert(!self.isLayerBacked, @"A_SMapNode can not use the interactive map feature whilst .isLayerBacked = YES, set .layerBacked = NO to use the interactive map feature.");
  A_SDN::MutexLocker l(__instanceLock__);
  if (liveMap == _liveMap) {
    return;
  }
...
- (void)takeSnapshot
{
  ...
  A_SLayout *layout = self.calculatedLayout;
...
- (void)addLiveMap
{
  ...
  if (!_mapView) {
    __weak A_SMapNode *weakSelf = self;
    ...
    A_SMapNodeShowAnnotationsOptions showAnnotationsOptions = self.showAnnotationsOptions;
    if (showAnnotationsOptions & A_SMapNodeShowAnnotationsOptionsZoomed) {
      BOOL const animated = showAnnotationsOptions & A_SMapNodeShowAnnotationsOptionsAnimated;
...
-(A_SMapNodeShowAnnotationsOptions)showAnnotationsOptions {
  A_SDN::MutexLocker l(__instanceLock__);
  return _showAnnotationsOptions;
}

-(void)setShowAnnotationsOptions:(A_SMapNodeShowAnnotationsOptions)showAnnotationsOptions {
  A_SDN::MutexLocker l(__instanceLock__);
...
- (CGSize)calculateSizeThatFits:(CGSize)constrainedSize
{
  ...
  if (!A_SIsCGSizeValidForLayout(constrainedSize)) {
    //A_SDisplayNodeAssert(NO, @"Invalid width or height in A_SMapNode");
