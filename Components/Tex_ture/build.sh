...
DERIVED_DATA_PATH="~/A_SDKDerivedData"
...
if [ "$MODE" = "tests" -o "$MODE" = "all" ]; then
    echo "Building & testing Async_DisplayKit."
    pod install
    set -o pipefail && xcodebuild \
        -workspace Async_DisplayKit.xcworkspace \
        -scheme Async_DisplayKit \
        -sdk "$SDK" \
        -destination "$PLATFORM" \
...
if [ "$MODE" = "tests_listkit" ]; then
    echo "Building & testing Async_DisplayKit+IGListKit."
    pod install --project-directory=SubspecWorkspaces/A_SDKListKit
    set -o pipefail && xcodebuild \
        -workspace SubspecWorkspaces/A_SDKListKit/A_SDKListKit.xcworkspace \
        -scheme A_SDKListKitTests \
        -sdk "$SDK" \
        -destination "$PLATFORM" \
        build-for-testing test | xcpretty $FORMATTER
    success="1"
...
if [ "$MODE" = "examples" -o "$MODE" = "all" ]; then
    echo "Verifying that all Async_DisplayKit examples compile."
...
# Support building a specific example: sh build.sh example examples/A_SDKLayoutTransition
...
if [ "$MODE" = "life-without-cocoapods" -o "$MODE" = "all" ]; then
    echo "Verifying that Async_DisplayKit functions as a static library."
...
if [ "$MODE" = "framework" -o "$MODE" = "all" ]; then
    echo "Verifying that Async_DisplayKit functions as a dynamic framework (for Swift/Carthage users)."
