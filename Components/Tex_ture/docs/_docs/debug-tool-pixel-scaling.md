...
nextPage: debug-tool-A_SRangeController.html
---

## Visualize A_SImageNode.image’s pixel scaling
<br>
This debug feature adds a red text overlay on the bottom right hand corner of an A_SImageNode if (and only if) the image’s size in pixels does not match it’s bounds size in pixels, e.g.
...
<ul>
  ...
  <li>add <code>[A_SImageNode setShouldShowImageScalingOverlay:YES]</code> at the top of your AppDelegate's <code>didFinishLaunchingWithOptions:</code> method</li>
</ul>

**Make sure to call this method before initializing any A_SImageNodes.**
